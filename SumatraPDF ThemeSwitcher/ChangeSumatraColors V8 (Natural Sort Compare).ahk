...
; --- Build naturally sorted key array ---
list := ""
keyArray := []
for k in themes
    keyArray.Push(k)

; Natural sort (handles "Color2" < "Color10" < "Color20" correctly)
Loop, % keyArray.MaxIndex() {
    i := A_Index
    Loop, % keyArray.MaxIndex() - i {
        j := A_Index + i - 1
        if (NaturalCompare(keyArray[j], keyArray[j+1]) > 0) {
            tmp := keyArray[j]
...
SearchFilter:
    ...
    newList := ""
    ; Build naturally sorted filtered list
    filteredArray := []
    for k in themes {
        if (SearchTerm = "" || InStr(k, SearchTerm))
            filteredArray.Push(k)
    }
    ; Natural sort the filtered results
    Loop, % filteredArray.MaxIndex() {
        i := A_Index
        Loop, % filteredArray.MaxIndex() - i {
            j := A_Index + i - 1
            if (NaturalCompare(filteredArray[j], filteredArray[j+1]) > 0) {
                tmp := filteredArray[j]
                filteredArray[j] := filteredArray[j+1]
                filteredArray[j+1] := tmp
            }
        }
    }
    for _, k in filteredArray
        newList .= k "|"
...
;  Handles embedded numbers so Color2 < Color10 < Color20
; -------------------------------------------------------------------
NaturalCompare(a, b) {
    Loop {
        ; If both strings exhausted — equal
        if (a = "" && b = "")
            return 0
        ; If only a exhausted — a is less
        if (a = "")
            return -1
        ; If only b exhausted — b is less
        if (b = "")
            return 1

        aIsDigit := RegExMatch(a, "^\d")
        bIsDigit := RegExMatch(b, "^\d")

        if (aIsDigit && bIsDigit) {
            ; Extract leading numbers from both
            RegExMatch(a, "^\d+", aNum)
            RegExMatch(b, "^\d+", bNum)
            diff := (aNum + 0) - (bNum + 0)
            if (diff != 0)
                return diff
            ; Numbers equal — consume them and continue
            a := SubStr(a, StrLen(aNum) + 1)
            b := SubStr(b, StrLen(bNum) + 1)
        } else {
            ; Compare single non-digit characters
            aChar := SubStr(a, 1, 1)
            bChar := SubStr(b, 1, 1)
            if (aChar != bChar) {

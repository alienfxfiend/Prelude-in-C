...
void UpdateStatus(HWND hwnd) {
    ...
    swprintf_s(status, 512,
        L"Mode: %s | Brush: %d | Color: RGB(%d,%d,%d) | Grid: %s | Zoom: %.1fx | Canvas Pos: (%lld,%lld)",
        isEraserMode ? L"Eraser" : L"Draw",
        brushSize, r, g, b,
        showGrid ? L"On" : L"Off",
        gridZoomFactor, scrollX, scrollY
    );
    g_statusText = status;
}
...
LRESULT CALLBACK WindowProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_KEYDOWN:
    {
        ...
        else if (wParam == 'A') { useAlphaGrid = !useAlphaGrid; UpdateStatus(hwnd); InvalidateRect(hwnd, NULL, FALSE); }

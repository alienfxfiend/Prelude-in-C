...
// Add this line:
std::vector<SerializedStroke> redoStack;
...
void UpdateMenuState(HWND hwnd) {
    ...
    // Add these lines:
    EnableMenuItem(hMenu, IDM_EDIT_UNDO, (!isReadOnlyMode && !strokeHistory.empty()) ? MF_ENABLED : MF_GRAYED);
    EnableMenuItem(hMenu, IDM_EDIT_REDO, (!isReadOnlyMode && !redoStack.empty()) ? MF_ENABLED : MF_GRAYED);
}
...
LRESULT CALLBACK WindowProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_COMMAND:
    {
        ...
        case IDM_EDIT_UNDO:
            if (!isReadOnlyMode && !strokeHistory.empty()) {
                std::lock_guard<std::mutex> lock(strokeMutex);
                redoStack.push_back(std::move(strokeHistory.back()));
            ...
            break;
        case IDM_EDIT_REDO:
            if (!isReadOnlyMode && !redoStack.empty()) {
                std::lock_guard<std::mutex> lock(strokeMutex);
                strokeHistory.push_back(std::move(redoStack.back()));
                redoStack.pop_back();
                offscreenDirty = true;
                unsavedChanges = true;
                UpdateStatus(hwnd);
    ...
    case WM_KEYDOWN:
    {
        ...
        // Add these lines for Ctrl+Z / Ctrl+Y support:
        if (GetKeyState(VK_CONTROL) & 0x8000) {
            if (wParam == 'Z') { SendMessage(hwnd, WM_COMMAND, IDM_EDIT_UNDO, 0); return 0; }
            if (wParam == 'Y') { SendMessage(hwnd, WM_COMMAND, IDM_EDIT_REDO, 0); return 0; }
        }

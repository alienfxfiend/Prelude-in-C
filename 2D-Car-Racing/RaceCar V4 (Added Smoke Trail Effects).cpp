...
#include <stdio.h>
#include <string>
#include <vector>        // <--- Essential for the particle system
...
struct Particle {
    ...
    float size;
    bool isDirt;
    bool isSmoke;    // New field
    float opacity;   // For smoke fade effect
};

std::vector<Particle> particles;  // Our global particle container

// Smoke Constants & Variables
const int MAX_SMOKE_PARTICLES = 100;
const float SMOKE_SPAWN_RATE = 0.15f; // seconds between particles
...
void GenerateSmoke(int carX, int carY, float carAngle, bool isPlayer) {
    float angleRad = carAngle * (float)M_PI / 180.0f;

    // Aligned to your coordinate system (0 deg = North)
    float rearOffsetX = -sinf(angleRad) * (CAR_HEIGHT / 2.0f);
    float rearOffsetY = cosf(angleRad) * (CAR_HEIGHT / 2.0f);

    // Position at rear of car
    float baseX = carX + CAR_WIDTH / 2.0f + rearOffsetX;
    float baseY = carY + CAR_HEIGHT / 2.0f + rearOffsetY;

    // Offset to tire positions (perpendicular to car direction)
    float tireSpread = isPlayer ? 15.0f : 18.0f;
    float rightX = baseX + cosf(angleRad) * tireSpread;
    float rightY = baseY + sinf(angleRad) * tireSpread;
    float leftX = baseX - cosf(angleRad) * tireSpread;
    float leftY = baseY - sinf(angleRad) * tireSpread;

    // Create left tire smoke
    particles.push_back({
        leftX, leftY,
        (rand() % 20 - 10) / 10.0f, -1.0f + (rand() % 10) / 10.0f,
        1.0f, 2.0f + (rand() % 10) / 10.0f,
        RGB(150 + rand() % 50, 150 + rand() % 50, 150 + rand() % 50),
        5.0f + rand() % 10,
        false, true, 0.7f
        });

    // Create right tire smoke
    particles.push_back({
        rightX, rightY,
        (rand() % 20 - 10) / 10.0f, -1.0f + (rand() % 10) / 10.0f,
        1.0f, 2.0f + (rand() % 10) / 10.0f,
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_TIMER:
        ...
        else if (gameStarted)
        {
            ...
            if (GetAsyncKeyState(VK_UP) & 0x8000) currentSpeed = (float)speed;
            if (GetAsyncKeyState(VK_DOWN) & 0x8000) currentSpeed = -(float)speed / 2.0f;

            if (currentSpeed != 0) {
                if (GetAsyncKeyState(VK_LEFT) & 0x8000) playerAngle -= angularVelocity;
                if (GetAsyncKeyState(VK_RIGHT) & 0x8000) playerAngle += angularVelocity;
            }

            while (playerAngle >= 360.0f) playerAngle -= 360.0f;
            while (playerAngle < 0.0f) playerAngle += 360.0f;

            float playerRadAngle = playerAngle * (float)M_PI / 180.0f;
            playerX += (int)(sinf(playerRadAngle) * currentSpeed);
            playerY -= (int)(cosf(playerRadAngle) * currentSpeed);

            // Update smoke timer and generate smoke when accelerating
            bool isAccelerating = (GetAsyncKeyState(VK_UP) & 0x8000) ||
                (GetAsyncKeyState(VK_DOWN) & 0x8000);

            if (isAccelerating) {
                smokeTimer -= 1.0f / FPS;
                if (smokeTimer <= 0) {
                    GenerateSmoke(playerX, playerY, playerAngle, true);
            ...
            // --- Collision & Spark Generation (God Mode OFF) ---
            if (!godMode)
            {
                ...
                {
                    // 1. Generate Sparks on Impact!
                    float cx = (playerX + aiX + CAR_WIDTH) * 0.5f;
                    float cy = (playerY + aiY + CAR_HEIGHT) * 0.5f;

                    for (int i = 0; i < 80; ++i)
                    {
                        float angle = (rand() % 628) / 100.0f;
                        float sparkSpeed = 8.0f + (rand() % 120) / 10.0f;
                        particles.push_back({
                            cx + (rand() % 40 - 20),
                            cy + (rand() % 40 - 20),
                            cosf(angle) * sparkSpeed,
                            sinf(angle) * sparkSpeed,
                            1.0f,
                            0.7f,
                            RGB(255, 140 + rand() % 115, rand() % 80),
                            4.0f,
                            false,
                            false, 1.0f // isSmoke, opacity
                            });
                    }

                    // 2. Professional Collision Resolution using SAT + MTV
            ...
            // --- Generate Dirt When Off-Road ---
            RECT r1 = { 0, 0, ROAD_WIDTH, HEIGHT };
            RECT r2 = { WIDTH - ROAD_WIDTH, 0, WIDTH, HEIGHT };
            RECT r3 = { 0, 0, WIDTH, ROAD_WIDTH * 2 };
            RECT car = { playerX, playerY, playerX + CAR_WIDTH, playerY + CAR_HEIGHT };
            RECT tmp;
            bool onRoad = IntersectRect(&tmp, &car, &r1) || IntersectRect(&tmp, &car, &r2) || IntersectRect(&tmp, &car, &r3);

            if (!onRoad && abs(currentSpeed) >= 3)
            {
                float px = playerX + CAR_WIDTH / 2.0f;
                float py = (float)playerY + CAR_HEIGHT + 10.0f;
                //float py = playerY + CAR_HEIGHT + 10;
                float rad = playerAngle * (float)M_PI / 180.0f;

                for (int i = 0; i < 15; ++i)
                {
                    float a = rad + 3.14159f + (rand() % 100 - 50) * 0.04f;
                    particles.push_back({
                        px + cosf(a) * 25,
                        py + sinf(a) * 25,
                        cosf(a) * 3.5f + sinf(rad) * 2.0f,
                        sinf(a) * 3.5f + cosf(rad) * 2.0f,
                        1.0f,
                        1.5f,
                        RGB(150 + rand() % 70, 110 + rand() % 60, 90 + rand() % 50),
                        14.0f,
                        true, 
                        false, 1.0f // isSmoke, opacity
                        });
        ...
        case WM_PAINT:
        {
            ...
            // ==============================================================
            //  DRAW SPARKS & DIRT PARTICLES
            // ==============================================================
            HPEN nullPen = (HPEN)GetStockObject(NULL_PEN);
            HGDIOBJ oldPen = SelectObject(memDC, nullPen); // Remove outlines for cleaner particles
            ...
            for (auto it = particles.begin(); it != particles.end(); )
            {
                ...
                // Prevent smoke from dropping quickly like rocks
                if (!it->isSmoke) {
                    it->vy += 0.5f;  // gravity for dirt/sparks
                }
                else {
                    it->vy -= 0.05f; // smoke drifts up slightly
                }

                it->life -= 0.022f;

                if (it->life <= 0.0f || it->y > HEIGHT + 100)
                {
                ...
                int b = (it->color >> 16) & 0xFF;

                if (it->isSmoke) {
                    // Fade to dark color as life decreases to simulate opacity loss in GDI
                    r = (int)(r * it->life);
                    g = (int)(g * it->life);
                    b = (int)(b * it->life);
                }
                ...
                HGDIOBJ oldBrushState = SelectObject(memDC, pBrush);

                if (it->isSmoke)
                {
                    // Smoke puff (grows as it fades)
                    int pSize = (int)(it->size * (1.0f + (1.0f - it->life)));
                    Ellipse(memDC,
                        (int)(it->x - pSize), (int)(it->y - pSize),
                        (int)(it->x + pSize), (int)(it->y + pSize));
                }
                else if (it->isDirt)
                {
                    // Dirt puff (scales down based on life)
                    int pSize = (int)(it->size * it->life);
                    Ellipse(memDC,
                        (int)(it->x - pSize), (int)(it->y - pSize),
                        (int)(it->x + pSize), (int)(it->y + pSize));
                }
                else
                {
                    ...
                    for (int j = 0; j < 4; ++j)
                    {
                        float sx = it->x + (rand() % 20 - 10) * it->life;
                        float sy = it->y + (rand() % 20 - 10) * it->life;
                        Ellipse(memDC, (int)(sx - 3), (int)(sy - 3), (int)(sx + 3), (int)(sy + 3));
                    }
                }

                SelectObject(memDC, oldBrushState);
                DeleteObject(pBrush);
                ++it;
            }
            SelectObject(memDC, oldPen); // Restore original pen state

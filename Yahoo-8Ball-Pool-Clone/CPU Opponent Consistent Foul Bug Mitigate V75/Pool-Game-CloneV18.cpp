...
// [+] NEW: WinSock UDP networking — must be included BEFORE windows.h
// to avoid conflicts with the legacy winsock.h header.
#include <winsock2.h>
#include <ws2tcpip.h>
// [+] NEW: Windows CNG (Cryptography Next Generation) for HMAC-SHA256
#include <bcrypt.h>


#include <windows.h>
#include <commdlg.h> // [+] FIX: Required for the Custom Color Picker
...
#include <xaudio2.h>  // [+] NEW: XAudio2 for robust, overlapping sound-effect playback
#include <tchar.h> //midi func
#include <thread>
#include <atomic>
#include <mutex>      // [+] FIX: Required for std::mutex and std::lock_guard
#include <queue>      // [+] FIX: Required for std::queue and std::pair
...
// [+] NEW: Dark Mode API Support
#include <dwmapi.h>
#include <uxtheme.h> // [+] NEW: Required for Menu Bar Dark Mode
#include <vssym32.h> // [+] Required for theme constants
#ifndef DWMWA_USE_IMMERSIVE_DARK_MODE
#define DWMWA_USE_IMMERSIVE_DARK_MODE 20
#endif
// [+] ADD THESE: Undocumented Windows 10/11 Menu Drawing Hooks
...
// --- Owner-draw helper for submenus ONLY ---
void SetOwnerDrawOnMenu(HMENU hMenu, bool isRoot = true) {
    ...
    for (int i = 0; i < count; ++i) {
        ...
        // CRITICAL: Do NOT set owner-draw on the root Menu Bar items!
        // This prevents the OS from reverting to legacy gray GDI during clicks.
        if (!isRoot) {
        ...
        if (mii.hSubMenu) {
            SetOwnerDrawOnMenu(mii.hSubMenu, false); // Submenus are NOT root
        }
    }
}
...
typedef union tagUAHMENUITEMMETRICS {
    struct { DWORD cx; DWORD cy; } rgsize[2];
    struct { DWORD dwPixelWidth; DWORD dwPixelHeight; } rgsizeBar[2];
} UAHMENUITEMMETRICS;

typedef struct tagUAHMENUPOPUPMETRICS {
    DWORD rgcx[4];
    DWORD fUpdateMaxWidths : 2;
    DWORD fupdateMaxHeight : 2;
} UAHMENUPOPUPMETRICS;
...
#pragma comment(lib, "ole32.lib") // [+] NEW: Required by XAudio2Create (CoCreateInstance)
#pragma comment(lib, "Comdlg32.lib") // [+] FIX: Required for the Custom Color Picker
// [+] NEW: Link Uxtheme for the Menu Bar fix
#pragma comment(lib, "uxtheme.lib") 
#pragma comment(lib, "dwmapi.lib") 
// [+] NEW: WinSock + CNG crypto libraries for networked play
#pragma comment(lib, "ws2_32.lib")
#pragma comment(lib, "bcrypt.lib")
// [+] NEW: Undocumented Menu Bar Drawing Constants
...
enum GameMode {
    HUMAN_VS_HUMAN,
    HUMAN_VS_AI,
    NETWORK_PLAY         // [+] NEW: Two human players over the internet
};
...
std::wstring g_ai1Name = L"Johnny Cache"; // [+] NEW: CPU Player 1 Name

// [+] NEW: Sprite Avatar Logic Globals
int g_p1SpriteIndex = 0;
int g_p2SpriteIndex = 13;
ID2D1Bitmap* g_pSpriteBitmaps[24] = { nullptr };
static Gdiplus::Bitmap* g_pDialogPreviewBitmaps[24] = { nullptr }; // Previews for the setup dialog
// --- End New Globals ---
...
bool g_isDemoMode = false;     // NEW: Global flag for Demo Mode (CPU vs CPU)

// [+] NEW: ==================== NETWORK PLAY GLOBALS ====================
enum NetworkRole { NR_NONE, NR_HOST, NR_CLIENT };

enum NetPacketType : uint32_t {
    NET_HELLO = 1,   // Client → Server: initiate handshake
    NET_CHALLENGE = 2,   // Server → Client: 32-byte random nonce
    NET_AUTH = 3,   // Client → Server: HMAC-SHA256(password, nonce)
    NET_AUTH_OK = 4,   // Server → Client: authentication accepted
    NET_AUTH_FAIL = 5,   // Server → Client: wrong password
    NET_SHOT = 6,   // Bidirectional: shot parameters (angle, power, spin)
    NET_STATE_SYNC = 7,   // Bidirectional: authoritative full-table state
    NET_HEARTBEAT = 8,   // Bidirectional: keep-alive / disconnect detection
    NET_DISCONNECT = 9,   // Bidirectional: graceful disconnect notice
    NET_CUEBALL_POS = 10,  // Bidirectional: ball-in-hand cue ball placement
    NET_POCKET_CALL = 11,  // Bidirectional: 8-ball called pocket
    NET_GAME_CONFIG = 12,  // Host → Client: authoritative game rules (mode, color, score)
};
...
struct NetStateSync {
    ...
    int32_t gameState;
    int32_t p1BallsPocketed;
    int32_t p2BallsPocketed;
    int32_t p1AssignedType;
    int32_t p2AssignedType;
    int32_t ballsOnTable;
    int32_t calledPocketP1;
    int32_t calledPocketP2;
    int32_t ballCount;
};
...
NetworkRole      g_netRole = NR_NONE;
SOCKET           g_netSocket = INVALID_SOCKET;
sockaddr_in      g_netPeerAddr = {};
std::atomic<bool> g_netConnected(false);
std::atomic<bool> g_netDisconnectRequested(false);
std::thread      g_netRecvThread;
std::mutex       g_netMutex;
std::queue<std::pair<sockaddr_in, std::vector<uint8_t>>> g_netIncomingPackets;
std::wstring     g_netPassword;
uint16_t         g_netPort = 5150;
std::string      g_netPeerIP = "127.0.0.1";
std::atomic<uint32_t> g_netSeqCounter(0);
std::atomic<bool> g_netAuthDone(false);
std::atomic<bool> g_netRemoteReady(false);
std::wstring     g_netStatusMsg = L"Not connected";
uint8_t          g_netChallenge[32] = {};
int              g_netHeartbeatCounter = 0;
int              g_netLastPacketFrame = 0;
int              g_netFrameCounter = 0;
// [+] END: ================ NETWORK PLAY GLOBALS ====================

// --- NEW: Hint Feature Globals ---
bool g_hintActive = false;
int g_hintTargetBallId = -1;
int g_hintIndex = 0;
...
void StartMidi(HWND hwnd, int resourceId);
void StopMidi();
...
// [+] NEW: When > 0, DrawSpinPopup() renders the large, centered 3D English
// indicator overlay.  A large value (e.g. 9999) means "actively dragging —
// keep the popup visible for the entire drag".  A small value (e.g. 90 ≈
// 1.5s) means "linger briefly after release so a quick click is also
// visible".  Decremented every frame when NOT actively dragging.
int g_spinPopupLingerCounter = 0;
...
// [+] NEW: Network function forward declarations
bool IsLocalPlayerTurn();
void NetSHA256(const uint8_t* data, size_t len, uint8_t out[32]);
void NetHMAC_SHA256(const uint8_t* key, size_t keyLen, const uint8_t* data, size_t dataLen, uint8_t out[32]);
void NetSendPacket(NetPacketType type, const void* payload, size_t payloadLen);
void NetRecvThreadFunc();
bool NetStartHost(uint16_t port, const std::wstring& password);
bool NetStartClient(const std::string& ip, uint16_t port, const std::wstring& password);
void NetDisconnect();
void NetProcessPackets();
void NetSendStateSync();
void NetSendShot(float angle, float power, float spinX, float spinY);
void NetSendCueBallPos(float x, float y);
void NetSendPocketCall(int pocket);
void NetSendHeartbeat();
INT_PTR CALLBACK NetworkSetupDlgProc(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam);
// [+] END: Network forward declarations
...
void DrawSpinPopup(ID2D1RenderTarget* pRT); // [+] NEW: Large centered popup overlay
...
D2D1_POINT_2F ComputeAdaptiveAimTarget(int pocketIndex, Ball* targetBall, Ball* cueBall);

// Helper Functions
float GetDistance(float x1, float y1, float x2, float y2);
float GetDistanceSq(float x1, float y1, float x2, float y2);
float GetDistanceToNearestRail(float x, float y); // [+] FIX: CPU foul-prevention helper
...
// Helper to extract MIDI from resource to Temp directory for MCI playback
std::wstring ExtractMidiResource(int resourceId) {
    static std::wstring tempMidiPath;
    if (!tempMidiPath.empty()) return tempMidiPath; // Already extracted this session

    wchar_t tempPath[MAX_PATH];
    GetTempPathW(MAX_PATH, tempPath);
    tempMidiPath = std::wstring(tempPath) + L"BSQ_temp.mid";

    HRSRC hResource = FindResource(GetModuleHandle(NULL), MAKEINTRESOURCE(resourceId), L"MIDI");
    if (hResource) {
        ...
        if (hLoadedResource) {
            ...
            if (pLockedResource && dwResourceSize > 0) {
                HANDLE hFile = CreateFileW(tempMidiPath.c_str(), GENERIC_WRITE, 0, NULL, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
                if (hFile != INVALID_HANDLE_VALUE) {
                    DWORD dwBytesWritten;
                    WriteFile(hFile, pLockedResource, dwResourceSize, &dwBytesWritten, NULL);
                    CloseHandle(hFile);
                }
            }
        }
...
// --- NEW: XAudio2-based sound effect playback ---
// Each effect is decoded from its WAV resource ONCE and cached in memory.
// PlayGameSound() then hands that cached buffer to a fresh, lightweight
// XAudio2 "source voice" — these are cheap internal objects, NOT OS devices,
// so any number of effects can overlap each other and the MCI MIDI music
// without stealing channels, stuttering, or getting cut off.
...
std::map<int, LoadedSound> g_loadedSounds;
IXAudio2* g_pXAudio2 = nullptr;
IXAudio2MasteringVoice* g_pMasteringVoice = nullptr;
std::vector<IXAudio2SourceVoice*> g_activeVoices;
...
bool InitAudioEngine() {
    if (g_pXAudio2) return true;

    HRESULT hr = XAudio2Create(&g_pXAudio2, 0, XAUDIO2_DEFAULT_PROCESSOR);
    if (FAILED(hr) || !g_pXAudio2) {
        g_pXAudio2 = nullptr;
        return false;
    }

    hr = g_pXAudio2->CreateMasteringVoice(&g_pMasteringVoice);
    if (FAILED(hr) || !g_pMasteringVoice) {
        g_pXAudio2->Release();
        g_pXAudio2 = nullptr;
        return false;
    }
...
// Releases all XAudio2 resources. Call once on app shutdown.
void ShutdownAudioEngine() {
    for (IXAudio2SourceVoice* pVoice : g_activeVoices) {
        pVoice->Stop(0);
        pVoice->DestroyVoice();
    }
...
void PlayGameSound(int resourceId) {
    ...
    for (size_t i = 0; i < g_activeVoices.size(); ) {
        ...
        g_activeVoices[i]->GetState(&state);
        if (state.BuffersQueued == 0) {
            g_activeVoices[i]->DestroyVoice();
            g_activeVoices.erase(g_activeVoices.begin() + i);
        }
        else {
            ++i;
        }
    ...
    if (FAILED(pSourceVoice->SubmitSourceBuffer(&buffer)) || FAILED(pSourceVoice->Start(0))) {
        pSourceVoice->DestroyVoice();
        return;
    }

    g_activeVoices.push_back(pSourceVoice);
}
// --- End Audio Helpers ---
...
// [+] FIX: Tell the compiler about our new functions so the "=" operator works!
std::vector<AIShotInfo> AIFindAllShots_8Ball();
std::vector<AIShotInfo> AIFindAllShots_StraightPool();
std::vector<AIShotInfo> AIFindAllShots_NineBall();
void TriggerAIMove();
void AIMakeDecision();
void AIPlaceCueBall();
...
// NEW
AIShotInfo EvaluateShot_8Ball(Ball* targetBall, int pocketIndex, bool isBank, bool relaxedPath = false);
...
AIShotInfo EvaluateShot_StraightPool(Ball* targetBall, int pocketIndex, bool isBank, bool relaxedPath = false);
//AIShotInfo EvaluateShot_StraightPool(Ball* targetBall, int pocketIndex, bool isBank);
AIShotInfo EvaluateShot_NineBall(Ball* targetBall, int pocketIndex, bool isBank, bool isCombo, bool relaxedPath = false);
...
// NEW
bool IsPathClear(D2D1_POINT_2F start, D2D1_POINT_2F end, int ignoredBallId1, int ignoredBallId2, float clearanceMult = 2.02f);
...
// [+] NEW: Splash Screen Declarations and Globals
Gdiplus::Bitmap* g_pSplashBitmap = nullptr;
HGLOBAL g_hSplashBuffer = nullptr;
LRESULT CALLBACK SplashWndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam);
void ShowSplashScreen(HINSTANCE hInstance);

// [+] NEW: Splash screen loading-progress animation state
int g_splashElapsedMs = 0;
const int SPLASH_TOTAL_DURATION_MS = 7000;
const int SPLASH_PROGRESS_INTERVAL_MS = 50;
const UINT_PTR SPLASH_CLOSE_TIMER_ID = 1;
const UINT_PTR SPLASH_PROGRESS_TIMER_ID = 2;
...
// [+] FIX: Declare the vector HERE, after the compiler knows what AIShotInfo is.
std::vector<AIShotInfo> g_cachedHints;
...
void SaveSettings() {
    ...
    if (outFile.is_open()) {
        ...
        std::wstring ai1_safe = g_ai1Name; std::replace(ai1_safe.begin(), ai1_safe.end(), L' ', L'_');
        outFile << p1_safe.c_str() << std::endl;
        outFile << p2_safe.c_str() << std::endl;
        outFile << ai_safe.c_str() << std::endl;
        outFile << ai1_safe.c_str() << std::endl;
...
void LoadSettings() {
    ...
    if (inFile.is_open()) {
        int gt;
        if (inFile >> gt) {
            if (gt >= GameType::EIGHT_BALL_MODE && gt <= GameType::STRAIGHT_POOL) currentGameType = static_cast<GameType>(gt);
            else currentGameType = GameType::EIGHT_BALL_MODE;
        }
        int gm, aid, obm, tc;
        ...
        if (inFile >> targetScoreStraightPool) {
        ...
        std::wstring p1_safe, p2_safe, ai_safe, ai1_safe;
        ...
        if (inFile >> ai1_safe) { std::replace(ai1_safe.begin(), ai1_safe.end(), L'_', L' '); g_ai1Name = ai1_safe; }

        inFile >> g_stats.p1_wins >> g_stats.p1_losses >> g_stats.ai_wins >> g_stats.human_vs_human_games;

        // [+] CRITICAL FIX: Clear stream fail states if older save files are missing the new data
        inFile >> g_isPracticeMode;
        if (inFile.fail()) { g_isPracticeMode = false; inFile.clear(); }

        inFile >> g_isDemoMode;
        if (inFile.fail()) { g_isDemoMode = false; inFile.clear(); }

        if (!(inFile >> g_customTableColor.r >> g_customTableColor.g >> g_customTableColor.b)) {
        ...
        if (inFile.fail()) { g_p1SpriteIndex = 0; g_p2SpriteIndex = 1; inFile.clear(); }
...
// --- Cryptographic helpers (Windows CNG / BCrypt) ---
...
void NetHMAC_SHA256(const uint8_t* key, size_t keyLen,
...
void NetSendPacket(NetPacketType type, const void* payload, size_t payloadLen) {
    if (g_netSocket == INVALID_SOCKET || !g_netConnected) return;

    std::vector<uint8_t> packet(sizeof(NetPacketHeader) + payloadLen);
    NetPacketHeader* hdr = reinterpret_cast<NetPacketHeader*>(packet.data());
    hdr->type = static_cast<uint32_t>(type);
    hdr->seq = g_netSeqCounter.fetch_add(1);
    if (payloadLen > 0)
        memcpy(packet.data() + sizeof(NetPacketHeader), payload, payloadLen);

    sendto(g_netSocket, (const char*)packet.data(), (int)packet.size(), 0,
        (sockaddr*)&g_netPeerAddr, sizeof(g_netPeerAddr));
}

void NetRecvThreadFunc() {
    char buf[4096];
    while (g_netConnected && !g_netDisconnectRequested) {
        sockaddr_in from;
        int fromLen = sizeof(from);
        int ret = recvfrom(g_netSocket, buf, sizeof(buf), 0,
            (sockaddr*)&from, &fromLen);
        if (ret > 0) {
            // Security: only accept packets from the authenticated peer
            if (g_netAuthDone &&
                (from.sin_addr.s_addr != g_netPeerAddr.sin_addr.s_addr ||
                    from.sin_port != g_netPeerAddr.sin_port))
                continue;

            std::lock_guard<std::mutex> lock(g_netMutex);
            g_netIncomingPackets.push({ from,
                std::vector<uint8_t>(buf, buf + ret) });
        }
    }
}
...
bool NetStartHost(uint16_t port, const std::wstring& password) {
    g_netSocket = socket(AF_INET, SOCK_DGRAM, IPPROTO_UDP);
    if (g_netSocket == INVALID_SOCKET) return false;
    ...
    if (bind(g_netSocket, (sockaddr*)&addr, sizeof(addr)) == SOCKET_ERROR) {
    ...
    DWORD nonBlock = 1;
    ioctlsocket(g_netSocket, FIONBIO, &nonBlock);

    g_netRole = NR_HOST;
    g_netPassword = password;
    g_netPort = port;
    g_netConnected = true;
    g_netDisconnectRequested = false;
    g_netAuthDone = false;

    // Generate a cryptographic random challenge (32 bytes)
    BCryptGenRandom(nullptr, g_netChallenge, 32, BCRYPT_USE_SYSTEM_PREFERRED_RNG);

    g_netRecvThread = std::thread(NetRecvThreadFunc);
    g_netStatusMsg = L"Hosting on port " + std::to_wstring(port) +
        L" — waiting for opponent…";
    return true;
}

bool NetStartClient(const std::string& ip, uint16_t port,
    const std::wstring& password) {
    g_netSocket = socket(AF_INET, SOCK_DGRAM, IPPROTO_UDP);
    if (g_netSocket == INVALID_SOCKET) return false;

    g_netPeerAddr.sin_family = AF_INET;
    g_netPeerAddr.sin_port = htons(port);
    inet_pton(AF_INET, ip.c_str(), &g_netPeerAddr.sin_addr);
    ...
    // Kick off handshake
    NetSendPacket(NET_HELLO, nullptr, 0);

    g_netRecvThread = std::thread(NetRecvThreadFunc);
    // [+] FIX: Safely convert std::string to std::wstring for the status message
    std::wstring wideIP(ip.length(), L'\0');
    for (size_t i = 0; i < ip.length(); ++i) {
        wideIP[i] = static_cast<wchar_t>(ip[i]);
    }
    g_netStatusMsg = L"Connecting to " + wideIP +
        L":" + std::to_wstring(port) + L" …";
    return true;
}

void NetDisconnect() {
    if (!g_netConnected) return;

    g_netDisconnectRequested = true;
    NetSendPacket(NET_DISCONNECT, nullptr, 0);

    g_netConnected = false;

    if (g_netRecvThread.joinable())
        g_netRecvThread.join();

    if (g_netSocket != INVALID_SOCKET) {
        closesocket(g_netSocket);
        g_netSocket = INVALID_SOCKET;
    }
    ...
    g_netRole = NR_NONE;
    g_netAuthDone = false;
    g_netStatusMsg = L"Disconnected";
}

// --- Process all queued incoming packets (called from GameUpdate) ---

void NetProcessPackets() {
    std::queue<std::pair<sockaddr_in, std::vector<uint8_t>>> localQ;
    {
        std::lock_guard<std::mutex> lock(g_netMutex);
        std::swap(localQ, g_netIncomingPackets);
    }

    while (!localQ.empty()) {
        auto& [from, data] = localQ.front();

        if (data.size() < sizeof(NetPacketHeader)) { localQ.pop(); continue; }

        NetPacketHeader* hdr = reinterpret_cast<NetPacketHeader*>(data.data());
        uint8_t* payload = data.data() + sizeof(NetPacketHeader);
        size_t           plen = data.size() - sizeof(NetPacketHeader);

        g_netLastPacketFrame = g_netFrameCounter;

        switch (static_cast<NetPacketType>(hdr->type)) {

            // ── Authentication handshake ──
        case NET_HELLO:
            if (g_netRole == NR_HOST && !g_netAuthDone) {
                g_netPeerAddr = from;          // remember who knocked
                NetSendPacket(NET_CHALLENGE, g_netChallenge, 32);
                g_netStatusMsg = L"Incoming connection — challenge sent…";
            }
            break;

        case NET_CHALLENGE:
            if (g_netRole == NR_CLIENT && !g_netAuthDone && plen == 32) {
                std::wstring_convert<std::codecvt_utf8<wchar_t>> conv;
                std::string pwd = conv.to_bytes(g_netPassword);
                uint8_t hmac[32];
                NetHMAC_SHA256(reinterpret_cast<const uint8_t*>(pwd.c_str()),
                    pwd.size(), payload, 32, hmac);
                NetSendPacket(NET_AUTH, hmac, 32);
                g_netStatusMsg = L"Authentication response sent…";
            }
            break;

        case NET_AUTH:
            if (g_netRole == NR_HOST && !g_netAuthDone && plen == 32) {
                std::wstring_convert<std::codecvt_utf8<wchar_t>> conv;
                std::string pwd = conv.to_bytes(g_netPassword);
                uint8_t expected[32];
                NetHMAC_SHA256(reinterpret_cast<const uint8_t*>(pwd.c_str()),
                    pwd.size(), g_netChallenge, 32, expected);
                if (memcmp(payload, expected, 32) == 0) {
                    NetSendPacket(NET_AUTH_OK, nullptr, 0);

                    // [+] NEW: Host sends the authoritative game configuration
                    // to the Client immediately after successful authentication.
                    NetGameConfig cfg = {
                        static_cast<int32_t>(currentGameType),
                        static_cast<int32_t>(selectedTableColor),
                        targetScoreStraightPool
                    };
                    NetSendPacket(NET_GAME_CONFIG, &cfg, sizeof(cfg));

                    g_netAuthDone = true;
                    g_netRemoteReady = true;
                    g_netStatusMsg = L"Authenticated! Opponent connected.";
                }
                else {
                    NetSendPacket(NET_AUTH_FAIL, nullptr, 0);
                    g_netStatusMsg = L"Authentication FAILED — wrong password!";
                    g_netDisconnectRequested = true;
                }
            }
            break;
        ...
        case NET_AUTH_OK:
            if (g_netRole == NR_CLIENT && !g_netAuthDone) {
                g_netAuthDone = true;
                g_netRemoteReady = true;
                g_netStatusMsg = L"Authenticated! Connected to host.";
            }
            break;

            // [+] NEW: Client receives game rules from Host
        case NET_GAME_CONFIG:
            if (g_netRole == NR_CLIENT && plen >= sizeof(NetGameConfig)) {
                NetGameConfig* cfg = reinterpret_cast<NetGameConfig*>(payload);
                currentGameType = static_cast<GameType>(cfg->gameType);
                selectedTableColor = static_cast<TableColor>(cfg->tableColor);
                targetScoreStraightPool = cfg->targetScore;

                // Apply the table color immediately for visual feedback
                if (selectedTableColor == CUSTOM) {
                    TABLE_COLOR = g_customTableColor;
                }
                else {
                    TABLE_COLOR = tableColorValues[selectedTableColor];
                }
                g_netStatusMsg = L"Authenticated! Host rules received.";
            }
            break;

        case NET_AUTH_FAIL:
            if (g_netRole == NR_CLIENT) {
                g_netStatusMsg = L"Authentication FAILED — wrong password!";
                g_netDisconnectRequested = true;
            }
            break;
            ...
            // ── Gameplay packets ──
        case NET_SHOT:
            if (plen >= sizeof(NetShotData)) {
                NetShotData* s = reinterpret_cast<NetShotData*>(payload);
                // The remote player took a shot — apply it locally
                firstHitBallIdThisShot = -1;
                firstHitTimeThisShot = -1.0f;
                cueHitObjectBallThisShot = false;
                railHitAfterContact = false;
                if (!g_soundEffectsMuted) PlayGameSound(IDR_WAV_CUE);
                ApplyShot(s->power, s->angle, s->spinX, s->spinY);
        ...
        case NET_STATE_SYNC:
            if (plen >= sizeof(NetStateSync)) {
                NetStateSync* sync =
                    reinterpret_cast<NetStateSync*>(payload);
                size_t off = sizeof(NetStateSync);
                size_t need = sync->ballCount * sizeof(NetBallState);
                if (plen >= off + need) {
                    // Apply authoritative state from the shooter
                    currentPlayer = sync->currentPlayer;
                    currentGameState = static_cast<GameState>(sync->gameState);
                    player1Info.ballsPocketedCount = sync->p1BallsPocketed;
                    player2Info.ballsPocketedCount = sync->p2BallsPocketed;
                    player1Info.assignedType =
                        static_cast<BallType>(sync->p1AssignedType);
                    player2Info.assignedType =
                        static_cast<BallType>(sync->p2AssignedType);
                    ballsOnTableCount = sync->ballsOnTable;
                    calledPocketP1 = sync->calledPocketP1;
                    calledPocketP2 = sync->calledPocketP2;

                    NetBallState* bs =
                        reinterpret_cast<NetBallState*>(payload + off);
                    for (int i = 0; i < sync->ballCount; ++i) {
                        Ball* b = GetBallById(bs[i].id);
                        if (b) {
                            b->x = bs[i].x;  b->y = bs[i].y;
                            b->vx = bs[i].vx; b->vy = bs[i].vy;
                            b->isPocketed = bs[i].isPocketed != 0;
                            b->rotationAngle = bs[i].rotationAngle;
                        }
                    }
                    shotPower = 0; isAiming = false; isDraggingStick = false;
                    InvalidateRect(hwndMain, nullptr, FALSE);
                }
            }
            break;

        case NET_CUEBALL_POS:
            if (plen >= sizeof(float) * 2) {
                float* pos = reinterpret_cast<float*>(payload);
                // Security: validate bounds before applying
                if (pos[0] >= TABLE_LEFT && pos[0] <= TABLE_RIGHT &&
                    pos[1] >= TABLE_TOP && pos[1] <= TABLE_BOTTOM) {
                    Ball* cue = GetCueBall();
                    if (cue) {
                        cue->x = pos[0]; cue->y = pos[1];
                        cue->vx = 0; cue->vy = 0;
                    }
                }
            }
        ...
        case NET_POCKET_CALL:
            if (plen >= sizeof(int32_t)) {
                int32_t p = *reinterpret_cast<int32_t*>(payload);
                if (currentPlayer == 1) calledPocketP2 = p;
                else                    calledPocketP1 = p;
            }
            break;

        case NET_HEARTBEAT:
            break;  // just confirms liveness

        case NET_DISCONNECT:
            g_netStatusMsg = L"Opponent disconnected!";
            g_netConnected = false;
            if (hwndMain) {
                MessageBox(hwndMain,
                    L"Your opponent has disconnected from the game.",
                    L"Network — Opponent Left",
                    MB_OK | MB_ICONINFORMATION);
                if (currentGameState != GAME_OVER) {
                    int localP = (g_netRole == NR_HOST) ? 1 : 2;
                    FinalizeGame(localP, (localP == 1) ? 2 : 1,
                        L"(Opponent Disconnected)");
                    InvalidateRect(hwndMain, nullptr, FALSE);
                }
            }
            break;
    ...
    // ── Disconnect timeout (5 seconds with no packets) ──
    if (g_netConnected && g_netAuthDone &&
        g_netFrameCounter - g_netLastPacketFrame > 300) {
        g_netStatusMsg = L"Connection timed out!";
        g_netConnected = false;
        if (hwndMain && currentGameState != GAME_OVER) {
            int localP = (g_netRole == NR_HOST) ? 1 : 2;
            FinalizeGame(localP, (localP == 1) ? 2 : 1,
                L"(Connection Lost)");
            InvalidateRect(hwndMain, nullptr, FALSE);
...
void NetSendShot(float angle, float power, float spinX, float spinY) {
    if (!g_netConnected || !g_netAuthDone) return;
    NetShotData s = { angle, power, spinX, spinY };
    // Send 3× for UDP redundancy
    for (int i = 0; i < 3; ++i)
        NetSendPacket(NET_SHOT, &s, sizeof(s));
}
...
void NetSendStateSync() {
    ...
    NetStateSync sync = {};
    sync.currentPlayer = currentPlayer;
    sync.gameState = static_cast<int32_t>(currentGameState);
    sync.p1BallsPocketed = player1Info.ballsPocketedCount;
    sync.p2BallsPocketed = player2Info.ballsPocketedCount;
    sync.p1AssignedType = static_cast<int32_t>(player1Info.assignedType);
    sync.p2AssignedType = static_cast<int32_t>(player2Info.assignedType);
    sync.ballsOnTable = ballsOnTableCount;
    sync.calledPocketP1 = calledPocketP1;
    sync.calledPocketP2 = calledPocketP2;
    sync.ballCount = static_cast<int32_t>(balls.size());

    std::vector<uint8_t> pkt(sizeof(NetStateSync) +
        balls.size() * sizeof(NetBallState));
    memcpy(pkt.data(), &sync, sizeof(NetStateSync));
    NetBallState* bs = reinterpret_cast<NetBallState*>(
        pkt.data() + sizeof(NetStateSync));
    for (size_t i = 0; i < balls.size(); ++i) {
        ...
        bs[i].vx = balls[i].vx;
        bs[i].vy = balls[i].vy;
        bs[i].isPocketed = balls[i].isPocketed ? 1 : 0;
        bs[i].rotationAngle = balls[i].rotationAngle;
    }
    NetSendPacket(NET_STATE_SYNC, pkt.data(), pkt.size());
}

void NetSendCueBallPos(float x, float y) {
    if (!g_netConnected || !g_netAuthDone) return;
    float pos[2] = { x, y };
    NetSendPacket(NET_CUEBALL_POS, pos, sizeof(pos));
}

void NetSendPocketCall(int pocket) {
    if (!g_netConnected || !g_netAuthDone) return;
    int32_t p = pocket;
    NetSendPacket(NET_POCKET_CALL, &p, sizeof(p));
}

void NetSendHeartbeat() {
    if (!g_netConnected || !g_netAuthDone) return;
    NetSendPacket(NET_HEARTBEAT, nullptr, 0);
}

// --- Helper: is it the local player's turn in network mode? ---
bool IsLocalPlayerTurn() {
    if (gameMode != NETWORK_PLAY) return true;
    int localPlayer = (g_netRole == NR_HOST) ? 1 : 2;
...
INT_PTR CALLBACK EditNamesDialogProc(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_INITDIALOG:
    {
        ...
        SetDlgItemText(hDlg, IDC_EDIT_AI1_NAME, g_ai1Name.c_str()); // [+] CPU P1
        SetDlgItemText(hDlg, IDC_EDIT_AI_NAME, g_aiName.c_str());   // CPU P2
        return (INT_PTR)TRUE;
    }
    case WM_COMMAND:
    {
        ...
        case IDOK:
        {
            ...
            GetDlgItemText(hDlg, IDC_EDIT_AI1_NAME, buffer, 256); // [+] CPU P1
            g_ai1Name = buffer;
            ...
            if (g_ai1Name.empty()) g_ai1Name = L"Johnny Cache"; // [+] CPU P1
...
// [+] NEW: Network Setup Dialog Procedure
INT_PTR CALLBACK NetworkSetupDlgProc(HWND hDlg, UINT message,
    WPARAM wParam, LPARAM lParam) {
    switch (message) {
    case WM_INITDIALOG: {
        // Default: Host mode, default port 5150
        CheckRadioButton(hDlg, IDC_RADIO_NET_HOST, IDC_RADIO_NET_CLIENT,
            IDC_RADIO_NET_HOST);

        // [+] NEW: Auto-detect the local IP address to make hosting easier
        std::wstring detectedIP = L"127.0.0.1";
        char hostName[256];
            ...
            if (getaddrinfo(hostName, nullptr, &hints, &res) == 0) {
                for (struct addrinfo* ptr = res; ptr != nullptr; ptr = ptr->ai_next) {
                    ...
                    if (inet_ntop(AF_INET, &sockAddr->sin_addr, ipStr, sizeof(ipStr)) != nullptr) {
                        // Skip loopback (127.0.0.1), we want the actual LAN IP
                        if (strcmp(ipStr, "127.0.0.1") != 0) {
                            detectedIP = std::wstring(ipStr, ipStr + strlen(ipStr));
                            break;
                        }
                    }
                }
        ...
        SetDlgItemText(hDlg, IDC_EDIT_NET_IP, detectedIP.c_str());

        SetDlgItemInt(hDlg, IDC_EDIT_NET_PORT, g_netPort, FALSE);
        SetDlgItemText(hDlg, IDC_EDIT_NET_PASSWORD, g_netPassword.c_str());
        SetDlgItemText(hDlg, IDC_STATIC_NET_STATUS,
            g_netStatusMsg.c_str());

        // Disable IP edit when Host is selected
        EnableWindow(GetDlgItem(hDlg, IDC_EDIT_NET_IP), FALSE);
    ...
    case WM_COMMAND:
        switch (LOWORD(wParam)) {
        case IDC_RADIO_NET_HOST:
            EnableWindow(GetDlgItem(hDlg, IDC_EDIT_NET_IP), FALSE);
            return (INT_PTR)TRUE;
        case IDC_RADIO_NET_CLIENT:
            EnableWindow(GetDlgItem(hDlg, IDC_EDIT_NET_IP), TRUE);
            return (INT_PTR)TRUE;

        case IDC_BTN_NET_CONNECT: {
            // Disconnect any existing session first
            if (g_netConnected) NetDisconnect();

            wchar_t buf[256];
            GetDlgItemText(hDlg, IDC_EDIT_NET_PASSWORD, buf, 256);
            g_netPassword = buf;

            BOOL ok;
            UINT portVal = GetDlgItemInt(hDlg, IDC_EDIT_NET_PORT, &ok, FALSE);
            if (!ok || portVal == 0 || portVal > 65535) {
                MessageBox(hDlg, L"Invalid port number (1–65535).",
                    L"Network Error", MB_OK | MB_ICONWARNING);
                return (INT_PTR)TRUE;
            }
            g_netPort = static_cast<uint16_t>(portVal);

            bool isHost = (IsDlgButtonChecked(hDlg, IDC_RADIO_NET_HOST)
                == BST_CHECKED);

            if (isHost) {
                if (NetStartHost(g_netPort, g_netPassword)) {
                    SetDlgItemText(hDlg, IDC_STATIC_NET_STATUS,
                        g_netStatusMsg.c_str());
                }
                else {
                    SetDlgItemText(hDlg, IDC_STATIC_NET_STATUS,
                        L"Failed to start hosting. Check port.");
                }
            }
            else {
                GetDlgItemText(hDlg, IDC_EDIT_NET_IP, buf, 256);
                std::wstring wsIP(buf);
                // [+] FIX: Explicitly convert wchar_t to char to avoid C4244 narrowing warnings
                std::string ip;
                ip.reserve(wsIP.length());
                for (size_t i = 0; i < wsIP.length(); ++i) {
                    ip.push_back(static_cast<char>(wsIP[i]));
                }

                if (NetStartClient(ip, g_netPort, g_netPassword)) {
                    SetDlgItemText(hDlg, IDC_STATIC_NET_STATUS,
                        g_netStatusMsg.c_str());
                }
                else {
        ...
        case IDOK:
            if (g_netConnected && !g_netAuthDone) {
                MessageBox(hDlg,
                    L"Connection is not yet authenticated. "
                    L"Please wait for the opponent to connect "
                    L"and authenticate, or click Cancel.",
                    L"Network — Not Ready",
            ...
            if (!g_netConnected) {
                MessageBox(hDlg,
                    L"No active network connection. "
                    L"Click \"Start / Connect\" first.",
                    L"Network — Not Connected",
                    MB_OK | MB_ICONWARNING);
                return (INT_PTR)TRUE;
            }
            EndDialog(hDlg, IDOK);
        ...
        case IDCANCEL:
            // If connected but user cancels, tear down
            if (g_netConnected && !g_netAuthDone)
                NetDisconnect();
            EndDialog(hDlg, IDCANCEL);
            return (INT_PTR)TRUE;
...
// [+] END: Network Setup Dialog Procedure

// --- NEW Dialog Procedure ---
INT_PTR CALLBACK NewGameDialogProc(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam) {
    static int s_scrollPos = 0; // Remembers our scrollbar position
    ...
    auto UpdateControlStates = [hDlg]() {
        ...
        bool isDemo = IsDlgButtonChecked(hDlg, IDC_CHECK_DEMO) == BST_CHECKED;
        ...
        // [+] NEW: Network Play disables Practice, Demo, AI, and Break controls
        bool isNetwork = IsDlgButtonChecked(hDlg, IDC_RADIO_NETWORK) == BST_CHECKED;
        EnableWindow(GetDlgItem(hDlg, IDC_CHECK_PRACTICE), !isDemo && !isNetwork);
        EnableWindow(GetDlgItem(hDlg, IDC_CHECK_DEMO), !isPractice && !isNetwork);
        EnableWindow(GetDlgItem(hDlg, IDC_BTN_NETCONFIG), isNetwork);

        if (isPractice) EnableWindow(GetDlgItem(hDlg, IDC_CHECK_DEMO), FALSE);
        else EnableWindow(GetDlgItem(hDlg, IDC_CHECK_DEMO), TRUE);

        // Disable Player Type config if Practice, Demo, or Network
        EnableWindow(GetDlgItem(hDlg, IDC_RADIO_2P), !isPractice && !isDemo);
        EnableWindow(GetDlgItem(hDlg, IDC_RADIO_CPU), !isPractice && !isDemo);
        EnableWindow(GetDlgItem(hDlg, IDC_RADIO_NETWORK), !isPractice && !isDemo);

        // AI options apply to CPU play and Demo mode
        bool enableAIOptions = (isCPUGame || isDemo) && !isPractice && !isNetwork;
        ...
        // [+] NEW: In Network mode, disable name editing and sprite selection for P2
        // (P2 name is set to the remote opponent's name from the network)
        if (isNetwork) {
            EnableWindow(GetDlgItem(hDlg, IDC_COMBO_P2_SPRITE), FALSE);
        }
        else {
            EnableWindow(GetDlgItem(hDlg, IDC_COMBO_P2_SPRITE), TRUE);
        }
    ...
    auto ForceNewGameVScroll = [hDlg]() {
        ...
        // [+] FIX: Updated content height to match the enlarged 350x270 dialog
        RECT rcContent = { 0, 0, 0, 350 };
    ...
    case WM_INITDIALOG:
    {
        ...
        for (int i = 0; i < 24; ++i) {
            if (!g_pDialogPreviewBitmaps[i]) {
                HRSRC hRes = FindResource(GetModuleHandle(NULL), MAKEINTRESOURCE(IDB_SPRITE_01 + i), L"PNG");
                if (hRes) {
                    ...
                    if (hBuffer) {
                        ...
                        if (SUCCEEDED(CreateStreamOnHGlobal(hBuffer, FALSE, &pStream))) {
                            Gdiplus::Bitmap* tmp = Gdiplus::Bitmap::FromStream(pStream);
                            if (tmp && tmp->GetLastStatus() == Gdiplus::Ok) {
                                // [+] FIX: GDI+ is lazy-loading. We MUST draw it to a new memory-backed Bitmap
                                // before we free the stream, otherwise it goes blank!
                                g_pDialogPreviewBitmaps[i] = new Gdiplus::Bitmap(tmp->GetWidth(), tmp->GetHeight(), PixelFormat32bppARGB);
                                Gdiplus::Graphics g(g_pDialogPreviewBitmaps[i]);
        ...
        // [+] NEW: Populate the ComboBoxes with Sprite Options
        for (int i = 0; i < 24; ++i) {
            std::wstring name = L"Sprite " + std::to_wstring(i + 1);
            SendDlgItemMessage(hDlg, IDC_COMBO_P1_SPRITE, CB_ADDSTRING, 0, (LPARAM)name.c_str());
            SendDlgItemMessage(hDlg, IDC_COMBO_P2_SPRITE, CB_ADDSTRING, 0, (LPARAM)name.c_str());
        }
        SendDlgItemMessage(hDlg, IDC_COMBO_P1_SPRITE, CB_SETCURSEL, g_p1SpriteIndex, 0);
        SendDlgItemMessage(hDlg, IDC_COMBO_P2_SPRITE, CB_SETCURSEL, g_p2SpriteIndex, 0);

        // [+] FIX: Force the picture boxes to render their previews immediately on startup
        ...
        // [+] NEW: Add Network Play as a third radio option
        if (gameMode == NETWORK_PLAY) {
            CheckRadioButton(hDlg, IDC_RADIO_2P, IDC_RADIO_NETWORK, IDC_RADIO_NETWORK);
        }
        ...
        CheckDlgButton(hDlg, IDC_CHECK_DEMO, g_isDemoMode ? BST_CHECKED : BST_UNCHECKED);
    ...
    { // Add an opening brace to create a new scope for the entire case.
        ...
        /* {
            ...
            // [+] NEW: Open the Network Setup sub-dialog
        case IDC_BTN_NETCONFIG:
            DialogBoxParam(
                (HINSTANCE)GetWindowLongPtr(hDlg, GWLP_HINSTANCE),
                MAKEINTRESOURCE(IDD_NETWORKDLG), hDlg,
                NetworkSetupDlgProc, 0);

            // [+] NEW: If we are the Client and connected, lock UI to match Host's rules
            if (g_netRole == NR_CLIENT && g_netConnected && g_netAuthDone) {
                HWND hGameMode = GetDlgItem(hDlg, IDC_COMBO_GAMEMODE);
                SendMessage(hGameMode, CB_SETCURSEL, (WPARAM)currentGameType, 0);
                EnableWindow(hGameMode, FALSE);

                HWND hTableColor = GetDlgItem(hDlg, IDC_COMBO_TABLECOLOR);
                SendMessage(hTableColor, CB_SETCURSEL, (WPARAM)selectedTableColor, 0);
                EnableWindow(hTableColor, FALSE);

                SetDlgItemInt(hDlg, IDC_EDIT_BALLS_WIN, targetScoreStraightPool, FALSE);
                EnableWindow(GetDlgItem(hDlg, IDC_EDIT_BALLS_WIN), FALSE);
                EnableWindow(GetDlgItem(hDlg, IDC_STATIC_BALLS_WIN), FALSE);
            }
            else {
                // Restore controls if disconnected
                EnableWindow(GetDlgItem(hDlg, IDC_COMBO_GAMEMODE), TRUE);
                EnableWindow(GetDlgItem(hDlg, IDC_COMBO_TABLECOLOR), TRUE);
                EnableWindow(GetDlgItem(hDlg, IDC_EDIT_BALLS_WIN), TRUE);
                EnableWindow(GetDlgItem(hDlg, IDC_STATIC_BALLS_WIN), TRUE);
            }
            return (INT_PTR)TRUE;
        ...
        case IDOK:
            ...
            g_isDemoMode = (IsDlgButtonChecked(hDlg, IDC_CHECK_DEMO) == BST_CHECKED);

            // Retrieve Player Type and AI settings
            if (IsDlgButtonChecked(hDlg, IDC_RADIO_NETWORK) == BST_CHECKED) {
                if (!g_netConnected || !g_netAuthDone) {
                    MessageBox(hDlg,
                        L"Network Play requires an authenticated "
                        L"connection. Click \"Configure Network…\" "
                        L"and connect before starting.",
                        L"Network — Not Connected",
                        MB_OK | MB_ICONWARNING);
                    return (INT_PTR)TRUE;
                }
                gameMode = NETWORK_PLAY;
                isPlayer2AI = false;
                g_isPracticeMode = false;
                g_isDemoMode = false;
                // Host = Player 1, Client = Player 2
                if (g_netRole == NR_HOST) {
                    player1Info.name = g_player1Name;
                    player2Info.name = L"Remote Opponent";
                }
                else {
                    player1Info.name = L"Remote Opponent";
                    player2Info.name = g_player1Name;
                }
            }
            else if ((IsDlgButtonChecked(hDlg, IDC_RADIO_CPU) == BST_CHECKED || g_isDemoMode) && !g_isPracticeMode) {
                gameMode = HUMAN_VS_AI;
                if (g_isDemoMode) isPlayer2AI = true;
            ...
            // [+] NEW: Save the Avatar dropdown indices before exiting
            g_p1SpriteIndex = (int)SendDlgItemMessage(hDlg, IDC_COMBO_P1_SPRITE, CB_GETCURSEL, 0, 0);
            g_p2SpriteIndex = (int)SendDlgItemMessage(hDlg, IDC_COMBO_P2_SPRITE, CB_GETCURSEL, 0, 0);
        ...
        case IDCANCEL: // Handle Cancel or closing the dialog
            ...
            // [+] NEW: Force image updates when Dropdowns change
        case IDC_COMBO_P1_SPRITE:
        case IDC_COMBO_P2_SPRITE:
            if (HIWORD(wParam) == CBN_SELCHANGE) {
                int picId = (LOWORD(wParam) == IDC_COMBO_P1_SPRITE) ? IDC_PIC_P1_SPRITE : IDC_PIC_P2_SPRITE;
                HWND hPic = GetDlgItem(hDlg, picId);
                InvalidateRect(hPic, NULL, TRUE); // Instantly redraws the preview box
            }
            return (INT_PTR)TRUE;
        }
    ...
    // [+] FOOLPROOF FIX: Safely draw GDI+ PNGs directly into the Static Preview controls
    case WM_DRAWITEM:
    {
        LPDRAWITEMSTRUCT dis = (LPDRAWITEMSTRUCT)lParam;
        // [FIX] For SS_OWNERDRAW static controls, the control's resource ID is in
        // dis->CtlID, NOT dis->itemID. itemID is always 0 for static controls
        // (it is only meaningful for list/combo items and menus).
        if (dis->CtlType == ODT_STATIC && (dis->CtlID == IDC_PIC_P1_SPRITE || dis->CtlID == IDC_PIC_P2_SPRITE)) {

            // 1. Draw the standard dialog background so the transparency isn't messy
            FillRect(dis->hDC, &dis->rcItem, GetSysColorBrush(COLOR_BTNFACE));

            // 2. Determine which combobox to check based on the image being drawn
            int comboId = (dis->CtlID == IDC_PIC_P1_SPRITE) ? IDC_COMBO_P1_SPRITE : IDC_COMBO_P2_SPRITE;
            int selectedIdx = (int)SendDlgItemMessage(hDlg, comboId, CB_GETCURSEL, 0, 0);

            // 3. Draw the corresponding GDI+ PNG preserving the alpha layer!
            if (selectedIdx >= 0 && selectedIdx < 24 && g_pDialogPreviewBitmaps[selectedIdx]) {
                Gdiplus::Graphics graphics(dis->hDC);
                // Dynamically fit to the static control's boundaries
                graphics.DrawImage(g_pDialogPreviewBitmaps[selectedIdx],
                    0, 0, dis->rcItem.right - dis->rcItem.left, dis->rcItem.bottom - dis->rcItem.top);
            }
            return (INT_PTR)TRUE;
        }
        break;
...
void ShowNewGameDialog(HINSTANCE hInstance) {
    if (DialogBoxParam(hInstance, MAKEINTRESOURCE(IDD_NEWGAMEDLG), hwndMain, NewGameDialogProc, 0) == IDOK) {
        ...
        // [+] NEW: Network play — assign names based on role
        if (gameMode == NETWORK_PLAY) {
            if (g_netRole == NR_HOST) {
                player1Info.name = g_player1Name;
                player2Info.name = L"Remote Player";
            }
            else {
                player1Info.name = L"Remote Player";
                player2Info.name = g_player1Name;
            }
        }

        // [+] FIX: Assign Johnny Cache to Demo Mode
        // [+] FIX: Assign CPU P1 Name to Demo Mode
        if (g_isDemoMode) {
            player1Info.name = g_ai1Name;
        ...
        // [+] CRITICAL FIX: Ensure Custom Color is loaded from memory, NOT the placeholder array!
        if (selectedTableColor == CUSTOM) {
            TABLE_COLOR = g_customTableColor;
        }
        else {
...
// [+] NEW: Robust Dark Mode Logic (Process Level)
// Ordinal 135: SetPreferredAppMode - Forces the process into Dark Mode logic early.
void ForceDarkMode_Process() {
    HMODULE hUxtheme = LoadLibraryEx(L"uxtheme.dll", nullptr, LOAD_LIBRARY_SEARCH_SYSTEM32);
    if (hUxtheme) {
        // 135 = SetPreferredAppMode (Force Dark = 2)
...
// [+] NEW: Robust Dark Mode Logic (Window Level)
// Ordinal 133: AllowDarkModeForWindow - Tells UxTheme this specific HWND is allowed to be dark.
// Ordinal 136: FlushMenuThemes - Clears the white cached menu bar paint.
void ForceDarkMode_Window(HWND hwnd) {
    HMODULE hUxtheme = LoadLibraryEx(L"uxtheme.dll", nullptr, LOAD_LIBRARY_SEARCH_SYSTEM32);
    if (hUxtheme) {
        ...
        if (AllowDarkModeForWindow) {
            AllowDarkModeForWindow(hwnd, true); // Explicitly permit dark painting for this HWND
        }

        // IMPORTANT: If the menu was already created "white", this forces it to re-evaluate colors.
        if (FlushMenuThemes) {
            FlushMenuThemes();
        }

        // Extra: Set "Explorer" theme which supports Dark Mode better than default "Windows" theme
...
// [+] NEW: Splash Screen Window Procedure
LRESULT CALLBACK SplashWndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    switch (msg)
    {
    case WM_CREATE:
        g_splashElapsedMs = 0;
        SetTimer(hwnd, SPLASH_CLOSE_TIMER_ID, SPLASH_TOTAL_DURATION_MS, NULL);
        SetTimer(hwnd, SPLASH_PROGRESS_TIMER_ID, SPLASH_PROGRESS_INTERVAL_MS, NULL);
        return 0;

    case WM_ERASEBKGND:
        return 1; // Prevent background erasure (reduces flicker)
    ...
    case WM_PAINT:
    {
        ...
        HBITMAP hbmMem = CreateCompatibleBitmap(hdc, 768, 576);
        HGDIOBJ hOldBmp = SelectObject(hdcMem, hbmMem);

        if (g_pSplashBitmap)
        {
            Gdiplus::Graphics graphics(hdcMem);
            graphics.DrawImage(g_pSplashBitmap, 0, 0, 768, 576);
        }

        // Border
        HPEN hPen = CreatePen(PS_SOLID, 1, RGB(0, 0, 0));
        HGDIOBJ hOld = SelectObject(hdcMem, hPen);
        SelectObject(hdcMem, GetStockObject(NULL_BRUSH));
        Rectangle(hdcMem, 0, 0, 768, 576);
        ...
        // Progress bar
        {
            ...
            const int barTop = 576 - 24;

            float progress = (float)g_splashElapsedMs / SPLASH_TOTAL_DURATION_MS;
            if (progress < 0.0f) progress = 0.0f;
            if (progress > 1.0f) progress = 1.0f;

            RECT trackRect = { barMargin, barTop, 768 - barMargin, barTop + barHeight };

            HBRUSH hTrackBrush = CreateSolidBrush(RGB(40, 40, 40));
            FillRect(hdcMem, &trackRect, hTrackBrush);
            DeleteObject(hTrackBrush);

            int fillWidth = (int)((trackRect.right - trackRect.left) * progress);
            RECT fillRect = { trackRect.left, trackRect.top, trackRect.left + fillWidth, trackRect.bottom };
            HBRUSH hFillBrush = CreateSolidBrush(RGB(0, 180, 255));
            FillRect(hdcMem, &fillRect, hFillBrush);
            DeleteObject(hFillBrush);

            HPEN hBarPen = CreatePen(PS_SOLID, 1, RGB(255, 255, 255));
            HGDIOBJ hOldPen = SelectObject(hdcMem, hBarPen);
            SelectObject(hdcMem, GetStockObject(NULL_BRUSH));
            Rectangle(hdcMem, trackRect.left, trackRect.top, trackRect.right, trackRect.bottom);
            SelectObject(hdcMem, hOldPen);
            DeleteObject(hBarPen);
        }

        BitBlt(hdc, 0, 0, 768, 576, hdcMem, 0, 0, SRCCOPY);
    ...
    case WM_TIMER:
        if (wParam == SPLASH_PROGRESS_TIMER_ID)
        {
            g_splashElapsedMs += SPLASH_PROGRESS_INTERVAL_MS;
            if (g_splashElapsedMs > SPLASH_TOTAL_DURATION_MS)
                g_splashElapsedMs = SPLASH_TOTAL_DURATION_MS;

            // Only invalidate the progress bar area
            RECT barRect = { 0, 576 - 30, 768, 576 };
        ...
        KillTimer(hwnd, SPLASH_PROGRESS_TIMER_ID);
        DestroyWindow(hwnd);
        return 0;
...
/*LRESULT CALLBACK SplashWndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {
    switch (msg) {
    case WM_CREATE:
        // Set a timer for 7000 milliseconds (7 seconds) to auto-close the splash screen
        g_splashElapsedMs = 0;
        SetTimer(hwnd, SPLASH_CLOSE_TIMER_ID, SPLASH_TOTAL_DURATION_MS, NULL);
        // [+] NEW: Faster timer drives the loading progress bar animation
        SetTimer(hwnd, SPLASH_PROGRESS_TIMER_ID, SPLASH_PROGRESS_INTERVAL_MS, NULL);
        return 0;
    case WM_PAINT: {
        PAINTSTRUCT ps;
        ...
        if (g_pSplashBitmap) {
            ...
            // Draw the 768x576 image cleanly onto the window
            graphics.DrawImage(g_pSplashBitmap, 0, 0, 768, 576);

            // Draw a subtle black border around the splash screen for a professional edge
            HPEN hPen = CreatePen(PS_SOLID, 1, RGB(0, 0, 0));
            HGDIOBJ hOld = SelectObject(hdc, hPen);
            SelectObject(hdc, GetStockObject(NULL_BRUSH));
            Rectangle(hdc, 0, 0, 768, 576);
            SelectObject(hdc, hOld);
            DeleteObject(hPen);

            // [+] NEW: Draw a loading progress bar along the bottom edge
            {
                const int barMargin = 40;
                const int barHeight = 8;
                const int barTop = 576 - 24;

                float progress = (float)g_splashElapsedMs / (float)SPLASH_TOTAL_DURATION_MS;
                if (progress < 0.0f) progress = 0.0f;
                if (progress > 1.0f) progress = 1.0f;
                ...
                HPEN hBarPen = CreatePen(PS_SOLID, 1, RGB(255, 255, 255));
                HGDIOBJ hOldBarPen = SelectObject(hdc, hBarPen);
                SelectObject(hdc, GetStockObject(NULL_BRUSH));
    ...
    case WM_LBUTTONDOWN: // Allow the user to click the splash screen to skip it
    case WM_KEYDOWN:     // Allow the user to press any key to skip it
        KillTimer(hwnd, SPLASH_CLOSE_TIMER_ID);
        KillTimer(hwnd, SPLASH_PROGRESS_TIMER_ID);
    ...
    case WM_TIMER:
        if (wParam == SPLASH_PROGRESS_TIMER_ID) {
            // [+] NEW: Advance the progress bar animation and repaint just that strip
            g_splashElapsedMs += SPLASH_PROGRESS_INTERVAL_MS;
            if (g_splashElapsedMs > SPLASH_TOTAL_DURATION_MS) g_splashElapsedMs = SPLASH_TOTAL_DURATION_MS;
            RECT barRect = { 0, 576 - 24, 768, 576 };
            InvalidateRect(hwnd, &barRect, FALSE);
            return 0;
        }
        // Timer ID == SPLASH_CLOSE_TIMER_ID: automatically close after 7 seconds
        KillTimer(hwnd, SPLASH_CLOSE_TIMER_ID);
        KillTimer(hwnd, SPLASH_PROGRESS_TIMER_ID);
        DestroyWindow(hwnd); // [+] CRITICAL FIX: Safely destroy the window. Do NOT use PostQuitMessage!
        return 0;
...
// [+] NEW: Splash Screen Launcher Function
void ShowSplashScreen(HINSTANCE hInstance) {
    // 1. Load the JPEG resource from the executable
    HRSRC hResource = FindResource(hInstance, MAKEINTRESOURCE(IDB_SPLASH_IMG), L"JPG");
    if (!hResource) return;
    ...
    // 2. Load the resource into a GDI+ Bitmap
    g_hSplashBuffer = GlobalAlloc(GMEM_MOVEABLE, imageSize);
    if (g_hSplashBuffer) {
        void* pBuffer = GlobalLock(g_hSplashBuffer);
        if (pBuffer) {
            CopyMemory(pBuffer, pResourceData, imageSize);
            IStream* pStream = NULL;
            if (CreateStreamOnHGlobal(g_hSplashBuffer, FALSE, &pStream) == S_OK) {
                g_pSplashBitmap = Gdiplus::Bitmap::FromStream(pStream);
                pStream->Release();
            }
            GlobalUnlock(g_hSplashBuffer);
        }
    }

    if (!g_pSplashBitmap) return; // Failsafe

    // 3. Register the Splash Screen Window Class
    WNDCLASS wc = { 0 };
    wc.lpfnWndProc = SplashWndProc;
    wc.hInstance = hInstance;
    wc.hCursor = LoadCursor(NULL, IDC_APPSTARTING); // Show loading cursor
    wc.lpszClassName = L"SplashClass";
    RegisterClass(&wc);

    // 4. Calculate exact center of the screen for a 768x576 image
    int width = 768;
    int height = 576;
    ...
    // 5. Create the Borderless Window (Topmost so it stays in front)
    HWND hwndSplash = CreateWindowEx(
        WS_EX_TOPMOST | WS_EX_TOOLWINDOW,
        L"SplashClass", L"Splash",
        WS_POPUP | WS_VISIBLE,
        x, y, width, height,
        NULL, NULL, hInstance, NULL
    );

    // 6. Local Message Loop: [+] CRITICAL FIX: Safe PeekMessage loop
    // This perfectly loops until the window is destroyed, without poisoning the main app thread.
    MSG msg;
    while (IsWindow(hwndSplash)) {
        if (PeekMessage(&msg, NULL, 0, 0, PM_REMOVE)) {
            TranslateMessage(&msg);
            DispatchMessage(&msg);
        }
        else {
            Sleep(10); // Prevent CPU hogging while waiting
        }
    }

    // 7. Cleanup Resources after Splash Screen is gone
    UnregisterClass(L"SplashClass", hInstance);

    if (g_pSplashBitmap) {
        delete g_pSplashBitmap;
        g_pSplashBitmap = nullptr;
    }
    if (g_hSplashBuffer) {
        GlobalFree(g_hSplashBuffer);
        g_hSplashBuffer = nullptr;
    }
}
...
int WINAPI wWinMain(HINSTANCE hInstance, HINSTANCE, PWSTR, int nCmdShow) {
    ...
    // [+] NEW: Initialize WinSock for networked play
    WSADATA wsaData;
    if (WSAStartup(MAKEWORD(2, 2), &wsaData) != 0) {
        MessageBox(NULL, L"WinSock Initialization Failed.",
            L"Network Error", MB_OK | MB_ICONERROR);
        CoUninitialize();
        return -1;
    }

    // [+] NEW: Execute the 7-Second Splash Screen BEFORE the Game Setup Dialog!
    // The program will intentionally halt here until the splash screen naturally closes.
    ShowSplashScreen(hInstance);

    // --- NEW: Load settings at startup ---
    LoadSettings();

    // [+] NEW: Force Process-Level Dark Mode (Fixes White Menu Bar via AHK)    
    // [+] NEW: Stage 1 - Force Process to prefer Dark Mode (Before Window Creation)
    ForceDarkMode_Process();
    ...
    // [+] FIX: Assign Johnny Cache to Demo Mode on App Launch
    // [+] FIX: Assign CPU P1 Name to Demo Mode on App Launch
    if (g_isDemoMode) {
        player1Info.name = g_ai1Name;
        player2Info.name = g_aiName;
    }
    ...
    //}
//} //newlydisabled march3-2026
//else { //newlydisabled march3-2026
    //player2Info.name = g_player2Name; //newlydisabled march3-2026
...
// --- End of Dialog Logic ---
    ...
    // Set all menu items to owner-draw for custom theming
    if (hMenu) {
        SetOwnerDrawOnMenu(hMenu);
    }*/

    hwndMain = CreateWindowEx(
        0, L"BLISS_GameEngine"/*"Direct2D_8BallPool"*/, L"Midnight Pool 4", dwStyle, // Create with a temporary title
        windowX, windowY, windowWidth, windowHeight,
        NULL, NULL, hInstance, NULL // Pass hMenu here
        //hwndMain = CreateWindowEx(
    ...
    // 3. Set the specific "Explorer" theme that supports dark menus
    SetWindowTheme(hwndMain, L"DarkMode_Explorer", NULL);

    // 4. Force a frame update to sync the Menu Bar
    ...
    // [+] ADD THIS: Sync the Menu State immediately after Window Creation
    ...
    // ----------------------------------------------------------------------
    StartMidi(hwndMain, IDR_MID_BGMUSIC); // Extract and Play Embedded MIDI
    ...
    // [+] NEW: Free the GDI+ setup dialog preview bitmaps to stop mem leaks
    for (int i = 0; i < 24; ++i) {
        if (g_pDialogPreviewBitmaps[i]) {
            delete g_pDialogPreviewBitmaps[i];
            g_pDialogPreviewBitmaps[i] = nullptr;
        }
    }
    ...
    NetDisconnect();      // [+] NEW: Clean up network connection
    WSACleanup();         // [+] NEW: Release WinSock
    ShutdownAudioEngine(); // [+] NEW: Release XAudio2 engine and any active sound voices
    CoUninitialize();
...
void UpdateWindowTitle() {
    ...
    if (g_isPracticeMode) {
    ...
    else if (g_isDemoMode) {
        windowTitle += L" (Demo Mode)";
        ...
        else if (gameMode == NETWORK_PLAY) {
            windowTitle += L" - Network: " + player1Info.name +
                L" vs " + player2Info.name;
        }
        else {
            windowTitle += L" - Human vs " + player2Info.name;
        }
    }
...
/*// Place this BEFORE WndProc (or in a header)
static void DrawDarkMenuBar(HWND hwnd) {
    ...
    // B. Draw menu item text (File, Game, Help, etc.)
    ...
    for (int i = 0; i < itemCount; i++) {
        ...
        if (GetMenuItemRect(hwnd, hMenu, i, &rcItem)) {
            ...
            if (GetMenuString(hMenu, i, menuText, 256, MF_BYPOSITION)) {
                DrawText(hdc, menuText, -1, &rcItem, DT_CENTER | DT_VCENTER | DT_SINGLELINE);
            }
        }
    }
...
// [+] ADD THIS: Dynamic Menu Creation (Bypasses WindowBlinds .rc Hooks)
void CreateAppMenu(HWND hwnd) {
    HMENU hMenuBar = CreateMenu();

    // 1. File Menu
    HMENU hFile = CreatePopupMenu();
    AppendMenu(hFile, MF_STRING, ID_FILE_NEWGAME, L"&New Game\tF2");
    ...
    AppendMenu(hGame, MF_STRING, ID_GAME_MUTEMUSIC, L"Mute &Music");
    AppendMenu(hGame, MF_STRING, ID_GAME_MUTESOUNDFX, L"Mute &Sound FX");
    AppendMenu(hGame, MF_SEPARATOR, 0, NULL);
    AppendMenu(hGame, MF_STRING, ID_GAME_CHEATMODE, L"&Cheat Mode\tG");
    AppendMenu(hGame, MF_STRING, ID_GAME_DEBUGMODE, L"&Kibitzer Mode\tK");
    AppendMenu(hGame, MF_STRING, ID_GAME_TESTMODE, L"&Test Mode\tT");
    AppendMenu(hGame, MF_STRING, ID_GAME_ENDACTION, L"End &Action\tZ");
    ...
    AppendMenu(hTableColor, MF_STRING, ID_TABLECOLOR_GREEN, L"&Green");
    AppendMenu(hTableColor, MF_STRING, ID_TABLECOLOR_RED, L"&Red");
    AppendMenu(hTableColor, MF_STRING, ID_TABLECOLOR_VIOLET, L"&Violet");
    ...
    AppendMenu(hHelp, MF_STRING, ID_HELP_CONTROLS, L"&Keyboard Controls...");
    AppendMenu(hHelp, MF_STRING, ID_HELP_INSTRUCTIONS, L"&Instructions...");
    AppendMenu(hHelp, MF_STRING, ID_HELP_ABOUT, L"&About...\tF1");
    AppendMenu(hMenuBar, MF_POPUP, (UINT_PTR)hHelp, L"&Help");

    // [+] ADD THIS: The Giant Fake Item to consume the gray empty space
    //AppendMenu(hMenuBar, MF_OWNERDRAW | MF_DISABLED, ID_MENUBAR_EMPTY_SPACE, L"");

    // Set Owner Draw on all items so we can paint them Dark Blue
    SetOwnerDrawOnMenu(hMenuBar);

    // Attach the dynamically created menu to the window
    SetMenu(hwnd, hMenuBar);
}

// [+] ADD THIS FUNCTION: Safely calculates remaining menu space using pure Non-Client Screen Coords
/*void UpdateMenuBarSpace(HWND hwnd) {
    HMENU hMenu = GetMenu(hwnd);
    if (!hMenu) return;
    ...
    for (int i = itemCount - 1; i >= 0; --i) {
        if (GetMenuItemID(hMenu, i) != ID_MENUBAR_EMPTY_SPACE) {
            lastRealItemIndex = i;
            break;
        }
    }
    ...
    if (lastRealItemIndex >= 0) {
        RECT rcLastItem;
        if (GetMenuItemRect(hwnd, hMenu, lastRealItemIndex, &rcLastItem)) {
            MENUBARINFO mbi = { sizeof(mbi) };
            if (GetMenuBarInfo(hwnd, OBJID_MENU, 0, &mbi)) {
                g_EmptyMenuSpaceWidth = mbi.rcBar.right - rcLastItem.right;

                // MASSIVE BUFFER: Subtract 20 pixels to guarantee it NEVER line-wraps
                // on any 64-bit DPI scaling. Our manual DC paint will cover the rest.
                g_EmptyMenuSpaceWidth -= 20;

                if (g_EmptyMenuSpaceWidth < 1) g_EmptyMenuSpaceWidth = 1;

                ModifyMenu(hMenu, ID_MENUBAR_EMPTY_SPACE, MF_BYCOMMAND | MF_OWNERDRAW | MF_DISABLED, ID_MENUBAR_EMPTY_SPACE, L"");
                DrawMenuBar(hwnd);
            }
        }
    }
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {
    ...
    switch (msg) {
        // [+] ULTIMATE DEFENSE 1: Intercept native theme background draw
    // [+] THE DEFINITIVE UAH BACKGROUND HOOK
    // [+] BULLETPROOF UAH BACKGROUND DRAW
    case WM_UAHDRAWMENU:
    {
        UAHMENU* pUAH = reinterpret_cast<UAHMENU*>(lParam);
        if (pUAH && pUAH->hdc) {
            MENUBARINFO mbi = { sizeof(mbi) };
            if (GetMenuBarInfo(hwnd, OBJID_MENU, 0, &mbi)) {
                RECT rcWin;
                ...
                // Calculate the entire horizontal strip
                RECT rcBar = mbi.rcBar;
                OffsetRect(&rcBar, -rcWin.left, -rcWin.top);
                rcBar.right = rcWin.right - rcWin.left; // Blast all the way to the right edge

                // Paint the entire strip Dark Blue
                HBRUSH hBrush = CreateSolidBrush(RGB(23, 29, 37));
                FillRect(pUAH->hdc, &rcBar, hBrush);
                DeleteObject(hBrush);
            }
        }
        return 0; // Tell Windows the background is fully handled
    }

    // [+] BULLETPROOF UAH ITEM DRAW (For File, Game, Help text)
    case WM_UAHDRAWMENUITEM:
    {
        UAHDRAWMENUITEM* pUDMI = reinterpret_cast<UAHDRAWMENUITEM*>(lParam);
        if (pUDMI && pUDMI->dis.hDC) {
            // 1. Draw Hover/Normal Background for the specific item
            COLORREF bgColor = (pUDMI->dis.itemState & ODS_SELECTED) ? RGB(65, 105, 225) : RGB(23, 29, 37);
            HBRUSH hBrush = CreateSolidBrush(bgColor);
            FillRect(pUDMI->dis.hDC, &pUDMI->dis.rcItem, hBrush);
            DeleteObject(hBrush);

            // 2. Fetch the text (File, Game, Help, etc.)
            wchar_t menuText[256] = { 0 };
            GetMenuString(pUDMI->uahMenu.hmenu, pUDMI->umi.iPosition, menuText, 256, MF_BYPOSITION);

            // 3. Draw the text native-style
            SetBkMode(pUDMI->dis.hDC, TRANSPARENT);
            SetTextColor(pUDMI->dis.hDC, (pUDMI->dis.itemState & ODS_DISABLED) ? RGB(120, 120, 120) : RGB(255, 255, 255));

            NONCLIENTMETRICS ncm = { sizeof(NONCLIENTMETRICS) };
            SystemParametersInfo(SPI_GETNONCLIENTMETRICS, sizeof(NONCLIENTMETRICS), &ncm, 0);
            HFONT hFont = CreateFontIndirect(&ncm.lfMenuFont);
            HFONT hOldFont = (HFONT)SelectObject(pUDMI->dis.hDC, hFont);

            DrawText(pUDMI->dis.hDC, menuText, -1, &pUDMI->dis.rcItem, DT_CENTER | DT_SINGLELINE | DT_VCENTER | DT_HIDEPREFIX);

            SelectObject(pUDMI->dis.hDC, hOldFont);
            DeleteObject(hFont);
        }
        return 0; // Tell Windows the item is fully handled
    }
    // [+] ADD THIS CASE: Intercepts the native theme engine drawing the menu background
/*case WM_UAHDRAWMENU:
{
    UAHMENU* pUAH = reinterpret_cast<UAHMENU*>(lParam);
    if (pUAH && pUAH->hdc) {
        // Get the exact bounding box Windows wants painted for the menu bar
        RECT rcClip;
        GetClipBox(pUAH->hdc, &rcClip);

        // Fill it flawlessly with our target dark color
        HBRUSH hBrush = CreateSolidBrush(RGB(23, 29, 37));
        FillRect(pUAH->hdc, &rcClip, hBrush);
        DeleteObject(hBrush);
    }
    return 0; // Signal that we have fully handled the background paint
}*/
// [+] INSERT: The flawless Native Theme Engine Hooks
/*// [+] UPDATED: The Flawless Native Theme Engine Hook
case WM_UAHDRAWMENU:
{
    UAHMENU* pUAH = (UAHMENU*)lParam;
    RECT rcWindow;
    ...
    if (GetMenuBarInfo(hwnd, OBJID_MENU, 0, &mbi)) {
        ...
        // [+] THE GUARANTEED FIX: Extend the right boundary!
        // GetMenuBarInfo only covers the text. We force the rectangle
        // to span the entire width of the application window.
        rcBar.right = rcWindow.right - rcWindow.left;

        // Fill the entire horizontal row (including empty space) with Dark Blue
        HBRUSH hbrDarkBlue = CreateSolidBrush(RGB(23, 29, 37)); // Dark Blue
        FillRect(pUAH->hdc, &rcBar, hbrDarkBlue);
        DeleteObject(hbrDarkBlue);
    }
    return 0; // Tell Windows: "We painted the background, don't use gray."
}
...
case WM_UAHDRAWMENUITEM:
{
    ...
    if (pUDMI->dis.itemState & ODS_SELECTED) {
        hbrBkgnd = CreateSolidBrush(RGB(65, 105, 225)); // Royal Blue when hovered
    }
    else {
        hbrBkgnd = CreateSolidBrush(RGB(23, 29, 37)); // Dark Blue normal state
    }

    FillRect(pUDMI->dis.hDC, &pUDMI->dis.rcItem, hbrBkgnd);
    DeleteObject(hbrBkgnd);

    // Fetch the Menu Item Text
    wchar_t menuText[256] = { 0 };
    GetMenuString(pUDMI->uahMenu.hmenu, pUDMI->umi.iPosition, menuText, 256, MF_BYPOSITION);

    // Fetch standard Menu Font to keep it looking native
    NONCLIENTMETRICS ncm = { sizeof(NONCLIENTMETRICS) };
    SystemParametersInfo(SPI_GETNONCLIENTMETRICS, sizeof(NONCLIENTMETRICS), &ncm, 0);
    HFONT hFont = CreateFontIndirect(&ncm.lfMenuFont);
    HFONT hOldFont = (HFONT)SelectObject(pUDMI->dis.hDC, hFont);
    ...
    // Draw the Text
    SetBkMode(pUDMI->dis.hDC, TRANSPARENT);
    SetTextColor(pUDMI->dis.hDC, RGB(255, 255, 255)); // White Text

    // Use DT_HIDEPREFIX so the '&' accelerators are handled natively
    DrawText(pUDMI->dis.hDC, menuText, -1, &pUDMI->dis.rcItem,
    ...
    return 0; // Tell Windows: "We painted the item, don't use gray."
}*/
// [+] END UAH INSERTION
    // [+] PERMANENT FIX: Force Entire MenuStrip Row to Stay Black
/*case WM_NCPAINT: {
    // 1. Let Windows draw the titlebar, borders, shadow — the full NC area
    DefWindowProc(hwnd, WM_NCPAINT, wParam, lParam);

    // 2. Immediately repaint our dark menu bar over the gray one Windows just drew
    DrawDarkMenuBar(hwnd);

    // 3. Schedule a deferred repaint to catch any async
    //    gray repainting Windows does AFTER this message returns
    //    (this is the key fix for the post-dialog/post-dropdown gray flash)
    KillTimer(hwnd, ID_DARK_MENUBAR_TIMER);
    SetTimer(hwnd, ID_DARK_MENUBAR_TIMER, 1, NULL);
    return 0;
}

               // [+] Handle titlebar active/inactive transitions
case WM_NCACTIVATE: {
    LRESULT result = DefWindowProc(hwnd, msg, wParam, lParam);
    DrawDarkMenuBar(hwnd);

    // Deferred repaint for safety
    KillTimer(hwnd, ID_DARK_MENUBAR_TIMER);
    SetTimer(hwnd, ID_DARK_MENUBAR_TIMER, 1, NULL);
    return result;
}

                  // [+] Intercept UAH menu drawing — Windows uses these to paint menu bar items
case WM_UAHDRAWMENU: {
    DrawDarkMenuBar(hwnd);
    return 0;
}

                   // [+] KEY FIX: Intercept individual menu bar item draw requests
                   //     This is what Windows sends to repaint each top-level menu item
                   //     (File, Game, Help) with the default gray theme after dropdown closes
case WM_UAHDRAWMENUITEM: {
    // Block default drawing entirely — our DrawDarkMenuBar handles everything
    DrawDarkMenuBar(hwnd);
    return 0;
}

                       // [+] Block UAH menu popup NC paint (dropdown border theming)
case WM_UAHNCPAINTMENUPOPUP: {
    return 0;
}

                           // [+] KEY FIX: When a menu dropdown closes (after clicking an item or pressing Escape),
                           //     Windows repaints the menu bar gray. This catches that moment.
case WM_EXITMENULOOP: {
    // Let default processing happen
    DefWindowProc(hwnd, msg, wParam, lParam);

    // Immediately fix it
    DrawDarkMenuBar(hwnd);

    // And schedule a deferred fix for the async repaint that follows
    KillTimer(hwnd, ID_DARK_MENUBAR_TIMER);
    SetTimer(hwnd, ID_DARK_MENUBAR_TIMER, 1, NULL);
    return 0;
}

                    // [+] Also catch menu loop entry for completeness
case WM_ENTERMENULOOP: {
    ...
    return 0;
                     ...
                     // [+] Catch when menus are being selected/highlighted — Windows repaints the bar
case WM_MENUSELECT: {
    DefWindowProc(hwnd, msg, wParam, lParam);
    DrawDarkMenuBar(hwnd);
    return 0;
                  ...
                  // [+] Handle window activation changes (alt-tab, dialog close, etc.)
case WM_ACTIVATE: {
    LRESULT result = DefWindowProc(hwnd, msg, wParam, lParam);
    DrawDarkMenuBar(hwnd);
    KillTimer(hwnd, ID_DARK_MENUBAR_TIMER);
    ...
    /*case WM_NCPAINT:
    case WM_UAHDRAWMENU: {
        // 1. Let Windows draw the default non-client area (borders/titlebar)
        if (msg == WM_NCPAINT) DefWindowProc(hwnd, WM_NCPAINT, wParam, lParam);
        ...
        if (GetMenuBarInfo(hwnd, OBJID_MENU, 0, &mbi)) {
            ...
            for (int i = 0; i < itemCount; i++) {
                ...
                if (GetMenuItemRect(hwnd, hMenu, i, &rcItem)) {
                    ...
                    wchar_t menuText[256];
                    if (GetMenuString(hMenu, i, menuText, 256, MF_BYPOSITION)) {
                        // Center text within the item's rectangle
                        DrawText(hdc, menuText, -1, &rcItem, DT_CENTER | DT_VCENTER | DT_SINGLELINE);
                    }
                }
            }
    ...
    case WM_ERASEBKGND: {
        ...
        /*// Paint the menu bar area black
        RECT rcMenu;
        GetMenuBarInfo(hwnd, OBJID_MENU, 0, (MENUBARINFO*)&rcMenu);

        HBRUSH hbrBlack = CreateSolidBrush(RGB(0, 0, 0));
        FillRect(hdc, &rcMenu, hbrBlack);
        DeleteObject(hbrBlack);*/
    ...
    case WM_MEASUREITEM: {
        ...
        if (mis->CtlType == ODT_MENU) {
            // [+] ADD THIS BLOCK: Make the fake item massive to cover the horizontal space
            // [+] MODIFY THIS: Use the exact calculated width so it never wraps
            /*if (mis->itemID == ID_MENUBAR_EMPTY_SPACE) {
                mis->itemWidth = g_EmptyMenuSpaceWidth;
            ...
            wchar_t text[256] = L"";
            HMENU hMenu = GetMenu(hwnd);
            GetMenuString(hMenu, mis->itemID, text, 255, MF_BYCOMMAND);
            ...
            mis->itemHeight = std::max<LONG>(20, sz.cy + 6);
    ...
    case WM_DRAWITEM: {
        ...
        if (dis->CtlType == ODT_MENU) {
            // [+] ADD THIS BLOCK: Paint the giant fake item solid dark to kill the gray glitch
            /*if (dis->itemID == ID_MENUBAR_EMPTY_SPACE) {
                HBRUSH hBg = CreateSolidBrush(RGB(23, 29, 37));
            ...
            if (miiSep.fType & MFT_SEPARATOR) {
                // [+] MODIFIED: Dark Blue Separator Background
                HBRUSH hbrBg = CreateSolidBrush(RGB(23, 29, 37));
                FillRect(dis->hDC, &dis->rcItem, hbrBg);
                DeleteObject(hbrBg);

                int y = (dis->rcItem.top + dis->rcItem.bottom) / 2;
                HPEN hPen = CreatePen(PS_SOLID, 1, RGB(100, 100, 180)); // Lighter blue line
                HPEN hOld = (HPEN)SelectObject(dis->hDC, hPen);

                MoveToEx(dis->hDC, dis->rcItem.left + 4, y, NULL);
                LineTo(dis->hDC, dis->rcItem.right - 4, y);
            ...
            // [+] MODIFIED: Dark Blue normal state, Royal Blue hover state
            COLORREF bg = (dis->itemState & ODS_SELECTED) ? RGB(65, 105, 225) : RGB(23, 29, 37);
            HBRUSH hBrush = CreateSolidBrush(bg);
            FillRect(dis->hDC, &dis->rcItem, hBrush);
            DeleteObject(hBrush);

            COLORREF textColor = (dis->itemState & ODS_DISABLED) ? RGB(120, 120, 120) : RGB(255, 255, 255);
            SetTextColor(dis->hDC, textColor);
            SetBkMode(dis->hDC, TRANSPARENT);

            // [+] ADD THIS: Create and select Segoe UI font for the menu items
            ...
            wchar_t buf[256] = L"";
            MENUITEMINFO miiText = { sizeof(miiText) };
            miiText.fMask = MIIM_STRING;
            miiText.dwTypeData = buf;
            miiText.cch = 255;
            GetMenuItemInfo((HMENU)dis->hwndItem, dis->itemID, FALSE, &miiText);

            RECT rcText = dis->rcItem;
            rcText.left += 24;

            wchar_t* tabPos = wcschr(buf, L'\t');
            if (tabPos) {
                *tabPos = L'\0';
                const wchar_t* shortcut = tabPos + 1;

                DrawText(dis->hDC, buf, -1, &rcText, DT_SINGLELINE | DT_VCENTER | DT_LEFT);
                RECT rcAccel = dis->rcItem;
                rcAccel.right -= 6;
                DrawText(dis->hDC, shortcut, -1, &rcAccel, DT_SINGLELINE | DT_VCENTER | DT_RIGHT);
            }
            else {
                DrawText(dis->hDC, buf, -1, &rcText, DT_SINGLELINE | DT_VCENTER | DT_LEFT);
            }
            ...
            if (dis->itemState & ODS_CHECKED) {
                ...
                rcCheck.top += 2; rcCheck.bottom -= 2;
                DrawFrameControl(dis->hDC, &rcCheck, DFC_MENU, DFCS_MENUCHECK);
            }

            // [+] ADD THIS: Clean up the font objects
    ...
    case WM_COMMAND:
    {
        ...
        case ID_GAME_MUTEMUSIC:
            ...
            if (isMusicPlaying.load()) {
                ...
                StartMidi(hwnd, IDR_MID_BGMUSIC);
        ...
        case ID_TABLECOLOR_CUSTOM:
        {
            if (wmId == ID_TABLECOLOR_CUSTOM) {
                ...
                // Prepopulate with current color
                cc.rgbResult = RGB(TABLE_COLOR.r * 255.0f, TABLE_COLOR.g * 255.0f, TABLE_COLOR.b * 255.0f);
                cc.Flags = CC_FULLOPEN | CC_RGBINIT;

                if (ChooseColor(&cc)) {
                    selectedTableColor = CUSTOM;
                    g_customTableColor = D2D1::ColorF(GetRValue(cc.rgbResult) / 255.0f, GetGValue(cc.rgbResult) / 255.0f, GetBValue(cc.rgbResult) / 255.0f);
                    TABLE_COLOR = g_customTableColor;
    ...
    case WM_CREATE:
    {
        // [+] ADD THIS: Forces the TitleBar to be Dark Mode (Windows 10/11)
        BOOL useDarkMode = TRUE;
        DwmSetWindowAttribute(hwnd, 20, &useDarkMode, sizeof(useDarkMode)); // 20 = DWMWA_USE_IMMERSIVE_DARK_MODE

        CreateAppMenu(hwnd);
        // [+] 1. Build the dynamic menu
        //CreateAppMenu(hwnd);
        // [+] ADD THIS: Force OS-Level Dark Mode for the Non-Client Area
            // This instructs the Windows 10/11 Compositor to render the frame/menu dark natively.
        //BOOL useDarkMode = TRUE;

        // For Windows 10 versions older than 20H1
        //DwmSetWindowAttribute(hwnd, 20 /* DWMWA_USE_IMMERSIVE_DARK_MODE_BEFORE_20H1 */, &useDarkMode, sizeof(useDarkMode));
        // For Windows 10 20H1+ and Windows 11
        //DwmSetWindowAttribute(hwnd, 19 /* DWMWA_USE_IMMERSIVE_DARK_MODE */, &useDarkMode, sizeof(useDarkMode));
        // [+] THE WINDOWBLINDS KILLER: Disable UxTheme for this window
            // This explicitly blocks WindowBlinds and the Windows Theme Engine 
            // from painting over our custom UAH menu bar, while keeping the DWM TitleBar intact.
        //SetWindowTheme(hwnd, L" ", L" ");

        // [+] 2. Apply Global Dark Blue Background to the Menu Strip Empty Area
        HMENU hMenu = GetMenu(hwnd);
        if (hMenu) {
        ...
        // [+] ADD THIS: Queue the menu update for immediately AFTER the window becomes visible
        //PostMessage(hwnd, WM_UPDATE_MENU_STRIP, 0, 0);
        return 0;
    }
    case WM_PAINT:
        OnPaint();
    ...
    case WM_SIZE: {
        ...
        // [+] ADD THIS BLOCK: Dynamic Menu Bar Filler
        HMENU hMenu = GetMenu(hwnd);
        if (hMenu && wParam != SIZE_MINIMIZED) {
            RECT rcHelp;
            // Get the screen rectangle of the 3rd menu item (Index 2 = "Help")
            if (GetMenuItemRect(hwnd, hMenu, 2, &rcHelp)) {
                POINT pt = { 0, 0 };
                ClientToScreen(hwnd, &pt);

                // Calculate width taken by File + Game + Help
                int menuItemsWidth = rcHelp.right - pt.x;

                RECT rcClient;
                GetClientRect(hwnd, &rcClient);

                // Calculate exact remaining space (-2px buffer to strictly prevent wrapping)
                g_EmptyMenuSpaceWidth = rcClient.right - menuItemsWidth - 2;
                if (g_EmptyMenuSpaceWidth < 1) g_EmptyMenuSpaceWidth = 1;

                // Force the OS to re-measure our fake item with the new exact width
                ModifyMenu(hMenu, ID_MENUBAR_EMPTY_SPACE, MF_BYCOMMAND | MF_OWNERDRAW | MF_DISABLED, ID_MENUBAR_EMPTY_SPACE, L"");
                DrawMenuBar(hwnd);
            }
        }
        ...
        return 0;*/
                ...
                // [+] ADD THIS CASE: Executes exactly when the window hits the screen
    /*case WM_UPDATE_MENU_STRIP:
    {
        UpdateMenuBarSpace(hwnd);
        return 0;
    }*/
    case WM_KEYDOWN:
    {
        ...
        // [+] NEW: In network mode, ignore gameplay keys if not local player's turn
        // (F1 About, F2 New Game, M mute still allowed)
        if (gameMode == NETWORK_PLAY && !IsLocalPlayerTurn() &&
            wParam != VK_F1 && wParam != VK_F2 && wParam != 'M' && wParam != 'm') {
            return 0;
        }
        ...
        if (wParam == 'M' || wParam == 'm') {
            ...
            if (isMusicPlaying) {
                ...
                // If music is stopped, start it again from embedded resource.
                StartMidi(hwndMain, IDR_MID_BGMUSIC);
            }
...
// Accept both uppercase and lowercase 'K'
        ...
        if (wParam == 'H' || wParam == 'h') {
            bool isHumanTurn = (!g_isDemoMode && ((currentPlayer == 1) || (currentPlayer == 2 && !isPlayer2AI)));
            bool isValidState = (currentGameState == PLAYER1_TURN || currentGameState == PLAYER2_TURN || currentGameState == AIMING || currentGameState == BREAKING);

            if (isHumanTurn && isValidState && !AreBallsMoving()) {
                if (!g_hintActive || g_cachedHints.empty()) {
                    if (currentGameType == GameType::EIGHT_BALL_MODE) g_cachedHints = AIFindAllShots_8Ball();
                    else if (currentGameType == GameType::NINE_BALL) g_cachedHints = AIFindAllShots_NineBall();
                    else g_cachedHints = AIFindAllShots_StraightPool();

                    if (!g_cachedHints.empty()) {
                        g_hintActive = true;
                        g_hintIndex = 0;
                    }
                }
                else {
                    g_hintIndex = (g_hintIndex + 1) % g_cachedHints.size();
                }

                if (g_hintActive && !g_cachedHints.empty()) {
                    AIShotInfo hint = g_cachedHints[g_hintIndex];
                    cueAngle = hint.angle;
                    shotPower = hint.power;
                    cueSpinX = hint.spinX;
                    cueSpinY = hint.spinY;
                    g_hintTargetBallId = hint.targetBall ? hint.targetBall->id : -1;

                    keyboardAimingActive = true;
                    isAiming = true;
                    if (currentGameState != BREAKING) currentGameState = AIMING;

                    // Automatically call pocket for UI feedback in 8-Ball
                    if (currentGameType == GameType::EIGHT_BALL_MODE && hint.involves8Ball) {
                        if (currentPlayer == 1) calledPocketP1 = hint.pocketIndex;
                        else calledPocketP2 = hint.pocketIndex;
                    }
                    InvalidateRect(hwnd, NULL, FALSE);
                }
            }
        ...
        if (canPlayerControl) {
            ...
            case VK_LEFT:
                if (currentGameState != SHOT_IN_PROGRESS && currentGameState != AI_THINKING) {
                    ...
                    isDraggingStick = false;
                    keyboardAimingActive = true;
                    g_hintActive = false; g_cachedHints.clear(); // Break hint
                }
                break;
            case VK_RIGHT:
                if (currentGameState != SHOT_IN_PROGRESS && currentGameState != AI_THINKING) {
            ...
            case VK_ESCAPE:
                ...
                if ((currentGameState == AIMING || currentGameState == BREAKING) || shotPower > 0 || g_hintActive)
                {
                    ...
                    // [+] NEW: Reset Hint State on Escape
                    g_hintActive = false;
                    g_cachedHints.clear();
                    g_hintTargetBallId = -1;
                    ...
                    InvalidateRect(hwnd, NULL, FALSE); // Force immediate UI refresh to remove hint glow
                }
                break;
                /*case VK_ESCAPE:
    ...
    case WM_MOUSEMOVE: {
        ...
        // --- FOOLPROOF FIX: Prevent Mouse Overriding Keyboard Aim ---
        ...
        {
            // Only update angle from mouse if the user is actively dragging with the mouse
            // AND we are NOT currently in keyboard-aiming mode.
            // If the user moves the mouse significantly, we can disable keyboard mode.

            float dx = (float)ptMouse.x - cueBall->x;
            float dy = (float)ptMouse.y - cueBall->y;

            // Calculate potential new angle
            float newMouseAngle = (dx != 0 || dy != 0) ? atan2f(dy, dx) : cueAngle;

            // Threshold to detect intentional mouse movement vs jitter
            // If the mouse angle is significantly different from current cueAngle, assume user switched to mouse.
            float angleDiff = fabsf(newMouseAngle - cueAngle);
            while (angleDiff > PI) angleDiff -= 2 * PI;
            while (angleDiff < -PI) angleDiff += 2 * PI;

            // If the user deliberately moves the mouse (large delta) OR clicks-and-drags (isAiming/isDraggingStick),
            // we respect the mouse. However, if they are just pressing keys, we ignore the mouse position.

            if (!keyboardAimingActive) {
                g_hintActive = false; // [+] FIX: Only break hint if mouse actively taking over

                // Standard mouse behavior
                cueAngle = newMouseAngle;
                ...
                // We only revert to mouse control if the user CLICKS again (handled in LBUTTONDOWN)
                // or if they drag the stick significantly (handled by isDraggingStick logic usually).
                // For now, do NOTHING here. This prevents the "snap back".
            }
        }
        // --- End Fix ---
        /*else if ((isAiming || isDraggingStick) &&
        ...
        // [+] FIX: Synchronized allowed game states to include PRE_BREAK_PLACEMENT and BALL_IN_HAND 
        // This ensures the Spin Indicator can be dragged smoothly at the start of a game or after a scratch.
                ...
                (!isPlayer2AI && currentPlayer == 2 && (currentGameState == PLAYER2_TURN || currentGameState == AIMING || currentGameState == BREAKING || currentGameState == BALL_IN_HAND_P2 || currentGameState == PRE_BREAK_PLACEMENT))))
        {
            ...
            // decrement it because isSettingEnglish is true.
            g_spinPopupLingerCounter = 9999;
            InvalidateRect(hwnd, NULL, FALSE); // Force instant redraw for real-time drag feedback
        }
        else {
        }
        return 0;
    ...
    case WM_LBUTTONDOWN: {
        ...
        // [+] NEW: In network mode, block all input if it's not the local player's turn
        if (gameMode == NETWORK_PLAY && !IsLocalPlayerTurn()) return 0;
        ...
        if (canAim) {
            ...
            if (clickedStick || clickedCueArea) {
                ...
                g_hintActive = false; // Cancel hint on explicit stick click
                aimStartPoint = D2D1::Point2F((float)ptMouse.x, (float)ptMouse.y);

                //added this newly
                // --- FIX: Reset keyboard flag when mouse aiming starts ---
                keyboardAimingActive = false;
                // ---------------------------------------------------------
    ...
    case WM_LBUTTONUP: {
                ...
                (!isPlayer2AI && currentPlayer == 2 && (currentGameState == AIMING || currentGameState == BREAKING))))
        {
            ...
            if (shotPower > 0.15f) { // Only shoot if there's power
                if (currentGameState != AI_THINKING) { // Don't allow human shot during AI thinking
                    ...
                    // [+] NEW: In 8-Ball network mode, sync the called pocket
                    if (gameMode == NETWORK_PLAY &&
                        currentGameType == GameType::EIGHT_BALL_MODE) {
                        int called = (currentPlayer == 1) ?
                            calledPocketP1 : calledPocketP2;
                        NetSendPocketCall(called);
                    }
                }
            }
            ...
            if (isPlayerAllowed && cueBall) {
                ...
                if (IsValidCueBallPosition(cueBall->x, cueBall->y, behindHeadstring)) {
                    ...
                    if (currentGameState == PRE_BREAK_PLACEMENT) {
                        ...
                        // [+] NEW: Sync cue ball position to opponent
                        if (gameMode == NETWORK_PLAY)
                            NetSendCueBallPos(cueBall->x, cueBall->y);
                    }
        ...
        if (isSettingEnglish) {
            isSettingEnglish = false;
            // [+] NEW: Release mouse capture and set the linger counter
            // to ~1.5 seconds (90 frames @ 60fps) so the popup stays
            // visible briefly after release. This makes a quick single
            // click also visible. After ~1.5s with no further
            // interaction the counter hits 0 and the popup disappears.
            ReleaseCapture();
            g_spinPopupLingerCounter = 90;
            InvalidateRect(hwnd, NULL, FALSE);
        }
...
// mouse-move updates the spin again.
    ...
    case WM_DESTROY:
        ...
        NetDisconnect();  // [+] NEW: Gracefully close network connection
        StopMidi();
        SaveSettings();
        PostQuitMessage(0);
        return 0;
...
// [+] NEW: Robust D2D1 Image Loader for Sprites
HRESULT LoadD2DBitmapFromResource(ID2D1RenderTarget* pRT, int resourceId, PCWSTR resourceType, ID2D1Bitmap** ppBitmap) {
    if (!g_pWICFactory_masks) return E_FAIL;

    HRSRC hRes = FindResource(GetModuleHandle(NULL), MAKEINTRESOURCE(resourceId), resourceType);
    ...
    if (SUCCEEDED(hr)) {
        ...
        hr = g_pWICFactory_masks->CreateDecoderFromStream(pStream, NULL, WICDecodeMetadataCacheOnLoad, &pDecoder);
        if (SUCCEEDED(hr)) {
            IWICBitmapFrameDecode* pSource = NULL;
            hr = pDecoder->GetFrame(0, &pSource);
            if (SUCCEEDED(hr)) {
                IWICFormatConverter* pConverter = NULL;
                hr = g_pWICFactory_masks->CreateFormatConverter(&pConverter);
                if (SUCCEEDED(hr)) {
...
// The canvas for each mask only needs to be slightly larger than the pocket opening plus the ball.
    ...
    // [+] FOOLPROOF FIX: Initialize the D2D Avatars from the PNG resources
    for (int i = 0; i < 24; ++i) {
        if (!g_pSpriteBitmaps[i]) {
            LoadD2DBitmapFromResource(pRenderTarget, IDB_SPRITE_01 + i, L"PNG", &g_pSpriteBitmaps[i]);
        }
    }
...
void DiscardDeviceResources() {
    // [+] NEW: Free the Sprite memory arrays to prevent memory leaks
    for (int i = 0; i < 24; ++i) {
        SafeRelease(&g_pSpriteBitmaps[i]);
    }
...
void InitGame() {
    ...
    if (gameMode == NETWORK_PLAY) {
        // [+] NEW: In network play, the Host (Player 1) always breaks first
        currentPlayer = (g_netRole == NR_HOST) ? 1 : 2;
        currentGameState = PRE_BREAK_PLACEMENT;
        aiTurnPending = false;
    }
    else if (isPlayer2AI && !g_isPracticeMode) { // AI breaks only if not practice mode
    ...
    gameOverMessage = L"";
    g_spinPopupLingerCounter = 0; // [+] NEW: Defensive hide of the spin popup on new game
    ...
    // [+] FIX: Force Demo Mode to immediately flag AI Turn
    if (g_isDemoMode) {
        aiTurnPending = true;
    }
...
// --------------------------------------------------------------------------------
void GameUpdate() {
    // [+] NEW: Process incoming network packets every frame
    if (g_netConnected) {
        g_netFrameCounter++;
        NetProcessPackets();

        // Send heartbeat every ~0.5 seconds (30 frames at 60 FPS)
        if (g_netAuthDone && ++g_netHeartbeatCounter >= 30) {
            g_netHeartbeatCounter = 0;
            NetSendHeartbeat();
        }
    }
    ...
    bool isAITurn = g_isDemoMode || (isPlayer2AI && currentPlayer == 2);
    if (isAITurn && !AreBallsMoving()) {

        // [+] DEMO MODE AUTO-RESUME FAILSAFE
        // This guarantees that if the AI ever gets stuck in a "waiting for human" state during Demo mode, it instantly takes over.
        if (g_isDemoMode && !aiTurnPending && !aiIsDisplayingAim && currentGameState != SHOT_IN_PROGRESS && currentGameState != AI_THINKING && currentGameState != GAME_OVER) {
            if (currentGameState == PLAYER1_TURN || currentGameState == PLAYER2_TURN ||
                currentGameState == BALL_IN_HAND_P1 || currentGameState == BALL_IN_HAND_P2 ||
                currentGameState == PRE_BREAK_PLACEMENT) {
                aiTurnPending = true;
        ...
        if ((currentPlayer == 2 && currentGameState == CHOOSING_POCKET_P2) || (currentPlayer == 1 && currentGameState == CHOOSING_POCKET_P1)) {
        ...
        if ((currentGameState == CHOOSING_POCKET_P2 || currentGameState == CHOOSING_POCKET_P1) && aiTurnPending) {
        ...
        if (aiTurnPending) {
            // [+] FIX: Added P1 handlers to support Demo Mode flawlessly
            if (currentGameState == BALL_IN_HAND_P2 || currentGameState == BALL_IN_HAND_P1) {
                AIPlaceCueBall();
                currentGameState = AI_THINKING;
                aiTurnPending = false;
                AIMakeDecision();
            ...
            else if (currentGameState == PLAYER2_TURN || currentGameState == PLAYER1_TURN || currentGameState == BREAKING) {
                currentGameState = AI_THINKING;
                aiTurnPending = false;
                AIMakeDecision();
            }
...
// or deviating from the AI/Chevron continuous geometry predictions.
                ...
                // Note: we separate them along the TRUE continuous normal for perfect physical accuracy
...
void ApplyShot(float power, float angle, float spinX, float spinY) {
    ...
    if (cueBall) {
        ...
        // [+] NEW: In network mode, send shot parameters to the remote opponent
        // BEFORE applying locally. This ensures both sides shoot simultaneously.
        if (gameMode == NETWORK_PLAY && IsLocalPlayerTurn()) {
            NetSendShot(angle, power, spinX, spinY);
        }
        ...
        // 1. Initial Forward Trajectory
        cueBall->vx = cosf(angle) * power;
        cueBall->vy = sinf(angle) * power;

        // Apply English (Spin) - Simplified effect (Unchanged)
        // 2. [FIXED] Apply English (Spin) Physics correctly
        // Top/Draw (spinY) alters forward momentum slightly
        cueBall->vx += cosf(angle) * spinY * 0.5f;
        cueBall->vy += sinf(angle) * spinY * 0.5f;

        // Left/Right English (spinX) causes "squirt" (perpendicular lateral deflection)
        cueBall->vx += sinf(angle) * spinX * 0.5f;
        cueBall->vy -= cosf(angle) * spinX * 0.5f;

        // [+] Clear Hints on Shot Execution
        g_hintActive = false;
        g_cachedHints.clear();
...
// correctly display pocketed Solids (Left) and Stripes (Right) in Practice Mode.
    ...
    pocketedThisTurn.clear(); // Clear for the next turn
        ...
        // [+] NEW: After shot results are finalized, send authoritative
    // state sync to the remote opponent so both sides are in lockstep.
    if (gameMode == NETWORK_PLAY && g_netConnected && g_netAuthDone) {
        NetSendStateSync();
    }
...
void FinalizeGame(int winner, int loser, const std::wstring& reason) {
    ...
    if (!g_soundEffectsMuted) {
        ...
        if (gameMode == HUMAN_VS_AI) {
            if (winner == 1) {
                ...
                PlayGameSound(IDR_WAV_WON);
            }
            else {
                // AI Won (Human Lost) -> Play Loss Sound
                PlayGameSound(IDR_WAV_LOSS);
            }
        }
        else {
    ...
    // 2. Update statistics based on the game mode and outcome (Bypass for Demo Mode)
    if (!g_isDemoMode) {
        if (gameMode == HUMAN_VS_AI) {
...
// [+] FIX: CPU Opponent Foul Bug — returns the distance from (x,y) to the nearest playable cushion line.
float GetDistanceToNearestRail(float x, float y) {
    float distLeft = x - (TABLE_LEFT + INNER_RIM_THICKNESS);
    float distRight = (TABLE_RIGHT - INNER_RIM_THICKNESS) - x;
    float distTop = y - (TABLE_TOP + INNER_RIM_THICKNESS);
    float distBottom = (TABLE_BOTTOM - INNER_RIM_THICKNESS) - y;
    float nearest = std::min(std::min(distLeft, distRight), std::min(distTop, distBottom));
    return std::max(nearest, 0.0f);
}
...
void CheckAndTransitionToPocketChoice(int playerID) {
    ...
    if (needsToCall) {
        ...
        if ((playerID == 2 && isPlayer2AI) || g_isDemoMode) {
            aiTurnPending = true;
        }
    }
}
    ...
    for (int i = 0; i <= gridSize; ++i) {
        for (int j = 0; j <= gridSize; ++j) {
            ...
            if (currentGameType == GameType::EIGHT_BALL_MODE) {
                ...
                shotFromHere = AIFindBestShot_NineBall();
            }
            else { // Assumes STRAIGHT_POOL
...
void AIFindBankShots(Ball* targetBall, std::vector<AIShotInfo>& possibleShots) {
    ...
    for (int p = 0; p < 6; ++p) { // For each pocket
        ...
        D2D1_POINT_2F aimTargetPos = ComputeAdaptiveAimTarget(p, targetBall, cue);

        for (int r = 0; r < 12; ++r) { // For each of the 12 rail segments
            D2D1_POINT_2F p1 = segments[r][0];
            D2D1_POINT_2F p2 = segments[r][1];

            // Reflect aim target across the line of the rail segment
            ...
            // Vector from p1 to aim target
            float px = aimTargetPos.x - p1.x;
            float py = aimTargetPos.y - p1.y;

            // Projection of (px, py) onto (dx, dy)
            float t = (px * dx + py * dy) / lenSq;

            // Closest point on line to aim target
            float cx = p1.x + t * dx;
            float cy = p1.y + t * dy;

            // True reflection across arbitrary segment
            virtualPocket = { 2.0f * cx - aimTargetPos.x, 2.0f * cy - aimTargetPos.y };
            ...
            if (IsPathClear({ cue->x, cue->y }, ghostBallForBank, cue->id, targetBall->id) &&
                ...
                IsPathClear(bouncePoint, aimTargetPos, targetBall->id, -1))
            {
...
D2D1_POINT_2F GetPocketAimTarget(int p) {
    ...
    switch (p) {
    case 0: return { pos.x + inset, pos.y + inset };
    case 1: return { pos.x, pos.y + inset };
    case 2: return { pos.x - inset, pos.y + inset };
    case 3: return { pos.x + inset, pos.y - inset };
    case 4: return { pos.x, pos.y - inset };
    case 5: return { pos.x - inset, pos.y - inset };
    }
    return pos;
}
//start 8ball fix
...
// ============================================================================
// Smoothly blends between the "sweet-spot" inset and the raw pocket center.
D2D1_POINT_2F ComputeAdaptiveAimTarget(int pocketIndex, Ball* targetBall, Ball* cueBall) {
    D2D1_POINT_2F pocketPos = pocketPositions[pocketIndex];
    D2D1_POINT_2F sweetSpotPos = GetPocketAimTarget(pocketIndex);

    // Fallback: if either ball pointer is null, return original sweet-spot
    if (!targetBall || !cueBall) return sweetSpotPos;

    // ── PASS 1 ── Determine the approximate cut angle 
    float dx_tp = pocketPos.x - targetBall->x;
    float dy_tp = pocketPos.y - targetBall->y;
    float dist_tp = sqrtf(dx_tp * dx_tp + dy_tp * dy_tp);
    if (dist_tp < 1e-4f) return pocketPos;

    // Preliminary ghost-ball using raw pocket centre
    D2D1_POINT_2F prelimGhost = {
        targetBall->x - (dx_tp / dist_tp) * (BALL_RADIUS * 2.0f),
        targetBall->y - (dy_tp / dist_tp) * (BALL_RADIUS * 2.0f)
    };

    // Approach vectors
    float vCueX = prelimGhost.x - cueBall->x;
    float vCueY = prelimGhost.y - cueBall->y;
    float vObjX = dx_tp;
    float vObjY = dy_tp;
    float magCue = sqrtf(vCueX * vCueX + vCueY * vCueY);
    float magObj = dist_tp;

    if (magCue < 1e-4f || magObj < 1e-4f) return sweetSpotPos;

    // Cosine of the cut angle
    float dot = (vCueX * vObjX + vCueY * vObjY) / (magCue * magObj);
    float cutAngle = acosf(std::clamp(dot, -1.0f, 1.0f));

    // ── PASS 2 ── Blend aim target based on cut angle 
    const float STRAIGHT_THRESHOLD = PI * 0.08f;  // ≈ 14.4°
    const float CUT_THRESHOLD = PI * 0.25f;  // ≈ 45°
    float blend;

    if (cutAngle <= STRAIGHT_THRESHOLD) {
        blend = 1.0f; // fully sweet-spot
    }
    else if (cutAngle >= CUT_THRESHOLD) {
        blend = 0.0f; // fully pocket centre
    }
    else {
        // Hermite smoothstep for a seamless C1-continuous transition
        float t = (cutAngle - STRAIGHT_THRESHOLD) / (CUT_THRESHOLD - STRAIGHT_THRESHOLD);
        blend = 1.0f - (t * t * (3.0f - 2.0f * t));
    }

    // Linear interpolation between pocket centre and sweet-spot
    return {
        pocketPos.x + (sweetSpotPos.x - pocketPos.x) * blend,
        pocketPos.y + (sweetSpotPos.y - pocketPos.y) * blend
    };
}
...
float CalculateShotPower(float cueToGhostDist, float targetToPocketDist, bool isBank) {
    float target_leg_power = 2.8f * sqrtf(targetToPocketDist);
    float cue_leg_power = 2.2f * sqrtf(cueToGhostDist);
    ...
    float max_possible_units = 2.5f * sqrtf(TABLE_WIDTH) + 2.0f * sqrtf(TABLE_HEIGHT);
    float power_ratio = std::clamp(total_power_units / (max_possible_units * 0.85f), 0.0f, 1.0f);

    // FIX: Ensure AI uses professional maximum strength for clear shots. 
    // 8.5f minimum guarantees the ball won't just be "nudged".
    const float MIN_POWER_OUTPUT = 8.5f;
    float finalPower = MIN_POWER_OUTPUT + power_ratio * (MAX_SHOT_POWER - MIN_POWER_OUTPUT);

    return std::clamp(finalPower, 8.5f, MAX_SHOT_POWER);
}
...
AIShotInfo EvaluateShot_8Ball(Ball* targetBall, int pocketIndex, bool isBank, bool relaxedPath /*= false*/) {
    ...
    // Relaxed path check uses a smaller clearance tube to find shots the strict pass missed.
    // Penalised in score so strict shots are always preferred.
    const float pathMult = relaxedPath ? 1.72f : 2.02f;

    D2D1_POINT_2F aimTargetPos = ComputeAdaptiveAimTarget(pocketIndex, targetBall, cue);
    D2D1_POINT_2F targetPos = { targetBall->x, targetBall->y };
    D2D1_POINT_2F ghostBallPos = CalculateGhostBallPos(targetBall, aimTargetPos);
    shotInfo.ghostBallPos = ghostBallPos;

    if (!IsPathClear({ cue->x, cue->y }, ghostBallPos, cue->id, targetBall->id, pathMult)) return shotInfo;
    if (!isBank && !IsPathClear(targetPos, aimTargetPos, targetBall->id, -1, pathMult))     return shotInfo;

    D2D1_POINT_2F vCue = { ghostBallPos.x - cue->x, ghostBallPos.y - cue->y };
    D2D1_POINT_2F vObj = { aimTargetPos.x - targetBall->x, aimTargetPos.y - targetBall->y };
    ...
    // Difficulty-scaled cut-angle limits: EASY ≈72°, MEDIUM ≈79°, HARD ≈85°
    float maxCutAngle = (aiDifficulty == EASY) ? (PI * 0.40f) :
        (aiDifficulty == MEDIUM) ? (PI * 0.44f) : (PI * 0.47f);
    float minCutScore = (aiDifficulty == EASY) ? 0.18f :
        (aiDifficulty == MEDIUM) ? 0.12f : 0.07f;

    if (cutAngle > maxCutAngle) return shotInfo;
    float cutScore = std::max(0.0f, 1.0f - cutAngle / maxCutAngle);
    if (cutScore < minCutScore) return shotInfo;

    float distancePenalty = (magCue * 0.015f) + (magObj * 0.01f);
    shotInfo.score = (cutScore * 400.0f) - distancePenalty;
    if (relaxedPath)          shotInfo.score -= 80.0f;  // Prefer strict-path shots
    if (isBank)               shotInfo.score -= 500.0f;
    if (shotInfo.involves8Ball) shotInfo.score += 150.0f;

    shotInfo.angle = atan2f(ghostBallPos.y - cue->y, ghostBallPos.x - cue->x);
    shotInfo.power = CalculateShotPower(magCue, magObj, isBank);

    // No spin: avoids the small velocity-component deflection that was causing misses
    shotInfo.spinX = 0.0f;
    shotInfo.spinY = 0.0f;
...
/*AIShotInfo EvaluateShot_8Ball(Ball* targetBall, int pocketIndex, bool isBank) {
    ...
    // We aim for an Adaptive Aim Target to ensure straight-line shots do not clip physical walls,
    // while cut-shots dynamically adapt to use the proper geometric pocket center.
    D2D1_POINT_2F pocketPos = pocketPositions[pocketIndex];
    D2D1_POINT_2F aimTargetPos = ComputeAdaptiveAimTarget(pocketIndex, targetBall, cue);
    D2D1_POINT_2F targetPos = { targetBall->x, targetBall->y };
    ...
    if (!isBank && !IsPathClear(targetPos, aimTargetPos, targetBall->id, -1)) return shotInfo;
    ...
    // Pruning: Impossible to cut more than ~81 degrees effectively
    if (cutAngle > (PI * 0.45f)) return shotInfo;

    // Calculate Cut Score: 1.0 is straight-in
    float cutScore = std::max(0.0f, 1.0f - cutAngle / (PI * 0.45f));
    ...
    // --- NEW: Rudimentary English Spin Logic ---
    if (!isBank && cutAngle < PI * 0.15f) {
        shotInfo.spinY = -0.65f; // Draw (backspin) for straight shots to avoid scratching
        shotInfo.power = std::clamp(shotInfo.power * 1.15f, 0.0f, MAX_SHOT_POWER);
    }
    else if (!isBank && cutAngle >= PI * 0.15f && cutAngle < PI * 0.35f) {
        shotInfo.spinY = 0.45f;  // Follow (topspin) for moderate cuts
        shotInfo.spinX = 0.0f;
    }
    else {
        shotInfo.spinX = 0.0f;
...
AIShotInfo EvaluateShot_StraightPool(Ball* targetBall, int pocketIndex, bool isBank, bool relaxedPath /*= false*/) {
    ...
    float distancePenalty = (magCue * 0.015f) + (magObj * 0.01f);
    shotInfo.score = (cutScore * 400.0f) - distancePenalty;
    if (relaxedPath) shotInfo.score -= 80.0f;
    if (isBank)      shotInfo.score -= 500.0f;
...
/*AIShotInfo EvaluateShot_StraightPool(Ball* targetBall, int pocketIndex, bool isBank) {
    ...
    // --- NEW: Rudimentary English Spin Logic ---
    if (!isBank && cutAngle < PI * 0.15f) {
        shotInfo.spinY = -0.65f; // Draw (backspin) for straight shots
        shotInfo.power = std::clamp(shotInfo.power * 1.15f, 0.0f, MAX_SHOT_POWER);
    }
    else if (!isBank && cutAngle >= PI * 0.15f && cutAngle < PI * 0.35f) {
        shotInfo.spinY = 0.45f;  // Follow (topspin)
        shotInfo.spinX = 0.0f;
    }
    else {
        shotInfo.spinX = 0.0f;
...
float EvaluateTableState_StraightPool(Ball* cueBall) {
    ...
    for (Ball& b : balls) {
        ...
        for (int p = 0; p < 6; ++p) {
            D2D1_POINT_2F aimTargetPos = ComputeAdaptiveAimTarget(p, &b, cueBall);
            D2D1_POINT_2F ghostPos = CalculateGhostBallPos(&b, aimTargetPos);
            if (IsPathClear({ cueBall->x, cueBall->y }, ghostPos, cueBall->id, b.id)) {
                potentialShots++;
                // Bonus for easier shots (closer)
                float dist = GetDistance(cueBall->x, cueBall->y, ghostPos.x, ghostPos.y) + GetDistance(b.x, b.y, aimTargetPos.x, aimTargetPos.y);
...
std::vector<AIShotInfo> AIFindAllShots_8Ball() {
    std::vector<AIShotInfo> possibleShots;
    Ball* cue = GetCueBall();
    if (!cue) return possibleShots;

    const BallType wantType = (currentPlayer == 1) ? player1Info.assignedType : player2Info.assignedType;
    const bool on8 = IsPlayerOnEightBall(currentPlayer);
    std::vector<Ball*> legalObjectBalls;
    Ball* eightBallTarget = nullptr;
    ...
    for (Ball* targetBall : targetsToEvaluate) {
        for (int p = 0; p < 6; ++p) {
            AIShotInfo cand = EvaluateShot_8Ball(targetBall, p, false, false);
            if (cand.possible) possibleShots.push_back(cand);
        }
        if (aiDifficulty > EASY) AIFindBankShots(targetBall, possibleShots);
    }

    // ── PASS 2: relaxed path clearance (MEDIUM/HARD only) ──────────────────
    // Fires when nothing at all was found in pass 1. Slightly penalised score.
    if (possibleShots.empty() && aiDifficulty >= MEDIUM) {
        for (Ball* targetBall : targetsToEvaluate) {
            for (int p = 0; p < 6; ++p) {
                AIShotInfo cand = EvaluateShot_8Ball(targetBall, p, false, true);
                if (cand.possible) possibleShots.push_back(cand);
            }
        }
    }

    // ── Combo shots (MEDIUM/HARD) ───────────────────────────────────────────
    if (aiDifficulty > EASY) {
        for (Ball* targetBall : targetsToEvaluate) {
            for (Ball* firstContact : legalObjectBalls) {
                if (firstContact->id == targetBall->id) continue;
                for (int p = 0; p < 6; ++p) {
                    D2D1_POINT_2F aimTargetPos = GetPocketAimTarget(p);
                    D2D1_POINT_2F ghostTarget = CalculateGhostBallPos(targetBall, aimTargetPos);
                    if (!IsPathClear({ targetBall->x, targetBall->y }, aimTargetPos, targetBall->id, -1)) continue;
                    if (!IsPathClear({ firstContact->x, firstContact->y }, ghostTarget, firstContact->id, targetBall->id)) continue;
                    D2D1_POINT_2F ghostFirst = CalculateGhostBallPos(firstContact, ghostTarget);
                    if (!IsPathClear({ cue->x, cue->y }, ghostFirst, cue->id, firstContact->id)) continue;

                    AIShotInfo comboShot;
                    comboShot.possible = true;
                    comboShot.targetBall = firstContact;
                    comboShot.pocketIndex = p;
                    comboShot.isBankShot = false;
                    comboShot.involves8Ball = (targetBall->id == 8);
                    comboShot.angle = atan2f(ghostFirst.y - cue->y, ghostFirst.x - cue->x);
                    float d1 = GetDistance(cue->x, cue->y, ghostFirst.x, ghostFirst.y);
                    float d2 = GetDistance(firstContact->x, firstContact->y, ghostTarget.x, ghostTarget.y);
                    float d3 = GetDistance(targetBall->x, targetBall->y, aimTargetPos.x, aimTargetPos.y);
                    comboShot.power = std::clamp(CalculateShotPower(d1, d2 + d3, false) * 1.3f, 6.0f, MAX_SHOT_POWER);
                    comboShot.score = 250.0f - (d1 + d2 + d3) * 0.5f;
                    if (comboShot.involves8Ball) comboShot.score += 50.0f;
                    comboShot.spinX = 0.0f; comboShot.spinY = 0.0f;
                    possibleShots.push_back(comboShot);
                }
            }
        }
    }

    std::sort(possibleShots.begin(), possibleShots.end(), [](const AIShotInfo& a, const AIShotInfo& b) {
    ...
    if (possibleShots.empty()) {
        ...
        // ── Safety: gentle legal hit to avoid a foul ───────────────────────
        if (!clusterFound) {
            AIShotInfo safetyShot;
            safetyShot.possible = true;
            safetyShot.score = -99999.0f;
            safetyShot.spinX = 0.0f;
            // [+] FIX: CPU Opponent Foul Bug — heavy topspin/follow keeps cue ball rolling to rail
            safetyShot.spinY = 0.5f;
            ...
            if (bestTarget) {
                ...
                // [+] FIX: CPU Opponent Foul Bug — dynamic power based on distance to nearest rail
                float distCueToTarget = GetDistance(cue->x, cue->y, bestTarget->x, bestTarget->y);
                float distTargetToRail = GetDistanceToNearestRail(bestTarget->x, bestTarget->y);
                safetyShot.power = CalculateShotPower(distCueToTarget, distTargetToRail, false);
            }
            else {
                ...
                if (closestLegal) {
                    ...
                    // [+] BUG FIX: If completely snookered, attempt a rudimentary 1-rail kick 
                    // shot to avoid plowing straight into the blocking ball and guaranteeing a foul.
                    bool kickFound = false;
                    D2D1_POINT_2F rails[4] = {
                        {cue->x, TABLE_TOP + INNER_RIM_THICKNESS},
                        {cue->x, TABLE_BOTTOM - INNER_RIM_THICKNESS},
                        {TABLE_LEFT + INNER_RIM_THICKNESS, cue->y},
                        {TABLE_RIGHT - INNER_RIM_THICKNESS, cue->y}
                    };
                    for (int i = 0; i < 4; ++i) {
                        D2D1_POINT_2F vTarget = { closestLegal->x, closestLegal->y };
                        if (i == 0) vTarget.y = rails[i].y - (closestLegal->y - rails[i].y);
                        else if (i == 1) vTarget.y = rails[i].y + (rails[i].y - closestLegal->y);
                        else if (i == 2) vTarget.x = rails[i].x - (closestLegal->x - rails[i].x);
                        else if (i == 3) vTarget.x = rails[i].x + (rails[i].x - closestLegal->x);

                        float kickAngle = atan2f(vTarget.y - cue->y, vTarget.x - cue->x);
                        float hitDistSq = 0.0f;
                        Ball* firstHit = FindFirstHitBall({ cue->x, cue->y }, kickAngle, hitDistSq);

                        // If the kick angle hits no obstructing ball before the rail, use it!
                        if (!firstHit || hitDistSq > GetDistanceSq(cue->x, cue->y, rails[i].x, rails[i].y)) {
                            safetyShot.angle = kickAngle;
                            safetyShot.power = MAX_SHOT_POWER * 0.65f;
                            kickFound = true;
                            break;
                        }
                    }
                    ...
                    if (!kickFound) {
                        safetyShot.angle = atan2f(closestLegal->y - cue->y, closestLegal->x - cue->x);
                        float distCueToTarget = GetDistance(cue->x, cue->y, closestLegal->x, closestLegal->y);
                        float distTargetToRail = GetDistanceToNearestRail(closestLegal->x, closestLegal->y);
                        safetyShot.power = CalculateShotPower(distCueToTarget, distTargetToRail, false);
                    }
                }
                else {
                    // [+] FIX: No legal ball exists anywhere on the table
                    safetyShot.angle = (float)(rand() % 314) / 100.0f;
                    safetyShot.power = MAX_SHOT_POWER;
...
/*std::vector<AIShotInfo> AIFindAllShots_8Ball() {
    ...
    for (Ball* targetBall : targetsToEvaluate) {
        for (int p = 0; p < 6; ++p) {
            ...
            if (cand.possible) {
                ...
                if (cutAmountSq > (BALL_RADIUS * 1.9f) * (BALL_RADIUS * 1.9f)) cand.score += 50.0f;
                possibleShots.push_back(cand);
            }
        }
        if (aiDifficulty > EASY) AIFindBankShots(targetBall, possibleShots);
    ...
    // --- NEW: AI Combo Shots Logic (8-Ball) ---
    if (aiDifficulty > EASY) {
        for (Ball* targetBall : targetsToEvaluate) { // The ball to sink
            for (Ball* firstContact : legalObjectBalls) { // The ball to hit first
                if (firstContact->id == targetBall->id) continue;
                for (int p = 0; p < 6; ++p) {
                    D2D1_POINT_2F aimTargetPos = GetPocketAimTarget(p);
                    D2D1_POINT_2F ghostTarget = CalculateGhostBallPos(targetBall, aimTargetPos);
                    ...
                    // NERFED: Lowered from 800 to 250 so direct shots take priority
                    comboShot.score = 250.0f - (d1 + d2 + d3) * 0.5f; // Lower base score for difficulty
                    if (comboShot.involves8Ball) comboShot.score += 50.0f; // Smaller 8-ball bonus
                    comboShot.spinX = 0; comboShot.spinY = 0.0f;
                    possibleShots.push_back(comboShot);
                }
            }
        }
    ...
    if (possibleShots.empty()) {
        // --- NEW: AI Cluster Breaking Logic (8-Ball) ---
        bool clusterFound = false;
        for (Ball* target : targetsToEvaluate) {
            int nearbyBalls = 0;
            for (Ball& other : balls) {
            ...
            if (nearbyBalls >= 2) { // Target is in a cluster
                float aimAngle = atan2f(target->y - cue->y, target->x - cue->x);
                float hitDistSq = 0.0f;
                Ball* hitB = FindFirstHitBall({ cue->x, cue->y }, aimAngle, hitDistSq);

                // FIX: Use raycast instead of IsPathClear to ignore balls behind the target
                if (hitB && hitB->id == target->id) {
                    AIShotInfo clusterShot;
                    clusterShot.possible = true;
                    clusterShot.targetBall = target;
                    clusterShot.pocketIndex = -1;
                    clusterShot.angle = atan2f(target->y - cue->y, target->x - cue->x);
                    clusterShot.power = MAX_SHOT_POWER; // BLAAAST
                    clusterShot.spinY = -0.5f; // Draw spin for control
                    clusterShot.spinX = 0.0f;
                    // NERFED: Lowered from 500 to 150 so it's a fallback, not a priority
                    clusterShot.score = 150.0f; // Heavily favored over basic safety
                    possibleShots.push_back(clusterShot);
                    clusterFound = true;
                    break;
                }
        ...
        if (!clusterFound) {
            ...
            if (bestSafetyTarget) {
                ...
                safetyShot.targetBall = bestSafetyTarget;
                safetyShot.angle = atan2f(ghostPos.y - cue->y, ghostPos.x - cue->x);
                safetyShot.power = MAX_SHOT_POWER * 0.2f;
            }
            else {
                safetyShot.angle = (float)(rand() % 314) / 100.0f;
                safetyShot.power = MAX_SHOT_POWER * 0.2f;
...
AIShotInfo AIFindBestShot_8Ball() {
    return AIFindAllShots_8Ball().front();
}
...
std::vector<AIShotInfo> AIFindAllShots_StraightPool() {
    ...
    bool isClusterScenario = (ballsOnTableCount >= 10);

    // ── PASS 1: strict path clearance ─────────────────────────────────────
    for (Ball& b : balls) {
        if (b.isPocketed || b.id == 0) continue;
        for (int p = 0; p < 6; ++p) {
            AIShotInfo cand = EvaluateShot_StraightPool(&b, p, false, false);
            if (cand.possible) {
                if (isClusterScenario) {
                    float distToRack = GetDistance(b.x, b.y, RACK_POS_X, RACK_POS_Y);
                    if (distToRack < BALL_RADIUS * 6.0f) {
                        cand.power = MAX_SHOT_POWER;
                        cand.score += 2000.0f;
                    }
                }
                possibleShots.push_back(cand);
    ...
    if (possibleShots.empty() && aiDifficulty >= MEDIUM) {
        for (Ball& b : balls) {
            if (b.isPocketed || b.id == 0) continue;
            for (int p = 0; p < 6; ++p) {
                AIShotInfo cand = EvaluateShot_StraightPool(&b, p, false, true);
                if (cand.possible) {
                    if (isClusterScenario) {
                        float distToRack = GetDistance(b.x, b.y, RACK_POS_X, RACK_POS_Y);
                        if (distToRack < BALL_RADIUS * 6.0f) {
    ...
    if (aiDifficulty > EASY) {
        for (Ball& targetBall : balls) {
            ...
            for (Ball& firstContact : balls) {
                if (firstContact.isPocketed || firstContact.id == 0 || firstContact.id == targetBall.id) continue;
                for (int p = 0; p < 6; ++p) {
                    D2D1_POINT_2F aimTargetPos = GetPocketAimTarget(p);
                    D2D1_POINT_2F ghostTarget = CalculateGhostBallPos(&targetBall, aimTargetPos);
                    if (!IsPathClear({ targetBall.x, targetBall.y }, aimTargetPos, targetBall.id, -1))          continue;
                    ...
                    comboShot.power = std::clamp(CalculateShotPower(d1, d2 + d3, false) * 1.3f, 6.0f, MAX_SHOT_POWER);
                    comboShot.score = 250.0f - (d1 + d2 + d3) * 0.5f;
                    if (isClusterScenario) comboShot.score += 50.0f;
                    comboShot.spinX = 0.0f;
                    comboShot.spinY = 0.0f;
                    possibleShots.push_back(comboShot);
                }
            }
        }
    ...
    if (possibleShots.empty()) {
        ...
        if (!clusterFound) {
            ...
            safetyShot.score = -99999.0f;
            safetyShot.spinX = 0.0f;
            // [+] FIX: CPU Opponent Foul Bug — heavy topspin/follow keeps cue ball rolling
            safetyShot.spinY = 0.5f;
            safetyShot.pocketIndex = -1;

            Ball* bestTarget = nullptr;
            float  bestDistSq = 1e10f;
            // [+] FIX: Straight Pool Wrong-Ball-First Foul — raycast to every ball
            // on the table and only accept one as bestTarget if it is genuinely
            // the FIRST ball the cue ball's path would strike.
            for (Ball& b : balls) {
                if (b.isPocketed || b.id == 0) continue;
                float angleToB = atan2f(b.y - cue->y, b.x - cue->x);
                float hitDistSq = 0.0f;
                Ball* firstHit = FindFirstHitBall({ cue->x, cue->y }, angleToB, hitDistSq);
                if (firstHit && firstHit->id == b.id && hitDistSq < bestDistSq) {
                    bestDistSq = hitDistSq;
                    bestTarget = &b;
                }
            }
            if (bestTarget) {
                ...
                // [+] FIX: CPU Opponent Foul Bug — power now scales with how far
                // the target ball must travel to reach a cushion
                float distCueToTarget = GetDistance(cue->x, cue->y, bestTarget->x, bestTarget->y);
                float distTargetToRail = GetDistanceToNearestRail(bestTarget->x, bestTarget->y);
                safetyShot.power = CalculateShotPower(distCueToTarget, distTargetToRail, false);
            }
            else {
                // [+] FIX: No clear path to ANY ball. Fall back to the closest
                // ball by raw distance.
                Ball* closestLegal = nullptr;
                float closestDistSq = 1e10f;
                for (Ball& b : balls) {
                    if (b.isPocketed || b.id == 0) continue;
                    float dSq = GetDistanceSq(cue->x, cue->y, b.x, b.y);
                    if (dSq < closestDistSq) { closestDistSq = dSq; closestLegal = &b; }
                }
                if (closestLegal) {
                    safetyShot.targetBall = closestLegal;
                    safetyShot.angle = atan2f(closestLegal->y - cue->y, closestLegal->x - cue->x);
                    ...
                    // [+] FIX: No ball at all on the table to reference
                    safetyShot.angle = (float)(rand() % 314) / 100.0f;
                    safetyShot.power = MAX_SHOT_POWER;
                    safetyShot.targetBall = nullptr;
                }
...
// [Location: Replace existing AIFindBestShot_StraightPool function]
// --- REPLACED: Straight Pool AI with Rack-Breaking Logic ---
// ==================== STRAIGHT POOL AI ====================
/*std::vector<AIShotInfo> AIFindAllShots_StraightPool() {
    std::vector<AIShotInfo> possibleShots;
    Ball* cue = GetCueBall();
    ...
    // 2. Evaluate Combo Shots (Straight Pool)
    // --- NEW: AI Combo Shots Logic (Straight Pool) ---
    if (aiDifficulty > EASY) {
        for (Ball& targetBall : balls) {
            if (targetBall.isPocketed || targetBall.id == 0) continue;
            for (Ball& firstContact : balls) {
                ...
                for (int p = 0; p < 6; ++p) {
                    ...
                    // NERFED: Lowered from 800 to 250
                    comboShot.score = 250.0f - (d1 + d2 + d3) * 0.5f;
                    if (isClusterScenario) comboShot.score += 50.0f; // Nerfed from +1500 // High priority if clustered
                    comboShot.spinX = 0; comboShot.spinY = 0;
                    possibleShots.push_back(comboShot);
                }
            }
        }
    ...
    // 3. Sort Shots by Score
    std::sort(possibleShots.begin(), possibleShots.end(), [](const AIShotInfo& a, const AIShotInfo& b) {
        return a.score > b.score;
        });

    // 4. Fallback: Cluster Breaking & Safety Logic
    if (possibleShots.empty()) {
        bool clusterFound = false;

        // Try to find a cluster to smash
        for (Ball& target : balls) {
            if (target.id == 0 || target.isPocketed) continue;
            ...
            if (nearbyBalls >= 2) { // Target is in a cluster
                ...
                if (hitB && hitB->id == target.id) {
                    ...
                    clusterShot.angle = atan2f(target.y - cue->y, target.x - cue->x);
                    clusterShot.power = MAX_SHOT_POWER; // BLAAAST
                    clusterShot.spinY = -0.5f; // Draw spin for cue ball control
                    clusterShot.spinX = 0.0f;
                    // NERFED: Lowered from 500 to 150
                    clusterShot.score = 150.0f; // Heavily favored over basic safety
                    possibleShots.push_back(clusterShot);
                    clusterFound = true;
                    break;
        ...
        // If no cluster to smash, execute standard safety
        if (!clusterFound) {
            AIShotInfo safetyShot;
            safetyShot.possible = true;
            safetyShot.score = -99999.0f;
            safetyShot.spinX = 0; safetyShot.spinY = 0; safetyShot.pocketIndex = -1;
            ...
            if (easiestTarget) {
                ...
                safetyShot.targetBall = easiestTarget;
                safetyShot.angle = atan2f(ghostPos.y - cue->y, ghostPos.x - cue->x);
                safetyShot.power = MAX_SHOT_POWER * 0.2f;
            }
            else {
...
AIShotInfo AIFindBestShot_StraightPool() {
    return AIFindAllShots_StraightPool().front();
}

// uncomment below 6-2-2026 latest gemini fixes for AI amend current below vv
...
float EvaluateTableState_NineBall(Ball* cueBall) {
    ...
    for (int p = 0; p < 6; ++p) {
        D2D1_POINT_2F aimTargetPos = ComputeAdaptiveAimTarget(p, nextTarget, cueBall);
        D2D1_POINT_2F ghostPos = CalculateGhostBallPos(nextTarget, aimTargetPos);
        if (IsPathClear({ cueBall->x, cueBall->y }, ghostPos, cueBall->id, nextTarget->id)) {
            float dist = GetDistance(cueBall->x, cueBall->y, ghostPos.x, ghostPos.y) + GetDistance(nextTarget->x, nextTarget->y, aimTargetPos.x, aimTargetPos.y);
...
AIShotInfo EvaluateShot_NineBall(Ball* targetBall, int pocketIndex, bool isBank, bool isCombo, bool relaxedPath /*= false*/) {
    ...
    if (relaxedPath)                          shotInfo.score -= 80.0f;
    if (isBank)                               shotInfo.score -= 500.0f;
    if (isCombo)                              shotInfo.score += 500.0f;
    if (targetBall->id == lowestBallOnTable)  shotInfo.score += 1000.0f;
    if (targetBall->id == 9)                  shotInfo.score += 10000.0f;
...
/*AIShotInfo EvaluateShot_NineBall(Ball* targetBall, int pocketIndex, bool isBank, bool isCombo) {
    ...
    // --- NEW: Rudimentary English Spin Logic ---
    if (!isBank && !isCombo && cutAngle < PI * 0.15f) {
        shotInfo.spinY = -0.65f; // Draw (backspin) for straight shots
        shotInfo.power = std::clamp(shotInfo.power * 1.15f, 0.0f, MAX_SHOT_POWER);
    }
    else if (!isBank && !isCombo && cutAngle >= PI * 0.15f && cutAngle < PI * 0.35f) {
        shotInfo.spinY = 0.45f;  // Follow (topspin)
        shotInfo.spinX = 0.0f;
    }
    else {
...
std::vector<AIShotInfo> AIFindAllShots_NineBall() {
    ...
    if (!targetBall) return possibleShots;

    // Push-out logic
    bool isAITurn = g_isDemoMode || (isPlayer2AI && currentPlayer == 2);
    if (isPushOutAvailable && isAITurn && !g_hintActive) {
        bool canSeeTarget = IsPathClear({ cue->x, cue->y }, { targetBall->x, targetBall->y }, cue->id, targetBall->id);
        bool hasGoodShot = false;
        if (canSeeTarget) {
            for (int p = 0; p < 6; ++p) {
                AIShotInfo cand = EvaluateShot_NineBall(targetBall, p, false, false, false);
                if (cand.possible && cand.score > 500.0f) { hasGoodShot = true; break; }
            }
        }
        isPushOutAvailable = false;
        if (!hasGoodShot) {
            isPushOutShot = true;
            AIShotInfo pushOutShot;
            pushOutShot.possible = true;
            pushOutShot.score = 0;
            pushOutShot.angle = atan2f((TABLE_TOP + TABLE_HEIGHT / 2.0f) - cue->y,
                (TABLE_LEFT + TABLE_WIDTH / 2.0f) - cue->x);
            pushOutShot.power = MAX_SHOT_POWER * 0.2f;
            possibleShots.push_back(pushOutShot);
            return possibleShots;
        }
    }

    // ── PASS 1: strict direct shots at lowest ball ─────────────────────────
    for (int p = 0; p < 6; ++p) {
        AIShotInfo cand = EvaluateShot_NineBall(targetBall, p, false, false, false);
        if (cand.possible) possibleShots.push_back(cand);
    }
    ...
    if (possibleShots.empty() && aiDifficulty >= MEDIUM) {
        for (int p = 0; p < 6; ++p) {
            AIShotInfo cand = EvaluateShot_NineBall(targetBall, p, false, false, true);
            if (cand.possible) possibleShots.push_back(cand);
        }
    }

    // ── Combo shots: cue -> intermediate -> lowest ball -> pocket ──────────
    for (Ball& intermediate : balls) {
        if (intermediate.isPocketed || intermediate.id == 0 || intermediate.id == targetBall->id) continue;
        for (int p = 0; p < 6; ++p) {
            AIShotInfo comboCand = EvaluateShot_NineBall(&intermediate, p, false, true, false);
            if (!comboCand.possible) continue;

            if (!IsPathClear({ targetBall->x, targetBall->y }, comboCand.ghostBallPos, targetBall->id, intermediate.id)) continue;
            D2D1_POINT_2F finalGhostForCue = CalculateGhostBallPos(targetBall, comboCand.ghostBallPos);
            if (!IsPathClear({ cue->x, cue->y }, finalGhostForCue, cue->id, targetBall->id)) continue;

            comboCand.angle = atan2f(finalGhostForCue.y - cue->y, finalGhostForCue.x - cue->x);
            float totalDist = GetDistance(cue->x, cue->y, finalGhostForCue.x, finalGhostForCue.y)
                + GetDistance(targetBall->x, targetBall->y, comboCand.ghostBallPos.x, comboCand.ghostBallPos.y);
            comboCand.score -= totalDist * 0.5f;
    ...
    if (possibleShots.empty()) {
        ...
        if (canSmash) {
            ...
            safetyShot.score = -99999.0f;
            safetyShot.spinX = 0.0f;
            // [+] FIX: CPU Opponent Foul Bug — heavy topspin/follow
            safetyShot.spinY = 0.5f;
            safetyShot.pocketIndex = -1;
            safetyShot.targetBall = targetBall;

            // [+] BUG FIX: If completely snookered in 9-Ball, attempt a 1-rail kick 
            // shot instead of blasting directly into the obstructing ball.
            bool kickFound = false;
            D2D1_POINT_2F rails[4] = {
                {cue->x, TABLE_TOP + INNER_RIM_THICKNESS},
                {cue->x, TABLE_BOTTOM - INNER_RIM_THICKNESS},
            ...
            for (int i = 0; i < 4; ++i) {
                D2D1_POINT_2F vTarget = { targetBall->x, targetBall->y };
                if (i == 0) vTarget.y = rails[i].y - (targetBall->y - rails[i].y);
                else if (i == 1) vTarget.y = rails[i].y + (rails[i].y - targetBall->y);
                else if (i == 2) vTarget.x = rails[i].x - (targetBall->x - rails[i].x);
                else if (i == 3) vTarget.x = rails[i].x + (rails[i].x - targetBall->x);
            ...
            if (!kickFound) {
                // If kicks fail, fall back to direct aim (inevitable foul but handles edge cases)
                ...
                // [+] FIX: Dynamic safety power based on distance to nearest rail
                float distCueToTarget = GetDistance(cue->x, cue->y, targetBall->x, targetBall->y);
                float distTargetToRail = GetDistanceToNearestRail(targetBall->x, targetBall->y);
                safetyShot.power = CalculateShotPower(distCueToTarget, distTargetToRail, false);
            }
...
/*std::vector<AIShotInfo> AIFindAllShots_NineBall() {
    ...
    // AI Pushout logic (Only if it's the AI's actual turn, ignore if we're just calculating hints)
    bool isAITurn = g_isDemoMode || (isPlayer2AI && currentPlayer == 2);
    if (isPushOutAvailable && isAITurn && !g_hintActive) {
        bool canSeeTarget = IsPathClear({ cue->x, cue->y }, { targetBall->x, targetBall->y }, cue->id, targetBall->id);
        bool hasGoodShot = false;
        if (canSeeTarget) {
            for (int p = 0; p < 6; ++p) {
                ...
                if (cand.possible && cand.score > 500.0f) {
                    hasGoodShot = true; break;
                }
            }
        }
        if (hasGoodShot) { isPushOutAvailable = false; }
        else {
            isPushOutAvailable = false;
            isPushOutShot = true;
            AIShotInfo pushOutShot;
            pushOutShot.possible = true;
            pushOutShot.score = 0;
            pushOutShot.angle = atan2f((TABLE_TOP + TABLE_HEIGHT / 2.0f) - cue->y, (TABLE_LEFT + TABLE_WIDTH / 2.0f) - cue->x);
            pushOutShot.power = MAX_SHOT_POWER * 0.2f;
            possibleShots.push_back(pushOutShot);
            return possibleShots;
        }
    ...
    for (Ball& intermediate : balls) {
        if (intermediate.isPocketed || intermediate.id == 0 || intermediate.id == targetBall->id) continue;
        for (int p = 0; p < 6; ++p) {
            AIShotInfo comboCand = EvaluateShot_NineBall(&intermediate, p, false, true);
            if (comboCand.possible) {
                if (IsPathClear({ targetBall->x, targetBall->y }, comboCand.ghostBallPos, targetBall->id, intermediate.id)) {
                    ...
                    if (IsPathClear({ cue->x, cue->y }, finalGhostForCue, cue->id, targetBall->id)) {
                        ...
                        // NERFED: Remove the wild +1000.0f bonus.
                        // Instead, penalize combos based on distance so direct shots stay king,
                        // UNLESS this combo sinks the 9-ball for a win.
                        float totalDist = GetDistance(cue->x, cue->y, finalGhostForCue.x, finalGhostForCue.y) +
                            GetDistance(targetBall->x, targetBall->y, comboCand.ghostBallPos.x, comboCand.ghostBallPos.y);

                        comboCand.score -= (totalDist * 0.5f); // Apply difficulty penalty
...
AIShotInfo AIFindBestShot_NineBall() {
    return AIFindAllShots_NineBall().front();
}
...
void AIMakeDecision() {
    ...
    if (bestShot.possible) {
        ...
        if (currentGameType == GameType::EIGHT_BALL_MODE && bestShot.involves8Ball) { // 8-ball is ID 8
            if (currentPlayer == 1) calledPocketP1 = bestShot.pocketIndex;
            else calledPocketP2 = bestShot.pocketIndex;
        }
        else {
...
D2D1_POINT_2F CalculateGhostBallPos(Ball* targetBall, D2D1_POINT_2F targetPos) {
    if (!targetBall) return { 0, 0 };

    // Calculate the vector from the target ball to the aim target
    float dx = targetPos.x - targetBall->x;
    float dy = targetPos.y - targetBall->y;
    ...
    // The ghost ball must be exactly 2.0 radii away from the target ball 
    // along the aim line, positioned BEHIND the target ball.
    return {
...
/*//  Calculate ghost-ball position so that cue hits target towards pocket (disable 19/3/26 7PM)
...
/*//  Heuristic: shorter + straighter + proper group = higher score (disable 19/3/26 7PM)
...
bool IsPathClear(D2D1_POINT_2F start, D2D1_POINT_2F end, int ignoredBallId1, int ignoredBallId2, float clearanceMult /*= 2.02f*/) {
    //bool IsPathClear(D2D1_POINT_2F start, D2D1_POINT_2F end, int ignoredBallId1, int ignoredBallId2) {
        ...
        // a 2.0x ball radius margin of the line segment.
    const float clearanceRadius = BALL_RADIUS * clearanceMult;
    const float clearanceRadiusSq = clearanceRadius * clearanceRadius;
    //const float clearanceRadius = BALL_RADIUS * 2.02f; // Added slight margin
    //const float clearanceRadiusSq = clearanceRadius * clearanceRadius;
    ...
    float& hitDistSq)
{
    ...
    // [+] BUG FIX: The collision radius for two balls is 2 * BALL_RADIUS, not 1 * BALL_RADIUS!
    const float collisionRadius = BALL_RADIUS * 2.0f;
    ...
    for (Ball& b : balls)
    {
        ...
        if (dSq <= collisionRadiusSq)            // intersection
        {
            float back = sqrtf(collisionRadiusSq - dSq);
...
// StartMidi (Resource ID) - primary start function
void StartMidi(HWND hwnd, int resourceId) {
    // If already playing, do nothing
    if (isMusicPlaying.load()) return;

    // Extract MIDI from resources to temp folder
    std::wstring midiPath = ExtractMidiResource(resourceId);
...
void DrawScene(ID2D1RenderTarget* pRT) {
    ...
    // [+] NEW: Large centred English popup overlay.  Visible while the
// linger counter is > 0.  Active drag keeps it pinned to 9999; a
// single click sets it to ~90 frames (~1.5s) on release; right-click
// on the spin indicator or starting a new game zeroes it.
    if (g_spinPopupLingerCounter > 0 && !isSettingEnglish) {
        g_spinPopupLingerCounter--;
    }
    if (g_spinPopupLingerCounter > 0) DrawSpinPopup(pRT);
...
// --- NEW: Draws subtle wood-grain streaks inside a rail rectangle ---
static void DrawWoodGrainStreaks(ID2D1RenderTarget* pRT, ID2D1Factory* pFactory,
    const D2D1_RECT_F& rect, bool horizontal)
{
    ...
    // Dark patch brush (soft, low opacity for natural wood look)
    ID2D1SolidColorBrush* pPatchBrush = nullptr;
    pRT->CreateSolidColorBrush(D2D1::ColorF(0.06f, 0.03f, 0.01f, 0.28f), &pPatchBrush);
    if (!pPatchBrush) return;

    // Slightly darker patches for depth
    ID2D1SolidColorBrush* pDarkPatchBrush = nullptr;
    pRT->CreateSolidColorBrush(D2D1::ColorF(0.04f, 0.02f, 0.005f, 0.22f), &pDarkPatchBrush);

    if (horizontal)
    {
        // === HORIZONTAL RAILS (top/bottom) ===
        // Create soft, irregular dark patches instead of lines
        const int NUM_PATCHES = 11;
        for (int i = 0; i < NUM_PATCHES; ++i)
        {
            float t = (i + 0.5f) / NUM_PATCHES;
            float centerX = rect.left + width * t;
            float centerY = rect.top + height * (0.35f + ((i % 3) - 1) * 0.15f);

            // Vary size and shape for natural look
            float w = 18.0f + (i % 4) * 6.0f;
            float h = 4.5f + (i % 3) * 2.0f;

            // Slight random offset for organic feel
            float offsetX = ((i * 17) % 7 - 3) * 4.0f;
            float offsetY = ((i * 11) % 5 - 2) * 1.5f;

            D2D1_ELLIPSE patch = D2D1::Ellipse(
                D2D1::Point2F(centerX + offsetX, centerY + offsetY), w, h
            );

            pRT->FillEllipse(&patch, pPatchBrush);

            // Add a few darker overlapping patches for depth
            if (i % 3 == 0) {
                D2D1_ELLIPSE darkPatch = D2D1::Ellipse(
                    D2D1::Point2F(centerX + offsetX * 0.6f, centerY + offsetY * 0.7f),
                    w * 0.55f, h * 0.7f
                );
                pRT->FillEllipse(&darkPatch, pDarkPatchBrush);
            }
        }
    }
    else
    {
        ...
        {
            float t = (i + 0.5f) / NUM_PATCHES;
            float centerX = rect.left + width * (0.4f + ((i % 3) - 1) * 0.12f);
            float centerY = rect.top + height * t;

            float w = 4.0f + (i % 3) * 2.0f;
            float h = 16.0f + (i % 4) * 5.0f;

            float offsetX = ((i * 13) % 5 - 2) * 1.2f;
            float offsetY = ((i * 19) % 7 - 3) * 4.0f;
    ...
    SafeRelease(&pPatchBrush);
    SafeRelease(&pDarkPatchBrush);
}
    ...
    const D2D1_RECT_F& rect, bool horizontal)
{
    ...
    // Layer 1: Main grain lines
    ID2D1SolidColorBrush* pGrainBrush = nullptr;
    pRT->CreateSolidColorBrush(D2D1::ColorF(0.08f, 0.04f, 0.01f, 0.45f), &pGrainBrush);

    // Layer 2: Fine detail grain (lighter)
    ID2D1SolidColorBrush* pFineBrush = nullptr;
    pRT->CreateSolidColorBrush(D2D1::ColorF(0.15f, 0.07f, 0.02f, 0.25f), &pFineBrush);

    if (horizontal)
    {
        // === HORIZONTAL RAILS (top/bottom) ===
        const int NUM_MAIN = 9;
        const int NUM_FINE = 14;

        auto drawStreak = [&](int i, int total, ID2D1SolidColorBrush* brush, float thickness, float ampMul) {
            float t = (i + 0.3f + (i % 3) * 0.1f) / total;
            float baseY = rect.top + height * t;
            float amp = (height * 0.18f) * ampMul;

            ID2D1PathGeometry* pGeom = nullptr;
            if (FAILED(pFactory->CreatePathGeometry(&pGeom)) || !pGeom) return;

            ID2D1GeometrySink* pSink = nullptr;
            if (SUCCEEDED(pGeom->Open(&pSink)) && pSink) {
                float x0 = rect.left;
                float x1 = rect.left + width * 0.33f;
                float x2 = rect.left + width * 0.66f;
                float x3 = rect.right;

                float y0 = baseY + ((i % 5) - 2) * (amp * 0.2f);
                float y1 = baseY + ((i * 7) % 5 - 2) * amp * 0.6f;
                float y2 = baseY + ((i * 3) % 5 - 2) * amp * 0.7f;
                float y3 = baseY + ((i % 4) - 1.5f) * (amp * 0.25f);

                pSink->BeginFigure(D2D1::Point2F(x0, y0), D2D1_FIGURE_BEGIN_HOLLOW);
                pSink->AddBezier(D2D1::BezierSegment(D2D1::Point2F(x1, y1),
                    D2D1::Point2F(x2, y2),
                    D2D1::Point2F(x3, y3)));
                pSink->EndFigure(D2D1_FIGURE_END_OPEN);
                pSink->Close();
            ...
            pRT->DrawGeometry(pGeom, brush, thickness);
            SafeRelease(&pGeom);
        };

        for (int i = 0; i < NUM_MAIN; ++i)
            drawStreak(i, NUM_MAIN, pGrainBrush, 0.9f + (i % 2) * 0.4f, 1.0f);

        for (int i = 0; i < NUM_FINE; ++i)
            drawStreak(i, NUM_FINE, pFineBrush, 0.5f, 0.6f);

        // Small knots
        for (int i = 0; i < 3; ++i) {
            float kx = rect.left + width * (0.2f + i * 0.28f);
            float ky = rect.top + height * (0.3f + (i % 2) * 0.4f);
            D2D1_ELLIPSE knot = D2D1::Ellipse(D2D1::Point2F(kx, ky), 1.2f, 0.6f);
            pRT->FillEllipse(&knot, pGrainBrush);
        }
    }
    else
    {
        ...
        const int NUM_FINE = 11;

        auto drawStreak = [&](int i, int total, ID2D1SolidColorBrush* brush, float thickness) {
            float t = (i + 0.4f) / total;
            float baseX = rect.left + width * t;
            float amp = width * 0.22f;
            ...
            if (SUCCEEDED(pGeom->Open(&pSink)) && pSink) {
                float y0 = rect.top;
                float y1 = rect.top + height * 0.33f;
                float y2 = rect.top + height * 0.66f;
                float y3 = rect.bottom;

                float x0 = baseX + ((i % 3) - 1) * amp * 0.15f;
                float x1 = baseX + ((i * 5) % 5 - 2) * amp * 0.55f;
                float x2 = baseX + ((i * 2) % 5 - 2) * amp * 0.65f;
                float x3 = baseX + ((i % 4) - 1.5f) * amp * 0.2f;
        ...
        for (int i = 0; i < NUM_MAIN; ++i)
            drawStreak(i, NUM_MAIN, pGrainBrush, 0.8f + (i % 2) * 0.35f);

        for (int i = 0; i < NUM_FINE; ++i)
            drawStreak(i, NUM_FINE, pFineBrush, 0.4f);
    }

    SafeRelease(&pGrainBrush);
    SafeRelease(&pFineBrush);
}*/
...
// with the correct 45-degree V-Cut chamfers at the pockets.
    ...
    // We'll create a small multi-stop gradient that reads like polished wood.
    {
        ...
        if (SUCCEEDED(pRT->CreateGradientStopCollection(railStops, 3, &pRailStops)) && pRailStops) {
            ...
            if (pRailTopBrush) {
                D2D1_RECT_F topRailA = D2D1::RectF(TABLE_LEFT + CORNER_HOLE_VISUAL_RADIUS, TABLE_TOP - WOODEN_RAIL_THICKNESS, TABLE_LEFT + TABLE_WIDTH / 2.f - MIDDLE_HOLE_VISUAL_RADIUS, TABLE_TOP);
                D2D1_RECT_F topRailB = D2D1::RectF(TABLE_LEFT + TABLE_WIDTH / 2.f + MIDDLE_HOLE_VISUAL_RADIUS, TABLE_TOP - WOODEN_RAIL_THICKNESS, TABLE_RIGHT - CORNER_HOLE_VISUAL_RADIUS, TABLE_TOP);
                pRT->FillRectangle(topRailA, pRailTopBrush);
                pRT->FillRectangle(topRailB, pRailTopBrush);
                // [+] NEW: Wood grain texture
                DrawWoodGrainStreaks(pRT, pFactory, topRailA, true);
                DrawWoodGrainStreaks(pRT, pFactory, topRailB, true);
            }

            // Draw bottom rails
            if (pRailBottomBrush) {
                D2D1_RECT_F bottomRailA = D2D1::RectF(TABLE_LEFT + CORNER_HOLE_VISUAL_RADIUS, TABLE_BOTTOM, TABLE_LEFT + TABLE_WIDTH / 2.f - MIDDLE_HOLE_VISUAL_RADIUS, TABLE_BOTTOM + WOODEN_RAIL_THICKNESS);
                D2D1_RECT_F bottomRailB = D2D1::RectF(TABLE_LEFT + TABLE_WIDTH / 2.f + MIDDLE_HOLE_VISUAL_RADIUS, TABLE_BOTTOM, TABLE_RIGHT - CORNER_HOLE_VISUAL_RADIUS, TABLE_BOTTOM + WOODEN_RAIL_THICKNESS);
                pRT->FillRectangle(bottomRailA, pRailBottomBrush);
                pRT->FillRectangle(bottomRailB, pRailBottomBrush);
                // [+] NEW: Wood grain texture
                DrawWoodGrainStreaks(pRT, pFactory, bottomRailA, true);
                DrawWoodGrainStreaks(pRT, pFactory, bottomRailB, true);
            }

            // Draw side rails
            if (pRailLeftBrush) {
                D2D1_RECT_F leftRail = D2D1::RectF(TABLE_LEFT - WOODEN_RAIL_THICKNESS, TABLE_TOP + CORNER_HOLE_VISUAL_RADIUS, TABLE_LEFT, TABLE_BOTTOM - CORNER_HOLE_VISUAL_RADIUS);
                pRT->FillRectangle(leftRail, pRailLeftBrush);
                // [+] NEW: Wood grain texture
                DrawWoodGrainStreaks(pRT, pFactory, leftRail, false);
            }
            if (pRailRightBrush) {
                D2D1_RECT_F rightRail = D2D1::RectF(TABLE_RIGHT, TABLE_TOP + CORNER_HOLE_VISUAL_RADIUS, TABLE_RIGHT + WOODEN_RAIL_THICKNESS, TABLE_BOTTOM - CORNER_HOLE_VISUAL_RADIUS);
                pRT->FillRectangle(rightRail, pRailRightBrush);
                // [+] NEW: Wood grain texture
                DrawWoodGrainStreaks(pRT, pFactory, rightRail, false);
            }
...
void DrawBalls(ID2D1RenderTarget* pRT)
{
    ...
    ID2D1SolidColorBrush* pMotionTrailBrush = nullptr; // [+] NEW: dark gray motion trail
    ...
    pRT->CreateSolidColorBrush(D2D1::ColorF(D2D1::ColorF::DarkGray/*0.2f, 0.2f, 0.2f, 0.35f*/), &pMotionTrailBrush); // [+] NEW

    for (const Ball& b : balls)
    {
        if (b.isPocketed) continue;

        // ==================== MOTION TRAIL (Improved & Pronounced) ====================
        if (pMotionTrailBrush) {
            float speedSq = b.vx * b.vx + b.vy * b.vy;
            const float TRAIL_MIN_SPEED = 0.2f; // Almost instantly triggers on movement

            if (speedSq > TRAIL_MIN_SPEED * TRAIL_MIN_SPEED) {
                float speed = sqrtf(speedSq);
                float dirX = b.vx / speed;
                float dirY = b.vy / speed;

                // Ramps up to max opacity much faster (at a speed of 4.0 instead of 10.0)
                float speedFactor = (speed - TRAIL_MIN_SPEED) / 4.0f;
                if (speedFactor < 0.0f) speedFactor = 0.0f;
                if (speedFactor > 1.0f) speedFactor = 1.0f;

                // Create a brilliant white-ish trail heavily tinted with the ball's original color
                D2D1_COLOR_F trailColor = (b.id == 0) ?
                    D2D1::ColorF(1.0f, 1.0f, 1.0f) :
                    D2D1::ColorF(0.85f + b.color.r * 0.15f, 0.85f + b.color.g * 0.15f, 0.85f + b.color.b * 0.15f);

                const int TRAIL_SEGMENTS = 12; // Dramatically longer trailing smear
                for (int t = 1; t <= TRAIL_SEGMENTS; ++t) {
                    ...
                    // Gentler taper so the trail stays thick for longer
                    float trailRadius = BALL_RADIUS * (1.0f - 0.055f * t);

                    if (trailRadius > 0) {
                        D2D1_ELLIPSE trailEllipse = D2D1::Ellipse(
                            D2D1::Point2F(b.x - dirX * trailDist, b.y - dirY * trailDist),
                            trailRadius, trailRadius
                        );
                        pRT->FillEllipse(&trailEllipse, pMotionTrailBrush);
                    }
                }
            }
        }
        ...
        if (pMotionTrailBrush) {
            float speedSq = b.vx * b.vx + b.vy * b.vy;
            const float TRAIL_MIN_SPEED = 1.5f; // units/frame before a trail appears
            if (speedSq > TRAIL_MIN_SPEED * TRAIL_MIN_SPEED) {
                float speed = sqrtf(speedSq);
                float dirX = b.vx / speed;
                float dirY = b.vy / speed;
                float speedFactor = std::clamp((speed - TRAIL_MIN_SPEED) / (MAX_SHOT_POWER - TRAIL_MIN_SPEED), 0.0f, 1.0f);

                const int TRAIL_SEGMENTS = 4;
                for (int t = 1; t <= TRAIL_SEGMENTS; ++t) {
                    float trailDist = t * (BALL_RADIUS * 0.55f);
                    float alpha = 0.30f * speedFactor * (1.0f - (float)t / (TRAIL_SEGMENTS + 1));
                    float trailRadius = BALL_RADIUS * (1.0f - 0.12f * t);
                    pMotionTrailBrush->SetOpacity(alpha);
                    D2D1_ELLIPSE trailEllipse = D2D1::Ellipse(
                        D2D1::Point2F(b.x - dirX * trailDist, b.y - dirY * trailDist),
                        trailRadius, trailRadius);
                    pRT->FillEllipse(&trailEllipse, pMotionTrailBrush);
                }
                pMotionTrailBrush->SetOpacity(1.0f);
            }
        }*/
        ...
        // [+] NEW: Hint Move Glow Effect
        if (g_hintActive && b.id == g_hintTargetBallId && b.id != 0) {
            ID2D1SolidColorBrush* pGlow = nullptr;
            pRT->CreateSolidColorBrush(D2D1::ColorF(1.0f, 0.4f, 0.7f, 0.8f), &pGlow); // Pink Glow
            if (pGlow) {
                D2D1_ELLIPSE glowE = D2D1::Ellipse(D2D1::Point2F(b.x, b.y), BALL_RADIUS * 1.8f, BALL_RADIUS * 1.8f);
                pRT->FillEllipse(&glowE, pGlow);
                SafeRelease(&pGlow);
            }
        }
    ...
    SafeRelease(&pMotionTrailBrush); // [+] NEW
}
...
void DrawCueStick(ID2D1RenderTarget* pRT)
{
    ...
    const float buttLen = 110.0f * scale;
    const float jointLen = 4.0f * scale;
    const float shaftLen = 210.0f * scale;
    const float ferruleLen = 8.0f * scale;
    const float tipLen = 4.0f * scale;

    // Tapered Widths (Thickest at bumper, thinnest at tip)
    const float bumperW = 9.5f * scale;
    const float buttW = 9.0f * scale;
    const float jointW = 6.5f * scale;
    const float shaftW = 4.5f * scale;
    const float tipW = 4.5f * scale;
    ...
    // Helper to pinpoint segment joints mathematically outward from the cue ball
    float currentDist = powerOffset + 5.0f; // 5.0f buffer prevents visual clipping with the ball

    auto getPt = [&](float dist) {
        return D2D1::Point2F(cue->x + cosT * dist, cue->y + sinT * dist);
    };

    // Calculate structural points
    D2D1_POINT_2F pTip = getPt(currentDist); currentDist += tipLen;
    D2D1_POINT_2F pFerrule = getPt(currentDist); currentDist += ferruleLen;
    D2D1_POINT_2F pShaft = getPt(currentDist); currentDist += shaftLen;
    D2D1_POINT_2F pJoint = getPt(currentDist); currentDist += jointLen;
    D2D1_POINT_2F pButt = getPt(currentDist); currentDist += buttLen;
    D2D1_POINT_2F pBumper = getPt(currentDist); currentDist += bumperLen;
    D2D1_POINT_2F pBase = getPt(currentDist);

    // Initialize sophisticated color brushes
    ID2D1SolidColorBrush* pBumperBrush = nullptr;
    ID2D1SolidColorBrush* pButtBrush = nullptr;
    ID2D1SolidColorBrush* pJointBrush = nullptr;
    ID2D1SolidColorBrush* pShaftBrush = nullptr;
    ID2D1SolidColorBrush* pFerruleBrush = nullptr;
    ID2D1SolidColorBrush* pTipBrush = nullptr;

    pRT->CreateSolidColorBrush(D2D1::ColorF(0.08f, 0.08f, 0.08f), &pBumperBrush);     // Dark rubber
    pRT->CreateSolidColorBrush(D2D1::ColorF(0.25f, 0.08f, 0.05f), &pButtBrush);       // Elegant Mahogany
    pRT->CreateSolidColorBrush(D2D1::ColorF(0.75f, 0.78f, 0.82f), &pJointBrush);       // Silver metallic collar
    pRT->CreateSolidColorBrush(D2D1::ColorF(0.96f, 0.88f, 0.65f), &pShaftBrush);       // Smooth maple wood
    pRT->CreateSolidColorBrush(D2D1::ColorF(0.98f, 0.98f, 0.95f), &pFerruleBrush);     // Ivory white
    pRT->CreateSolidColorBrush(D2D1::ColorF(0.12f, 0.45f, 0.85f), &pTipBrush);         // Master chalk blue

    // Sophisticated tapered polygon builder
    auto buildRect = [&](D2D1_POINT_2F p1, D2D1_POINT_2F p2, float w1, float w2) {
        D2D1_POINT_2F perp = { -sinT * 0.5f, cosT * 0.5f };
        D2D1_POINT_2F perp1 = { perp.x * w1, perp.y * w1 };
        D2D1_POINT_2F perp2 = { perp.x * w2, perp.y * w2 };

        D2D1_POINT_2F a = { p1.x + perp1.x, p1.y + perp1.y };
        D2D1_POINT_2F b = { p2.x + perp2.x, p2.y + perp2.y };
        D2D1_POINT_2F c = { p2.x - perp2.x, p2.y - perp2.y };
        D2D1_POINT_2F d = { p1.x - perp1.x, p1.y - perp1.y };
    ...
    // [+] FIX: Prominent, High-Fidelity Outerglow Dropshadow
    {
        ID2D1SolidColorBrush* pCueShadowBrush = nullptr;
        // Deep dense black with elevated alpha to stand out beautifully against any table color
        pRT->CreateSolidColorBrush(D2D1::ColorF(0.0f, 0.0f, 0.0f, 0.65f), &pCueShadowBrush);
        if (pCueShadowBrush) {
            const float shadowOffsetX = 6.0f;
            const float shadowOffsetY = 7.0f;
            const float shadowSpread = 2.0f; // Expanding geometry width simulates soft blur 

            auto offsetPt = [&](D2D1_POINT_2F p) { return D2D1::Point2F(p.x + shadowOffsetX, p.y + shadowOffsetY); };

            auto drawShadow = [&](D2D1_POINT_2F p1, D2D1_POINT_2F p2, float w1, float w2) {
                auto geom = buildRect(offsetPt(p1), offsetPt(p2), w1 + shadowSpread, w2 + shadowSpread);
                if (geom) { pRT->FillGeometry(geom, pCueShadowBrush); SafeRelease(&geom); }
            };

            // Draw shadow underlying all segments seamlessly
            drawShadow(pTip, pFerrule, tipW, tipW);
            drawShadow(pFerrule, pShaft, tipW, shaftW);
            drawShadow(pShaft, pJoint, shaftW, jointW);
            drawShadow(pJoint, pButt, jointW, buttW);
            drawShadow(pButt, pBumper, buttW, bumperW);
            drawShadow(pBumper, pBase, bumperW, bumperW);
        }
        SafeRelease(&pCueShadowBrush);
    }

    // Render the physical segments on top of the shadow
    auto drawSegment = [&](D2D1_POINT_2F p1, D2D1_POINT_2F p2, float w1, float w2, ID2D1SolidColorBrush* brush) {
        if (!brush) return;
        auto geom = buildRect(p1, p2, w1, w2);
        if (geom) { pRT->FillGeometry(geom, brush); SafeRelease(&geom); }
    };

    drawSegment(pTip, pFerrule, tipW, tipW, pTipBrush);
    drawSegment(pFerrule, pShaft, tipW, shaftW, pFerruleBrush);
    drawSegment(pShaft, pJoint, shaftW, jointW, pShaftBrush);
    drawSegment(pJoint, pButt, jointW, buttW, pJointBrush);
    drawSegment(pButt, pBumper, buttW, bumperW, pButtBrush);
    drawSegment(pBumper, pBase, bumperW, bumperW, pBumperBrush);

    SafeRelease(&pBumperBrush);
    SafeRelease(&pButtBrush);
    SafeRelease(&pJointBrush);
    SafeRelease(&pShaftBrush);
    SafeRelease(&pFerruleBrush);
...
void DrawAimingAids(ID2D1RenderTarget* pRT) {
    // [+] FIX: Support AI visualization for both P1 and P2 in Demo Mode
    bool isAITurn = g_isDemoMode || (isPlayer2AI && currentPlayer == 2);

    // Determine if aiming aids should be drawn for humans.
    bool isHumanInteracting = (!isAITurn) &&
        (currentGameState == PLAYER1_TURN || currentGameState == PLAYER2_TURN ||
            currentGameState == BREAKING || currentGameState == AIMING ||
    ...
    bool isAiVisualizingShot = (isAITurn && aiIsDisplayingAim);
...
// --- FOOLPROOF FIX: Use the AI's planned angle and power for drawing ---
    ...
    // --- BUG FIX: Account for English Squirt (Deflection) ---
    // English (Spin) alters the initial travel trajectory of the cue ball.
    // We must predict this squirt so ALL aiming aids align perfectly with the actual physics.
    float def_pwr = (powerToDraw > 0.1f) ? powerToDraw : 10.0f;
    float def_sX = isAiVisualizingShot ? aiPlannedShotDetails.spinX : cueSpinX;
    float def_sY = isAiVisualizingShot ? aiPlannedShotDetails.spinY : cueSpinY;

    float rawCosA = cosf(angleToDraw);
    float rawSinA = sinf(angleToDraw);

    float def_Vx = rawCosA * def_pwr + rawCosA * def_sY * 0.5f + rawSinA * def_sX * 0.5f;
    float def_Vy = rawSinA * def_pwr + rawSinA * def_sY * 0.5f - rawCosA * def_sX * 0.5f;

    float trueAimAngle = atan2f(def_Vy, def_Vx);
    // --- End Squirt Fix ---
  ...
  //=========================================================================================
    if (g_debugMode && (shotPower > 0.0f || isAiVisualizingShot)) {
        ...
        for (int step = 0; step < SIM_STEPS; ++step) {
            ...
            // Accurate Kibitzer Physics (Replaces crude bounding box)
            for (auto& b : simBalls) {
                if (b.isPocketed) continue;

                // Accurate pocket check using masks
                bool pocketed = false;
                for (int p = 0; p < 6; ++p) {
                    if (IsBallTouchingPocketMask_D2D(b, p)) {
                        b.isPocketed = true;
                ...
                // Accurate 12-segment rim collision
                D2D1_POINT_2F vertices[12] = {
                    { TABLE_LEFT + INNER_RIM_THICKNESS, TABLE_TOP + CHAMFER_SIZE }, { TABLE_LEFT + CHAMFER_SIZE, TABLE_TOP + INNER_RIM_THICKNESS },
                    { TABLE_RIGHT - CHAMFER_SIZE, TABLE_TOP + INNER_RIM_THICKNESS }, { TABLE_RIGHT - INNER_RIM_THICKNESS, TABLE_TOP + CHAMFER_SIZE },
                    { TABLE_RIGHT - INNER_RIM_THICKNESS, TABLE_BOTTOM - CHAMFER_SIZE }, { TABLE_RIGHT - CHAMFER_SIZE, TABLE_BOTTOM - INNER_RIM_THICKNESS },
                ...
                for (int j = 0; j < 12; ++j) {
                    D2D1_POINT_2F p1 = segments[j][0];
                    D2D1_POINT_2F p2 = segments[j][1];
                    float distSq = PointToLineSegmentDistanceSq(D2D1::Point2F(b.x, b.y), p1, p2);
                    ...
                    if (distSq < BALL_RADIUS * BALL_RADIUS) {
                        ...
                        float dot = b.vx * normalX + b.vy * normalY;
                        if (dot < 0.0f) { // Only bounce if moving into the wall
                            b.vx -= 2 * dot * normalX;
                            b.vy -= 2 * dot * normalY;
...
// --- Projection Line Calculation ---
    float cosA = cosf(trueAimAngle); // --- FIX: Use trueAimAngle to account for English Squirt ---
    float sinA = sinf(trueAimAngle);
    ...
    D2D1_POINT_2F ballCollisionPoint = { 0, 0 };
    D2D1_POINT_2F ghostBallPosForHit = { 0, 0 };

    hitBall = FindFirstHitBall(rayStart, trueAimAngle, firstHitDistSq); // --- FIX: Use trueAimAngle ---
    ...
    if (hitBall) {
        ...
        ghostBallPosForHit = D2D1::Point2F(hitBall->x - cosA * BALL_RADIUS, hitBall->y - sinA * BALL_RADIUS);
    }

    if (!g_isPracticeMode && hitBall && IsWrongTargetForCurrentPlayer(hitBall)) {
        ID2D1SolidColorBrush* pOverlayFill = nullptr;
        ID2D1SolidColorBrush* pOverlayRed = nullptr;
        ID2D1SolidColorBrush* pOverlaySpec = nullptr;
    ...
    D2D1_POINT_2F railHitPoint = rayEnd;
    ...
    // BOTTOM inner rim
    {
        float midX = pocketPositions[4].x;
    ...
    if (railHitPoint.x == rayEnd.x && railHitPoint.y == rayEnd.y) {
        D2D1_POINT_2F edgeA[4] = { { TABLE_LEFT,  TABLE_TOP }, { TABLE_RIGHT, TABLE_TOP }, { TABLE_RIGHT, TABLE_BOTTOM }, { TABLE_LEFT,  TABLE_BOTTOM } };
        D2D1_POINT_2F edgeB[4] = { { TABLE_RIGHT, TABLE_TOP }, { TABLE_RIGHT, TABLE_BOTTOM }, { TABLE_LEFT,  TABLE_BOTTOM }, { TABLE_LEFT,  TABLE_TOP } };
    ...
    D2D1_POINT_2F finalLineEnd = railHitPoint;
    bool aimingAtRail = true;

    if (hitBall && firstHitDistSq < minRailDistSq) {
        finalLineEnd = ballCollisionPoint;
        aimingAtRail = false;
    }
    ...
    if (pFactory)
    {
        ...
        float chv_cosA = cosf(trueAimAngle); // --- FIX: Use trueAimAngle ---
        float chv_sinA = sinf(trueAimAngle);
        float chv_hitDistSq = -1.0f;
        Ball* chv_ball = FindFirstHitBall(
            D2D1::Point2F(cueBall->x, cueBall->y),
            trueAimAngle, // --- FIX ---
            chv_hitDistSq);
        ...
        if (chv_ball && chv_hitDistSq >= 0.0f)
        {
            ...
            float chv_dSq = chv_relX * chv_relX + chv_relY * chv_relY;
            float chv_2r = 2.0f * BALL_RADIUS;
            float chv_disc = chv_projFwd * chv_projFwd - chv_dSq + chv_2r * chv_2r;
            if (chv_disc < 0.0f) chv_disc = 0.0f;
            float chv_t = chv_projFwd - sqrtf(chv_disc); // dist to true ghost
            float chv_ghostX = cueBall->x + chv_cosA * chv_t;
            float chv_ghostY = cueBall->y + chv_sinA * chv_t;

            // Line-of-centres: from true ghost to target centre
            float chv_dx = chv_ball->x - chv_ghostX;
            float chv_dy = chv_ball->y - chv_ghostY;
            float chv_len = sqrtf(chv_dx * chv_dx + chv_dy * chv_dy);
            if (chv_len < 0.0001f) chv_len = 1.0f;
            float chv_nx = chv_dx / chv_len;
            float chv_ny = chv_dy / chv_len;

            // --- Cue-ball velocity at impact (spin factored in) ---------------
            // --- BUG FIX: Reuse accurately computed squirt velocity directly ---
            float chv_cVx = def_Vx;
            float chv_cVy = def_Vy;
            float chv_sX = def_sX; // Keep for English throw
            float chv_sY = def_sY;

            // --- Object-ball post-impact velocity (elastic + English throw) ---
            float chv_vDotN = chv_cVx * chv_nx + chv_cVy * chv_ny;
            float chv_tVx = chv_nx; // fallback: straight line-of-centres
            float chv_tVy = chv_ny;
            if (chv_vDotN > 0.0f) {
                chv_tVx = chv_vDotN * chv_nx;
                chv_tVy = chv_vDotN * chv_ny;
                // English throw — identical constant to CheckCollisions
                const float kSpin = 0.08f;
                chv_tVx -= (chv_sY * chv_ny - chv_sX * chv_nx) * kSpin;
                chv_tVy -= (chv_sY * chv_nx + chv_sX * chv_ny) * kSpin;
            }

            float chv_tLen = sqrtf(chv_tVx * chv_tVx + chv_tVy * chv_tVy);
            if (chv_tLen < 0.0001f) chv_tLen = 1.0f;
            float chv_dirX = chv_tVx / chv_tLen;
            float chv_dirY = chv_tVy / chv_tLen;

            // --- Raycast: clip to nearest rail then nearest pocket mouth ------
            D2D1_POINT_2F chv_start = { chv_ball->x, chv_ball->y };
            float chv_maxD = 2000.0f;
            D2D1_POINT_2F chv_far = { chv_start.x + chv_dirX * chv_maxD,
                                        chv_start.y + chv_dirY * chv_maxD };
            D2D1_POINT_2F chv_railPt;
            if (FindFirstRailIntersection(chv_start, chv_far, chv_railPt))
                chv_maxD = GetDistance(chv_start.x, chv_start.y,
                    chv_railPt.x, chv_railPt.y);

            for (int p = 0; p < 6; ++p) {
                float bx = pocketPositions[p].x - chv_start.x;
                float by = pocketPositions[p].y - chv_start.y;
                float proj = bx * chv_dirX + by * chv_dirY;
                if (proj > 0.0f && proj < chv_maxD) {
                    float cx = chv_start.x + proj * chv_dirX;
                    float cy = chv_start.y + proj * chv_dirY;
                    float r = GetPocketVisualRadius(p);
                    if (GetDistanceSq(pocketPositions[p].x, pocketPositions[p].y,
                        cx, cy) < r * r)
                        chv_maxD = std::max(0.0f, std::min(chv_maxD, proj));
                }
            }
...
void DrawUI(ID2D1RenderTarget* pRT) {
    ...
    float uiHeight = 60;
    // [+] FIX: Shift the name rectangles inwards by 50 pixels to make room for Sprites and Arrows
    float p1Left = TABLE_LEFT + 50.0f;
    float p1Width = 150;
    float p2Left = TABLE_RIGHT - p1Width - 50.0f;
    ...
    if (g_isPracticeMode) {
        ...
        if (currentGameType == GameType::STRAIGHT_POOL) {
            oss << L"\nScore: " << player1StraightPoolScore;
        }
        D2D1_RECT_F practiceRect = D2D1::RectF(TABLE_LEFT, uiTop, TABLE_RIGHT, uiTop + uiHeight);

        // [+] FIX: Center alignment correction for Practice Mode Sprite
        if (g_p1SpriteIndex >= 0 && g_p1SpriteIndex < 24 && g_pSpriteBitmaps[g_p1SpriteIndex]) {
            float cx = TABLE_LEFT + TABLE_WIDTH / 2.0f;
            // [+] FIX: Nudged further left (-180.0f) to prevent overlap with the text
            D2D1_RECT_F sRect = D2D1::RectF(cx - 180.0f, uiTop + 10.0f, cx - 180.0f + 42.0f, uiTop + 44.0f);
            pRT->DrawBitmap(g_pSpriteBitmaps[g_p1SpriteIndex], sRect);
        }
        ...
        // [+] FIX: Position Sprites precisely between the Turn Arrow and the Player Names
        if (g_p1SpriteIndex >= 0 && g_p1SpriteIndex < 24 && g_pSpriteBitmaps[g_p1SpriteIndex]) {
            D2D1_RECT_F sRect1 = D2D1::RectF(p1Rect.left - 48.0f, p1Rect.top + 10.0f, p1Rect.left - 6.0f, p1Rect.top + 44.0f);
            pRT->DrawBitmap(g_pSpriteBitmaps[g_p1SpriteIndex], sRect1);
        }

        if (g_p2SpriteIndex >= 0 && g_p2SpriteIndex < 24 && g_pSpriteBitmaps[g_p2SpriteIndex]) {
            D2D1_RECT_F sRect2 = D2D1::RectF(p2Rect.right + 6.0f, p2Rect.top + 10.0f, p2Rect.right + 48.0f, p2Rect.top + 44.0f);
            pRT->DrawBitmap(g_pSpriteBitmaps[g_p2SpriteIndex], sRect2);
        }
        ...
        // --- Current Turn Arrow (Blue, Bigger, Beside Name) ---
        ID2D1SolidColorBrush* pArrowBrush = nullptr;
        pRT->CreateSolidColorBrush(TURN_ARROW_COLOR, &pArrowBrush);

        // [+] FIX: Removed "currentGameState != AI_THINKING" so the arrow stays on the player during AI computation
        if (pArrowBrush && currentGameState != GAME_OVER && currentGameState != SHOT_IN_PROGRESS) {
            float arrowSizeBase = 32.0f;
            float arrowCenterY = p1Rect.top + uiHeight / 2.0f;
            float arrowTipX, arrowBackX;

            D2D1_RECT_F playerBox = (currentPlayer == 1) ? p1Rect : p2Rect;

            // [+] FIX: Push the Arrow out to the far-left extreme so it points to the Sprite.
            // (For both players, the arrow sits on the left side pointing right, mirroring Yahoo Pool's style).
            arrowBackX = playerBox.left - 85.0f;
            arrowTipX = arrowBackX + arrowSizeBase * 0.75f;

            float notchDepth = 12.0f;
            float notchWidth = 10.0f;

            float cx = arrowBackX;
            float cy = arrowCenterY;

            D2D1_POINT_2F tip = D2D1::Point2F(arrowTipX, cy);
            D2D1_POINT_2F baseTop = D2D1::Point2F(cx, cy - arrowSizeBase / 2.0f);
            D2D1_POINT_2F baseBot = D2D1::Point2F(cx, cy + arrowSizeBase / 2.0f);

            D2D1_POINT_2F r1 = D2D1::Point2F(cx - notchDepth, cy - notchWidth / 2.0f);
            D2D1_POINT_2F r2 = D2D1::Point2F(cx, cy - notchWidth / 2.0f);
            D2D1_POINT_2F r3 = D2D1::Point2F(cx, cy + notchWidth / 2.0f);
            D2D1_POINT_2F r4 = D2D1::Point2F(cx - notchDepth, cy + notchWidth / 2.0f);
...
// red color and large font; only the text content differs.
    ...
    if (currentGameType == GameType::EIGHT_BALL_MODE && !g_isPracticeMode) {
        ...
        bool isAITurn = g_isDemoMode || (isPlayer2AI && currentPlayer == 2);
        int currentCalledPocket = (currentPlayer == 1) ? calledPocketP1 : calledPocketP2;

        bool aiCalled = (isAITurn && IsPlayerOnEightBall(currentPlayer) && currentCalledPocket >= 0);
        bool humanChoosingOrCalled = (!isAITurn) &&
            (currentGameState == CHOOSING_POCKET_P1 || currentGameState == CHOOSING_POCKET_P2 ||
                calledPocketP1 >= 0 || calledPocketP2 >= 0);

        if (aiCalled || humanChoosingOrCalled) {
            int idx = -1;
            if (aiCalled) idx = currentCalledPocket;
            else {
            ...
            pRT->CreateSolidColorBrush(TURN_ARROW_COLOR, &pPocketArrowBrush);
            ...
            if (aiCalled) promptText = L"AI has called this pocket";
            else if (currentGameState == CHOOSING_POCKET_P1 || currentGameState == CHOOSING_POCKET_P2) promptText = L"Choose pocket for 8-Ball...";
            else if (calledPocketP1 >= 0 || calledPocketP2 >= 0) promptText = L"Pocket called for 8-Ball";
    ...
    // [+] FIX: Show AI Thinking State dynamically on whichever player is currently controlled by AI
    if (!g_isPracticeMode && currentGameState == AI_THINKING && pTextFormat) {
        ID2D1SolidColorBrush* pThinkingBrush = nullptr;
        pRT->CreateSolidColorBrush(D2D1::ColorF(D2D1::ColorF::Orange), &pThinkingBrush);
        if (pThinkingBrush) {
            D2D1_RECT_F thinkingRect = (currentPlayer == 1) ? p1Rect : p2Rect;
            thinkingRect.top += 20;
            pTextFormat->SetTextAlignment(DWRITE_TEXT_ALIGNMENT_CENTER);
            pTextFormat->SetParagraphAlignment(DWRITE_PARAGRAPH_ALIGNMENT_CENTER);
...
void DrawPowerMeter(ID2D1RenderTarget* pRT) {
    ID2D1SolidColorBrush* pBorderBrush = nullptr;
    ID2D1SolidColorBrush* pBackgroundBrush = nullptr;     // [+] NEW: cobalt blue background
    ...
    // [+] NEW: Fill entire meter with semi-transparent dark cobalt blue (#07050d)
    pRT->CreateSolidColorBrush(
        D2D1::ColorF(7.0f / 255.0f, 5.0f / 255.0f, 13.0f / 255.0f, 0.72f),
        &pBackgroundBrush
    );
    if (pBackgroundBrush) {
        pRT->FillRectangle(&powerMeterRect, pBackgroundBrush);
    }
    ...
    // [+] FIX: Correctly identify if the AI is actively visualizing power in Demo Mode
    float fillRatio = 0;
    bool humanIsAimingPower = isAiming && (currentGameState == AIMING || currentGameState == BREAKING);
    bool isAITurn = g_isDemoMode || (isPlayer2AI && currentPlayer == 2);
    bool aiIsVisualizingPower = (isAITurn && currentGameState == AI_THINKING && aiIsDisplayingAim);

    if (humanIsAimingPower || aiIsVisualizingPower) {
    ...
    // [+] NEW: Gray segment notches (blocks) dividing the power meter into sections
    {
        ID2D1SolidColorBrush* pGraySegmentBrush = nullptr;
        pRT->CreateSolidColorBrush(D2D1::ColorF(D2D1::ColorF::DarkGray/*, 0.55f*/), &pGraySegmentBrush);
        if (pGraySegmentBrush) {
            const int NUM_SEGMENTS = 10;
            const float notchHeight = 2.0f;
            for (int i = 1; i < NUM_SEGMENTS; ++i) {
                float y = powerMeterRect.top + (powerMeterRect.bottom - powerMeterRect.top) * ((float)i / NUM_SEGMENTS);
                D2D1_RECT_F notchRect = D2D1::RectF(
                    powerMeterRect.left,
                    y - notchHeight / 2.0f,
                    powerMeterRect.right,
                    y + notchHeight / 2.0f
                );
                pRT->FillRectangle(&notchRect, pGraySegmentBrush);
            }
            SafeRelease(&pGraySegmentBrush);
        }
    }
    ...
    SafeRelease(&pBorderBrush);
    SafeRelease(&pBackgroundBrush);    // [+] NEW
...
void DrawSpinIndicator(ID2D1RenderTarget* pRT) {
    ...
    ID2D1GradientStopCollection* pStops = nullptr;
    ID2D1RadialGradientBrush* pSphereBrush = nullptr;
    ID2D1SolidColorBrush* pShadowBrush = nullptr;
    ID2D1SolidColorBrush* pRedBrush = nullptr;
    ID2D1SolidColorBrush* pCrosshairBrush = nullptr;
    ID2D1SolidColorBrush* pDotOutlineBrush = nullptr;
    ID2D1SolidColorBrush* pTextBrush = nullptr;

    // 1. Draw a prominent drop shadow under the 3D sphere
    pRT->CreateSolidColorBrush(D2D1::ColorF(0.0f, 0.0f, 0.0f, 0.4f), &pShadowBrush);
    if (pShadowBrush) {
        D2D1_ELLIPSE shadowEllipse = D2D1::Ellipse(
            D2D1::Point2F(spinIndicatorCenter.x + 3.0f, spinIndicatorCenter.y + 3.0f),
            spinIndicatorRadius * 1.05f, spinIndicatorRadius * 1.05f);
        pRT->FillEllipse(&shadowEllipse, pShadowBrush);
    }

    // 2. Create the 3D Radial Gradient for the Sphere
    D2D1_GRADIENT_STOP gs[3];
    gs[0].position = 0.0f;  gs[0].color = D2D1::ColorF(1.0f, 1.0f, 1.0f, 1.0f);   // Bright highlight
    gs[1].position = 0.5f;  gs[1].color = D2D1::ColorF(0.85f, 0.88f, 0.85f, 1.0f); // Slightly off-white
    gs[2].position = 1.0f;  gs[2].color = D2D1::ColorF(0.5f, 0.5f, 0.5f, 1.0f);   // Darker grey edge

    if (SUCCEEDED(pRT->CreateGradientStopCollection(gs, 3, &pStops))) {
        D2D1_RADIAL_GRADIENT_BRUSH_PROPERTIES props = D2D1::RadialGradientBrushProperties(
            D2D1::Point2F(spinIndicatorCenter.x - spinIndicatorRadius * 0.35f, spinIndicatorCenter.y - spinIndicatorRadius * 0.35f), // Offset highlight for 3D light source
            D2D1::Point2F(0, 0),
            spinIndicatorRadius * 1.3f, spinIndicatorRadius * 1.3f);
        pRT->CreateRadialGradientBrush(props, pStops, &pSphereBrush);
    }

    // Draw the 3D Sphere
    D2D1_ELLIPSE bgEllipse = D2D1::Ellipse(spinIndicatorCenter, spinIndicatorRadius, spinIndicatorRadius);
    if (pSphereBrush) pRT->FillEllipse(&bgEllipse, pSphereBrush);

    // 3. Draw Subdued Crosshairs
    pRT->CreateSolidColorBrush(D2D1::ColorF(D2D1::ColorF::Black, 0.35f), &pCrosshairBrush);
    if (pCrosshairBrush) {
        pRT->DrawLine(
            D2D1::Point2F(spinIndicatorCenter.x - spinIndicatorRadius + 2.0f, spinIndicatorCenter.y),
            D2D1::Point2F(spinIndicatorCenter.x + spinIndicatorRadius - 2.0f, spinIndicatorCenter.y),
            pCrosshairBrush, 1.0f);
        pRT->DrawLine(
            D2D1::Point2F(spinIndicatorCenter.x, spinIndicatorCenter.y - spinIndicatorRadius + 2.0f),
            D2D1::Point2F(spinIndicatorCenter.x, spinIndicatorCenter.y + spinIndicatorRadius - 2.0f),
            pCrosshairBrush, 1.0f);
    }

    // 4. Draw the actual Spin Dot
    pRT->CreateSolidColorBrush(ENGLISH_DOT_COLOR, &pRedBrush);
    pRT->CreateSolidColorBrush(D2D1::ColorF(D2D1::ColorF::Black), &pDotOutlineBrush);
    if (pRedBrush && pDotOutlineBrush) {
        float dotRadius = 4.0f;
        float dotX = spinIndicatorCenter.x + cueSpinX * (spinIndicatorRadius - dotRadius);
        ...
        pRT->DrawEllipse(&dotEllipse, pDotOutlineBrush, 1.0f);
    }

    // 5. Draw "English" Text (Yahoo Clone Style)
    pRT->CreateSolidColorBrush(D2D1::ColorF(D2D1::ColorF::Black), &pTextBrush);
    if (pTextBrush && pBallNumFormat) {
        D2D1_RECT_F textRect = D2D1::RectF(
            spinIndicatorCenter.x - 40.0f,
            spinIndicatorCenter.y + spinIndicatorRadius + 3.0f,
            spinIndicatorCenter.x + 40.0f,
            spinIndicatorCenter.y + spinIndicatorRadius + 20.0f
        );
        pRT->DrawTextW(L"English", 7, pBallNumFormat, &textRect, pTextBrush);
    }

    // Cleanup
    SafeRelease(&pStops);
    SafeRelease(&pSphereBrush);
    SafeRelease(&pShadowBrush);
    SafeRelease(&pRedBrush);
    SafeRelease(&pCrosshairBrush);
    SafeRelease(&pDotOutlineBrush);
    SafeRelease(&pTextBrush);
}

// [+] NEW: Draws a large, centered 3D English Spin Indicator popup overlay.
// Stylized identically to DrawSpinIndicator (3D radial-gradient sphere,
// subdued grey crosshairs, red spin dot, "English" label) so the visual
// language matches the small top-left indicator. Reads cueSpinX/cueSpinY
// every frame so it updates in real time as the user drags. Gated on
// g_spinPopupLingerCounter > 0 (set by LBUTTONDOWN, refreshed by
// WM_MOUSEMOVE during drag, set to ~90 on LBUTTONUP for post-release
// linger, and zeroed by right-click on the spin indicator / new game).
void DrawSpinPopup(ID2D1RenderTarget* pRT) {
    if (!pRT) return;

    // 1. Place the popup at the visual centre of the client window.  Use
    //    GetSize() rather than globals so it works after window resize.
    D2D1_SIZE_F rtSize = pRT->GetSize();
    const float popupRadius = 90.0f;  // ~3x the small indicator's radius
    D2D1_POINT_2F popupCenter = { rtSize.width / 2.0f, rtSize.height / 2.0f };
    ...
    ID2D1SolidColorBrush* pDotOutlineBrush = nullptr;
    ID2D1SolidColorBrush* pTextBrush = nullptr;
    ID2D1SolidColorBrush* pLabelBgBrush = nullptr;

    // 2. Translucent dimming panel behind the popup so the indicator pops
    //    visually even on busy areas of the table.
    pRT->CreateSolidColorBrush(D2D1::ColorF(0.0f, 0.0f, 0.0f, 0.35f), &pLabelBgBrush);
    if (pLabelBgBrush) {
        D2D1_RECT_F dimRect = D2D1::RectF(
            popupCenter.x - popupRadius - 25.0f,
            popupCenter.y - popupRadius - 25.0f,
            popupCenter.x + popupRadius + 25.0f,
            popupCenter.y + popupRadius + 25.0f
        );
        pRT->FillRectangle(&dimRect, pLabelBgBrush);
    }

    // 3. Drop shadow under the 3D sphere.
    pRT->CreateSolidColorBrush(D2D1::ColorF(0.0f, 0.0f, 0.0f, 0.5f), &pShadowBrush);
    if (pShadowBrush) {
        D2D1_ELLIPSE shadowEllipse = D2D1::Ellipse(
            D2D1::Point2F(popupCenter.x + 5.0f, popupCenter.y + 5.0f),
            popupRadius * 1.05f, popupRadius * 1.05f);
        pRT->FillEllipse(&shadowEllipse, pShadowBrush);
    }

    // 4. 3D radial-gradient sphere (same gradient stops as the small one).
    D2D1_GRADIENT_STOP gs[3];
    gs[0].position = 0.0f;  gs[0].color = D2D1::ColorF(1.0f, 1.0f, 1.0f, 1.0f);
    gs[1].position = 0.5f;  gs[1].color = D2D1::ColorF(0.85f, 0.88f, 0.85f, 1.0f);
    gs[2].position = 1.0f;  gs[2].color = D2D1::ColorF(0.5f, 0.5f, 0.5f, 1.0f);

    if (SUCCEEDED(pRT->CreateGradientStopCollection(gs, 3, &pStops))) {
        D2D1_RADIAL_GRADIENT_BRUSH_PROPERTIES props = D2D1::RadialGradientBrushProperties(
            D2D1::Point2F(popupCenter.x - popupRadius * 0.35f, popupCenter.y - popupRadius * 0.35f),
            D2D1::Point2F(0, 0),
            popupRadius * 1.3f, popupRadius * 1.3f);
        pRT->CreateRadialGradientBrush(props, pStops, &pSphereBrush);
    }

    D2D1_ELLIPSE bgEllipse = D2D1::Ellipse(popupCenter, popupRadius, popupRadius);
    if (pSphereBrush) pRT->FillEllipse(&bgEllipse, pSphereBrush);

    // 5. Subdued grey crosshairs (scaled up).
    pRT->CreateSolidColorBrush(D2D1::ColorF(D2D1::ColorF::Black, 0.35f), &pCrosshairBrush);
    if (pCrosshairBrush) {
        pRT->DrawLine(
            D2D1::Point2F(popupCenter.x - popupRadius + 4.0f, popupCenter.y),
            D2D1::Point2F(popupCenter.x + popupRadius - 4.0f, popupCenter.y),
            pCrosshairBrush, 1.5f);
        pRT->DrawLine(
            D2D1::Point2F(popupCenter.x, popupCenter.y - popupRadius + 4.0f),
            D2D1::Point2F(popupCenter.x, popupCenter.y + popupRadius - 4.0f),
            pCrosshairBrush, 1.5f);
    }

    // 6. Spin dot — scaled up, driven by the same cueSpinX/cueSpinY the
    //    small indicator uses, so the popup mirrors the small dot in real
    //    time as the user drags.
    pRT->CreateSolidColorBrush(ENGLISH_DOT_COLOR, &pRedBrush);
    pRT->CreateSolidColorBrush(D2D1::ColorF(D2D1::ColorF::Black), &pDotOutlineBrush);
    if (pRedBrush && pDotOutlineBrush) {
        float dotRadius = 8.0f;
        float dotX = popupCenter.x + cueSpinX * (popupRadius - dotRadius);
        float dotY = popupCenter.y + cueSpinY * (popupRadius - dotRadius);
        D2D1_ELLIPSE dotEllipse = D2D1::Ellipse(D2D1::Point2F(dotX, dotY), dotRadius, dotRadius);
        pRT->FillEllipse(&dotEllipse, pRedBrush);
        pRT->DrawEllipse(&dotEllipse, pDotOutlineBrush, 1.5f);
    }

    // 7. "English" label below the popup (bold for readability at this size).
    pRT->CreateSolidColorBrush(D2D1::ColorF(D2D1::ColorF::Black), &pTextBrush);
    if (pTextBrush && pTextFormatBold) {
        D2D1_RECT_F textRect = D2D1::RectF(
            popupCenter.x - 100.0f,
            popupCenter.y + popupRadius + 8.0f,
            popupCenter.x + 100.0f,
            popupCenter.y + popupRadius + 36.0f
        );
        pRT->DrawTextW(L"English", 7, pTextFormatBold, &textRect, pTextBrush);
    }

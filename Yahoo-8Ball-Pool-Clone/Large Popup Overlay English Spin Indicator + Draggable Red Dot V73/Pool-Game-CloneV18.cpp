...
INT_PTR CALLBACK NewGameDialogProc(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam) {
    ...
    auto UpdateControlStates = [hDlg]() {
        ...
        if (isDemo) EnableWindow(GetDlgItem(hDlg, IDC_CHECK_PRACTICE), FALSE);
        else EnableWindow(GetDlgItem(hDlg, IDC_CHECK_PRACTICE), TRUE);

        if (isPractice) EnableWindow(GetDlgItem(hDlg, IDC_CHECK_DEMO), FALSE);
        else EnableWindow(GetDlgItem(hDlg, IDC_CHECK_DEMO), TRUE);

        // Disable Player Type config if Practice or Demo
        ...
        bool enableAIOptions = (isCPUGame || isDemo) && !isPractice;
    ...
    { // Add an opening brace to create a new scope for the entire case.
        ...
        case IDOK:
            ...
            if ((IsDlgButtonChecked(hDlg, IDC_RADIO_CPU) == BST_CHECKED || g_isDemoMode) && !g_isPracticeMode) {

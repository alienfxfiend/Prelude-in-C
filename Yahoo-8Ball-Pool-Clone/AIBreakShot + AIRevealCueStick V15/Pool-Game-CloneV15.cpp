...
void AIBreakShot() {
    ...
    // 1. Check if the break shot is already planned or needs setup
...
void AIMakeDecision() {
    ...
    // --- NEW: Handle CPU's opening break shot ---
    /*if (isOpeningBreakShot && currentGameState == PRE_BREAK_PLACEMENT && isPlayer2AI && currentPlayer == 2) {
        Ball* cueBall = GetCueBall();
        if (cueBall) {
            ...
            float randX = kitchenMinX + static_cast<float>(rand()) / (static_cast<float>(RAND_MAX) / (kitchenMaxX - kitchenMinX));
            float randY = kitchenMinY + static_cast<float>(rand()) / (static_cast<float>(RAND_MAX) / (kitchenMaxY - kitchenMinY));

            cueBall->x = randX;
            cueBall->y = randY;
            cueBall->vx = cueBall->vy = 0.0f;

            float rackMinX = RACK_POS_X - BALL_RADIUS * 2;
            float rackMaxX = RACK_POS_X + BALL_RADIUS * 2;
            float rackMinY = RACK_POS_Y - BALL_RADIUS * 2;
            float rackMaxY = RACK_POS_Y + BALL_RADIUS * 2;

            float targetX = rackMinX + static_cast<float>(rand()) / (static_cast<float>(RAND_MAX) / (rackMaxX - rackMinX));
            float targetY = rackMinY + static_cast<float>(rand()) / (static_cast<float>(RAND_MAX) / (rackMaxY - rackMinY));

            float dx = targetX - cueBall->x;
            float dy = targetY - cueBall->y;

            float shotAngle = atan2f(dy, dx);
            float shotPower = MAX_SHOT_POWER;

            // Store shot details in your AI plan
            aiPlannedShotDetails.angle = shotAngle;
            aiPlannedShotDetails.power = shotPower;
            aiPlannedShotDetails.spinX = 0.0f;
            aiPlannedShotDetails.spinY = 0.0f;
            aiPlannedShotDetails.isValid = true;

            // Set visual flags so draw function shows cue stick
            isAiming = true;
            aiIsDisplayingAim = true;
            shotPower = bestShot.power;
            cueAngle = bestShot.angle;
            ...
            std::thread([](const TCHAR* soundName) {
                ...
                // Execute shot immediately
                ApplyShot(shotPower, shotAngle, 0.0f, 0.0f);
                currentGameState = SHOT_IN_PROGRESS;
                isOpeningBreakShot = false; // Reset flag
                return; // Exit to prevent further AI decision
        }
    }*/

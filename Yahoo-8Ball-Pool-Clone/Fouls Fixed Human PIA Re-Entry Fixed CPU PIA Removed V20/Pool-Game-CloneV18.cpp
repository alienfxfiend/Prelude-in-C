...
const D2D1_POINT_2F pocketPositions[6] = {
    {TABLE_LEFT, TABLE_TOP},                           // 0: Top-Left
    {TABLE_LEFT + TABLE_WIDTH / 2.0f, TABLE_TOP},      // 1: Top-Middle
    {TABLE_RIGHT, TABLE_TOP},                          // : Top-Right
    {TABLE_LEFT, TABLE_BOTTOM},                        // 3: Bottom-Left
    {TABLE_LEFT + TABLE_WIDTH / 2.0f, TABLE_BOTTOM},   // 2,4: Bottom-Middle
    {TABLE_RIGHT, TABLE_BOTTOM}                        // 5: Bottom-Right
};
...
bool CheckAndTransitionToPocketChoice(int playerID); //orig void
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_LBUTTONDOWN: {
        ...
        // --- FIX: This block handles re-selectable pocket choice ---
        if ((currentGameState == CHOOSING_POCKET_P1 && currentPlayer == 1) ||
            (currentGameState == CHOOSING_POCKET_P2 && currentPlayer == 2 && !isPlayer2AI)) {
            ...
            return 0; // If they click anywhere else, do nothing.
        }
                    ...
                    if (dx * dx + dy * dy <= POCKET_RADIUS * POCKET_RADIUS) {
                        ...
                        if (b->id != 8) {
                            ...
                            if (shooter.ballsPocketedCount >= 7
                                && calledPocketP1 < 0
                                && calledPocketP2 < 0
                                && !isPlayer2AI)    // <-- skip this entirely for the AI
                            {
                                // default to bottom?right pocket (index 5)
                                if (currentPlayer == 1)    calledPocketP1 = 2;
                                else                       calledPocketP2 = 2;
...
void ProcessShotResults() {
    ...
    if (ownBallsPocketedThisTurn > 0) {
        ...
        if (shooter.ballsPocketedCount >= 7 && !eightBallPocketed) {
            // --- If AI, skip pocket?call and proceed normally ---
            if (isPlayer2AI && currentPlayer == 2) {
                // Let AI aim immediately without hanging on CHOOSING_POCKET
                currentGameState = PLAYER2_TURN;
                aiTurnPending = true;
            }
            else {
                // Human: default arrow to bottom?right (index?5)
                if (currentPlayer == 1) calledPocketP1 = 2;
                else                    calledPocketP2 = 2;
                // Enter pocket?call state so arrow shows immediately
                currentGameState = (currentPlayer == 1)
                    ? CHOOSING_POCKET_P1
                    : CHOOSING_POCKET_P2;
            }
            return; // suspend further turn logic
        }
    }
    ...
    /*// --- RE?ENTER POCKET?CALL if still on 8?ball and missed it ---
    // If it's now this player's turn *again*, and they've already called
    // pocket #5, but still haven't sunk the 8?ball, re?show the arrow.
    if (!foulCommitted              // no foul hanging this turn
        && !eightBallPocketed       // still haven’t sunk the 8?ball
        && ((currentPlayer == 1 && calledPocketP1 == 3)
            || (currentPlayer == 2 && calledPocketP2 == 3))
        && currentGameState != GAME_OVER)
    {
        // Put them back into the choosing?pocket state so the arrow reappears
        currentGameState = (currentPlayer == 1)
            ? CHOOSING_POCKET_P1
            : CHOOSING_POCKET_P2;
    }*/
...
// This function now correctly handles resetting and re-engaging the pocket choice state.
void SwitchTurns() {
    // Reset shot-specific foul tracking for the new turn
    foulCommitted = false;
    firstHitBallIdThisShot = -1;
    cueHitObjectBallThisShot = false;
    railHitAfterContact = false;
    ...
    // Store the player whose turn is ending
    int outgoingPlayer = currentPlayer;

    // Switch to the next player
    if (currentPlayer == 1) {
        currentPlayer = 2;
    }
    else { // currentPlayer was 2
        currentPlayer = 1;
    }

    // If the outgoing player was on the 8-ball, reset their called pocket.
    // This is the key to allowing them to re-call it if the turn comes back to them.
    if (IsPlayerOnEightBall(outgoingPlayer)) {
        if (outgoingPlayer == 1) {
            calledPocketP1 = -1;
        }
        else { // outgoingPlayer is 2
            calledPocketP2 = -1;
        }
    }

    // Now, check if the INCOMING player needs to call a pocket.
    // If CheckAndTransitionToPocketChoice changes the state, we can just return.
    if (CheckAndTransitionToPocketChoice(currentPlayer)) {
        return; // The state is now CHOOSING_POCKET_P*, so we're done.
    }

    // If we are not choosing a pocket, proceed with normal turn state assignment.
...
// This function now returns a boolean and sets a default pocket.
// It returns true if the state was changed to CHOOSING_POCKET, and false otherwise.
bool CheckAndTransitionToPocketChoice(int playerID) {
    if (IsPlayerOnEightBall(playerID)) {
        if (playerID == 1 && calledPocketP1 == -1) {
            currentGameState = CHOOSING_POCKET_P1;
            pocketCallMessage = player1Info.name + L": Call your pocket for the 8-Ball.";
            calledPocketP1 = 2; // Default to Pocket 5 (index 4 is Bottom-Middle)
            return true;
        }
        else if (playerID == 2 && calledPocketP2 == -1) {
            currentGameState = CHOOSING_POCKET_P2;
            pocketCallMessage = player2Info.name + L": Call your pocket for the 8-Ball.";
            calledPocketP2 = 2; // Default to Pocket 5 (index 4 is Bottom-Middle)
            return true;
        }
    }
    return false;
...
void CheckAndTransitionToPocketChoice(int playerID) {
    ...
    if (needsToCall && *calledPocketForPlayer == -1) { // Only transition if a pocket hasn't been called yet
        ...
        if (playerID == 1) {
            ...
            if (isPlayer2AI) {
                // --- CPU chooses a random pocket immediately ---
                calledPocketP2 = rand() % 6;
                pocketCallMessage.clear();
                // go straight back to aiming state
                currentGameState = PLAYER2_TURN;
            }
            else {
                currentGameState = CHOOSING_POCKET_P2;
...
if (SUCCEEDED(pFactory->CreatePathGeometry(&pPath))) {
    ...
    SafeRelease(&pPath);
        ...
        */
        ...
        // --- Persistent Blue Arrow & Prompt for 8?Ball Call (human only, auto?default to pocket 5) ---
    if (!isPlayer2AI && (calledPocketP1 >= 0 || calledPocketP2 >= 0))
    {
        // determine index: use the stored call, but default to bottom?right if somehow unset
        int idx = (currentPlayer == 1 ? calledPocketP1 : calledPocketP2);
        if (idx < 0) idx = 5;  // bottom?right

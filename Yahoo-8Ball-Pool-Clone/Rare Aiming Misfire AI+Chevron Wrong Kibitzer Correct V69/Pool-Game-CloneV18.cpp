...
// Helper to play sound directly from embedded WAVE resource
void PlayGameSound(int resourceId) {
    if (g_soundEffectsMuted) return;
    // SND_RESOURCE allows PlaySound to grab the WAV directly out of our compiled EXE!
...
LRESULT CALLBACK SplashWndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {
    switch (msg) {
    case WM_CREATE:
        // Set a timer for 7000 milliseconds (7 seconds)
    ...
    case WM_TIMER:       // Automatically close after 7 seconds
        KillTimer(hwnd, 1);

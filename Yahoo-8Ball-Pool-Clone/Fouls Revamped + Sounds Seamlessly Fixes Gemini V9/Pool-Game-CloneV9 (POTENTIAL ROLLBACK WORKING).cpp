...
LRESULT CALLBACK WndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_LBUTTONUP: {
        ...
        if ((isAiming || isDraggingStick) && (currentGameState == AIMING || currentGameState == BREAKING)) {
            ...
            if (shotPower > 0.15f) { // Check power threshold
                if (currentGameState != AI_THINKING) {
                    // --- ACTION: Reset foul flags BEFORE applying shot ---
        ...
        if (isDraggingCueBall) {
            ...
            if (cueBall) {
                ...
                if (IsValidCueBallPosition(cueBall->x, cueBall->y, behindHeadstring)) {
                    ...
                    cueAngle = 0.0f;
                } // else: Invalid placement, state remains BALL_IN_HAND...
            }
        }
...
void RespawnCueBall(bool behindHeadstring) { // 'behindHeadstring' only relevant for initial break placement
    ...
    if (cueBall) {
        ...
        // 'currentPlayer' already reflects who's turn it is NOW
        if (currentPlayer == 1) { // Player 2 (AI/Human) fouled, Player 1 gets ball-in-hand
            ...
            if (isPlayer2AI) {
                // --- FIX: Set correct state for AI Ball-in-Hand ---
                currentGameState = BALL_IN_HAND_P2; // AI needs to place the ball
                aiTurnPending = true; // Trigger AI logic (will call AIPlaceCueBall)
                // --- End Fix ---
            }
            else {
        ...
        // If called for initial break placement (behindHeadstring=true), override state
        if (behindHeadstring) {
            currentGameState = PRE_BREAK_PLACEMENT;
            // Let player/AI handle placement based on who is breaking (set in InitGame)
        }
    }
}
...
void ProcessShotResults() {
    ...
    int firstPocketedId = -1; // Renamed for clarity
    ...
    bool turnFoul = false;
    std::wstring foulReason = L""; // Optional: store reason for display/debug

    // 1. Scratch (Cue ball pocketed)
    if (cueBallPocketed) {
        foulCommitted = true;
        turnFoul = true;
        foulReason = L"Scratch";
    }

    // 2. Wrong Ball First Foul (Only check if cue ball hit something)
    if (!turnFoul && firstHitBallIdThisShot != -1) { // Check only if not already a scratch foul
        Ball* firstHitBall = GetBallById(firstHitBallIdThisShot);
        if (firstHitBall) { // Ensure ball exists
            bool isBreakShot = (player1Info.assignedType == BallType::NONE); // Approx check for break state
            bool mustTarget8Ball = (!isBreakShot && currentPlayerInfo.assignedType != BallType::NONE && currentPlayerInfo.ballsPocketedCount >= 7);

            if (!isBreakShot) { // Standard play (not break)
                if (mustTarget8Ball) {
                    if (firstHitBall->id != 8) {
                        foulCommitted = true; turnFoul = true; foulReason = L"Must hit 8-ball first";
                    }
                }
                else if (currentPlayerInfo.assignedType != BallType::NONE) { // Colors assigned
                    if (firstHitBall->type != currentPlayerInfo.assignedType && firstHitBall->id != 8) {
                        foulCommitted = true; turnFoul = true; foulReason = L"Hit opponent's ball first";
                    }
                    else if (firstHitBall->id == 8) {
                        foulCommitted = true; turnFoul = true; foulReason = L"Hit 8-ball first";
                    }
                }
                // If colors NOT assigned yet (e.g. foul on shot immediately after break)
                // Any ball hit first is legal (unless scratch), this case doesn't add a foul here.
            }
            // Note: Break shot foul rules (hitting specific ball first) are often different/ignored.
        }
    }

    // 3. No Rail After Contact Foul
    if (!turnFoul && cueHitObjectBallThisShot && !railHitAfterContact && !anyNonCueBallPocketed) {
        // Condition: Cue hit an object ball, BUT after that collision,
        // NO ball hit a rail AND NO object ball was pocketed. This is a foul.
        foulCommitted = true;
        turnFoul = true;
        foulReason = L"No ball hit rail after contact";
    }

    // 4. Pocketing Opponent's Ball Foul (Optional stricter rule)
    // if (!turnFoul && opponentBallPocketed) {
    //     foulCommitted = true;
    //     turnFoul = true;
    //     foulReason = L"Pocketed opponent's ball";
    // }
    ...
    // 1. Break Shot Results (Assigning Colors) - Check fouls first
    if (turnFoul) { // If any foul occurred on the break
        SwitchTurns();
        RespawnCueBall(false); // Ball in hand for opponent
    }
    else if (player1Info.assignedType == BallType::NONE && anyNonCueBallPocketed) {
        // (Assign types logic - unchanged from previous fix)
        ...
        if (firstTypeVerified && (firstPocketedType == BallType::SOLID || firstPocketedType == BallType::STRIPE)) {
            ...
            player1Info.ballsPocketedCount = 0; player2Info.ballsPocketedCount = 0;
            for (int id : pocketedThisTurn) { /* ... count update ... */
            ...
            // Check if player continues (unchanged)
            bool pocketedOwnAssignedType = false;
            for (int id : pocketedThisTurn) { /* ... check pocketedOwnAssignedType ... */
                Ball* b = GetBallById(id);
                if (b && b->id != 0 && b->id != 8 && b->type == currentPlayerInfo.assignedType) {
                    pocketedOwnAssignedType = true; break;
                }
            }
            if (pocketedOwnAssignedType) { // Continue turn
                currentGameState = (currentPlayer == 1) ? PLAYER1_TURN : PLAYER2_TURN;
                if (currentPlayer == 2 && isPlayer2AI) aiTurnPending = true;
            }
            else { SwitchTurns(); } // Switch turns
        }
        else { SwitchTurns(); } // Only 8ball pocketed or error
    }
    // 2. Normal Play Results
    else {
        // Update counts (unchanged logic)
        int p1NewlyPocketed = 0; int p2NewlyPocketed = 0;
        for (int id : pocketedThisTurn) { /* ... count update ... */
            Ball* b = GetBallById(id);
        ...
        else if (legalBallPocketed) { // Legally pocketed own ball
            currentGameState = (currentPlayer == 1) ? PLAYER1_TURN : PLAYER2_TURN;
            if (currentPlayer == 2 && isPlayer2AI) aiTurnPending = true; // AI continues turn
        }
        else { // No legal ball pocketed (or no ball pocketed at all)
            SwitchTurns();
        }
    }

    // Clear lists / flags for the next actual shot
    pocketedThisTurn.clear();
    // Foul tracking flags reset before next shot in LBUTTONUP/AIMakeDecision
}

...
void CreatePocketMasks_D2D(int pixelWidth, int pixelHeight, float originX, float originY, float scale)
{
    ...
    for (int i = 0; i < 6; ++i) {
        ...
        pWicRT->Clear(D2D1::ColorF(0, 0.0f)); // fully transparent background
                ...
                // --- DEFINITIVE FIX: Create a smaller, more accurate circular-only mask ---
        // This mask represents ONLY the black hole, not the entire V-cut opening. This
        // makes pocketing more precise and realistic.

        // We draw the circular hole in the center of our local mask bitmap.
        D2D1_POINT_2F maskCenter = { pocketMaskWidth / 2.0f, pocketMaskHeight / 2.0f };

        // Use the visual radius, scaled to pixels, for a perfect match.
        float radiusInPixels = GetPocketVisualRadius(i) * pocketMaskScale;

        // To make the middle pockets slightly less generous, we can apply a small reduction here.
        if (i == 1 || i == 4) {
            radiusInPixels *= 0.95f; // Middle pocket masks are 5% smaller than the visual hole.
        }

        D2D1_ELLIPSE pocketEllipse = D2D1::Ellipse(maskCenter, radiusInPixels, radiusInPixels);
        pWicRT->FillEllipse(&pocketEllipse, pPocketBrush);

                /*// --- FOOLPROOF FIX: Create a unified "keyhole" mask for each pocket ---
        // This ensures the mask perfectly matches the visual opening.
        ...
        const float rimWidth = INNER_RIM_THICKNESS * pocketMaskScale;
        const float vCutDepth = CHAMFER_SIZE * pocketMaskScale;

        D2D1_POINT_2F v_tip, v_base1, v_base2;
        if (i == 1) { // Top-Middle
            v_tip = { maskCenter.x, maskCenter.y + rimWidth + vCutDepth };
            v_base1 = { maskCenter.x - vCutDepth, maskCenter.y + rimWidth };
            v_base2 = { maskCenter.x + vCutDepth, maskCenter.y + rimWidth };
        }
        else if (i == 4) { // Bottom-Middle
            v_tip = { maskCenter.x, maskCenter.y - rimWidth - vCutDepth };
            v_base1 = { maskCenter.x + vCutDepth, maskCenter.y - rimWidth };
            v_base2 = { maskCenter.x - vCutDepth, maskCenter.y - rimWidth };
        }
        else if (i == 0) { // Top-Left
            v_tip = { maskCenter.x + rimWidth + vCutDepth, maskCenter.y + rimWidth + vCutDepth };
            v_base1 = { maskCenter.x + rimWidth, maskCenter.y + vCutDepth };
            v_base2 = { maskCenter.x + vCutDepth, maskCenter.y + rimWidth };
        }
        else if (i == 2) { // Top-Right
            v_tip = { maskCenter.x - rimWidth - vCutDepth, maskCenter.y + rimWidth + vCutDepth };
            v_base1 = { maskCenter.x - vCutDepth, maskCenter.y + rimWidth };
            v_base2 = { maskCenter.x - rimWidth, maskCenter.y + vCutDepth };
        }

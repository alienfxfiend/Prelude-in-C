...
AIShotInfo EvaluateShot_8Ball(Ball* targetBall, int pocketIndex, bool isBank) {
    ...
    // We aim for the Playable 'Aim Target' to ensure straight-line shots do not clip physical walls.
    D2D1_POINT_2F pocketPos = pocketPositions[pocketIndex];
    D2D1_POINT_2F aimTargetPos = GetPocketAimTarget(pocketIndex);
    D2D1_POINT_2F targetPos = { targetBall->x, targetBall->y };
    ...
    if (possibleShots.empty()) {
        ...
        for (Ball* target : targetsToEvaluate) {
            ...
            if (nearbyBalls >= 2) { // Target is in a cluster
                D2D1_POINT_2F ghostPos = CalculateGhostBallPos(target, { target->x, target->y }); // Aim center
                if (IsPathClear({ cue->x, cue->y }, { target->x, target->y }, cue->id, target->id)) {

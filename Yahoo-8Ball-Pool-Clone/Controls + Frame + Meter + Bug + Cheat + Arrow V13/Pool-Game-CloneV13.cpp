    ...
    return 0;
        ...
        case WM_MOUSEMOVE: {
                    ...
                    (!isPlayer2AI && currentPlayer == 2 && (currentGameState == PLAYER2_TURN || currentGameState == AIMING || currentGameState == BREAKING))))
            {
                ...
                if (canUpdateVisualAngle && !isDraggingCueBall && !isAiming && !isDraggingStick)
                {
...
void DrawPowerMeter(ID2D1RenderTarget* pRT) {
    ...
    gradientStops[2].color = D2D1::ColorF(D2D1::ColorF::Red);

    pRT->CreateGradientStopCollection(gradientStops, 3, &pGradientStops);
    if (pGradientStops) {

...
void InitGame() {
    ...
    // --- ADDITIONS FOR AI MODE ---
    if (isPlayer2AI) {
        // Already set name in ShowNewGameDialog / wWinMain
        // player2Info.name = L"CPU"; // Or specific difficulty name set earlier

        // Randomly decide who breaks (50/50 chance)
        if ((rand() % 2) == 0) {
            // AI (Player 2) breaks
            currentPlayer = 2;
            // Need to handle cue ball placement for AI break?
            // For now, AI will need ball-in-hand logic if it breaks first.
            // Set state appropriate for AI break placement/shot
            currentGameState = PRE_BREAK_PLACEMENT; // AI needs to place ball first
            aiTurnPending = true; // Trigger AI logic immediately once placement done (or skip placement)
        }
        else {
            // Player 1 breaks
            currentPlayer = 1;
        ...
        // Human vs Human, Player 1 always breaks currently
        currentPlayer = 1;
    ...
    firstBallPocketedAfterBreak = false;
    pocketedThisTurn.clear();
    // aiIsThinking = false; // Replaced by state
...
void DrawUI(ID2D1RenderTarget* pRT) {
    ...
    if (currentGameState == AI_THINKING && pTextFormat) {
        ...
        if (pThinkingBrush) {
            // Display somewhere noticeable, maybe near Player 2's info box?
            D2D1_RECT_F thinkingRect = p2Rect; // Reuse P2 rect area
            thinkingRect.top += 20; // Move down slightly within the box

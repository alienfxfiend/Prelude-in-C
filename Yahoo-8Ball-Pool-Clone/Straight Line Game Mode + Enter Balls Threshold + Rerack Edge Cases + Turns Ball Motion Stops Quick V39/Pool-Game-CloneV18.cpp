...
const WCHAR* gameTypeNames[] = { L"8-Ball Billiards", L"Straight Pool" }; // For ComboBox
...
bool IsWrongTargetForCurrentPlayer(const Ball* ball) {
    // This check is only relevant for 8-Ball
...
void LoadSettings() {
    ...
    if (inFile.is_open()) {
        ...
        if (inFile >> gt) { // Load Game Type first
            if (gt >= GameType::EIGHT_BALL_MODE && gt <= GameType::STRAIGHT_POOL) currentGameType = static_cast<GameType>(gt); // CORRECTED
...
INT_PTR CALLBACK NewGameDialogProc(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam) {
    ...
    auto UpdateControlStates = [hDlg]() {
        ...
        GameType selectedGameType = (selectedGameModeIndex == 1) ? GameType::STRAIGHT_POOL : GameType::EIGHT_BALL_MODE; // CORRECTED
...
// Optional: Force centering if default isn't working
        ...
        SendMessage(hGameModeCombo, CB_ADDSTRING, 0, (LPARAM)gameTypeNames[1]); // Straight Pool
...
void ProcessShotResults() {
    ...
    for (int id : pocketedThisTurn) {
        ...
        if (b->id == 0) {
            ...
            ballsOnTableCount--; // Decrement balls on table count
    ...
    if (currentGameType == GameType::EIGHT_BALL_MODE && eightBallPocketedThisTurn) {
        ...
        CheckGameOverConditions(false, cueBallPocketedThisTurn); // Check if score target reached
        if (currentGameState == GAME_OVER) {
            pocketedThisTurn.clear();
            return;
        }
    ...
    if (cueBallPocketedThisTurn) {
        ...
        if (!firstHit) { // Rule: Hitting nothing is a foul (applies to both modes)
            turnFoul = true;
        }
        // 8-Ball specific foul checks
            ...
            if (player1Info.assignedType != BallType::NONE) { // Colors are assigned
                PlayerInfo& shootingPlayer = (currentPlayer == 1) ? player1Info : player2Info;
                // Check if player *was* on 8-ball *before* this shot finished
                bool wasOnEightBall = (shootingPlayer.assignedType != BallType::NONE && (shootingPlayer.ballsPocketedCount) >= 7);
                // Note: ballsPocketedCount was already incremented if they made a ball this turn

                if (wasOnEightBall) {
                    if (firstHit->id != 8) turnFoul = true; // Must hit 8-ball first when on it
                }
                else {
                    // Check if first hit ball is opponent's ball or 8 ball (if not on 8)
        ...
        // Straight Pool has no "wrong ball first" foul, only failure to hit *any* ball.
    }
    // No rail after contact foul (only apply in 8-ball if you want strict rules, skipping for now)
    // if (currentGameType == EIGHT_BALL && !turnFoul && cueHitObjectBallThisShot && !railHitAfterContact && numberedBallsPocketedThisTurn == 0 && !cueBallPocketedThisTurn) {
    //     turnFoul = true; // Optional 8-ball rule
    // }

    foulCommitted = turnFoul;
    if (turnFoul) {
        foulDisplayCounter = 120; // Show "FOUL!" text
    }

    // --- Step 4: State Transitions ---
    if (foulCommitted) {
        SwitchTurns();
        // Ball-in-hand placement depends on rules. Simple: anywhere.
        ...
        // Player always continues after break if they pocket legally (even if no assignment yet)
        playerContinuesTurn = true; // Ensure turn continues if balls were pocketed legally
        ...
        ReRackStraightPool();
        // Player continues turn after re-rack automatically (state set in ReRack function)
    }
    else if (playerContinuesTurn) {
        // Player legally pocketed their ball (8-ball) or any ball (Straight Pool)
        if (currentGameType == GameType::EIGHT_BALL_MODE) {
            CheckAndTransitionToPocketChoice(currentPlayer); // Check if 8-ball next
        }
        else {
            // Straight Pool: Just continue the turn
            currentGameState = (currentPlayer == 1) ? PLAYER1_TURN : PLAYER2_TURN;
            if (currentPlayer == 2 && isPlayer2AI) aiTurnPending = true;
        }
    }
...
void AIMakeDecision() {
    ...
    if (bestShot.possible) {
        ...
        if (currentGameType == GameType::EIGHT_BALL_MODE && bestShot.involves8Ball) {

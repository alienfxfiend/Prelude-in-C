...
void InitGame() {
    ...
    if (currentGameType == GameType::NINE_BALL) {
        ...
        std::random_shuffle(objectBalls.begin(), objectBalls.end()); // Shuffle all N
...
AIShotInfo EvaluateShot_NineBall(Ball* targetBall, int pocketIndex, bool isBank, bool isCombo) {
    ...
    // --- NEW: Aggressive "Aim Deeper" Logic ---
    // This avoids the "far jaw" logic and instead aims slightly inside the pocket
    // along the target-to-pocket line, giving a much better margin for error on V-cuts.
    float shotLineX = pocketPos.x - targetPos.x, shotLineY = pocketPos.y - targetPos.y;
    float len = sqrtf(shotLineX * shotLineX + shotLineY * shotLineY);
    if (len > 1e-6f) { shotLineX /= len; shotLineY /= len; }
    // Aim 3/4 of a ball's radius *past* the pocket center (deeper into the pocket)
    D2D1_POINT_2F aimTargetPos = {
        pocketPos.x - shotLineX * (BALL_RADIUS * 0.75f),
        pocketPos.y - shotLineY * (BALL_RADIUS * 0.75f)
    };
    ...
    // Scoring
    float distanceScore = 2000.0f - (cueToGhostDist + targetToPocketDist);
    // --- NEW: Less harsh penalty for cut shots ---
    // This makes the AI more willing to try angled shots.
    // Was: (0.1f + 0.9f * powf(straightness, 2));
    shotInfo.score = distanceScore * (0.5f + 0.5f * straightness);
    ...
    float perpAngle = shotInfo.angle + PI / 2.0f;
    float cueTravelDist = shotInfo.power * 2.0f; // Simplified travel distance
    D2D1_POINT_2F cueEndPos = {
         ghostBallPos.x + cosf(perpAngle) * cueTravelDist * 0.2f,
         ghostBallPos.y + sinf(perpAngle) * cueTravelDist * 0.2f
    };
    Ball tempCue = *cue; tempCue.x = cueEndPos.x; tempCue.y = cueEndPos.y;
    // Call the positional evaluator (which is already in your file)
...
AIShotInfo AIFindBestShot_NineBall() {
    ...
    for (Ball& otherBall : balls) {
        ...
        for (int p = 0; p < 6; ++p) {
            ...
            // --- NEW: Use "Aim Deeper" logic for the combo shot's target ball ---
            D2D1_POINT_2F otherBallTargetPos = { otherBall.x, otherBall.y };
            float comboLineX = pocketPos.x - otherBallTargetPos.x, comboLineY = pocketPos.y - otherBallTargetPos.y;
            float comboLen = sqrtf(comboLineX * comboLineX + comboLineY * comboLineY);
            if (comboLen > 1e-6f) { comboLineX /= comboLen; comboLineY /= comboLen; }
            D2D1_POINT_2F comboAimTarget = { pocketPos.x - comboLineX * (BALL_RADIUS * 0.75f), pocketPos.y - comboLineY * (BALL_RADIUS * 0.75f) };
            // This is where the lowest ball must hit the other ball:
            D2D1_POINT_2F ghostPosForCombo = CalculateGhostBallPos(&otherBall, comboAimTarget);
            // --- END NEW COMBO AIMING ---
            ...
            if (cand.possible) {
                ...
                float d3 = GetDistance(otherBall.x, otherBall.y, comboAimTarget.x, comboAimTarget.y); // Use aim target
                cand.power = CalculateShotPower(d1, d2 + d3, false); // Use the 3-part distance

                // Keep the score calculated by EvaluateShot_NineBall
                // but add the 9-ball win bonus again just in case.
...
/*// Part 1: Draw the Outer Wooden Rails (Solid Rectangles)
    ...
    //newlyomitted-added
       // --- [+] MOVE: Define rim polygons here so shadows can use them ---
    ...
    // --- Layer 5: Inner Rim Dropshadow ---
    ID2D1SolidColorBrush* pRimShadowBrush = nullptr;
    pRT->CreateSolidColorBrush(D2D1::ColorF(0.0f, 0.0f, 0.0f, 0.35f), &pRimShadowBrush);
    if (pRimShadowBrush) {
        // FIX: Define unique shadow offsets for each of the 6 rim pieces
        // (Offsets are {X, Y} - casting shadows inward onto the felt)
        const D2D1_POINT_2F shadowOffsets[6] = {
            { 2.5f, 3.5f },  // Top Rail (Left Part) -> casts shadow down-right
            { 2.5f, 3.5f },  // Top Rail (Right Part) -> casts shadow down-right
            { 2.5f, -3.5f }, // Bottom Rail (Left Part) -> casts shadow up-right
            { 2.5f, -3.5f }, // Bottom Rail (Right Part) -> casts shadow up-right
            { 3.5f, 2.5f },  // Left Rail -> casts shadow right-down
            { -3.5f, 2.5f }  // Right Rail -> casts shadow left-down
        };

        for (int i = 0; i < 6; ++i) {
            ID2D1PathGeometry* pShadowPath = nullptr;
            pFactory->CreatePathGeometry(&pShadowPath);
            if (pShadowPath) {
                ID2D1GeometrySink* pShadowSink = nullptr;
                if (SUCCEEDED(pShadowPath->Open(&pShadowSink))) {
                    // FIX: Use the new dynamic offset for this rim piece
                    pShadowSink->BeginFigure(
                        D2D1::Point2F(rim_pieces[i][0].x + shadowOffsets[i].x, rim_pieces[i][0].y + shadowOffsets[i].y),
                        D2D1_FIGURE_BEGIN_FILLED
                    );
                    D2D1_POINT_2F offsetPoints[3] = {
                       { rim_pieces[i][1].x + shadowOffsets[i].x, rim_pieces[i][1].y + shadowOffsets[i].y },
                       { rim_pieces[i][2].x + shadowOffsets[i].x, rim_pieces[i][2].y + shadowOffsets[i].y },
                       { rim_pieces[i][3].x + shadowOffsets[i].x, rim_pieces[i][3].y + shadowOffsets[i].y }
                    };
                    pShadowSink->AddLines(offsetPoints, 3);
                    pShadowSink->EndFigure(D2D1_FIGURE_END_CLOSED);
                    pShadowSink->Close();
                }
                SafeRelease(&pShadowSink);
                pRT->FillGeometry(pShadowPath, pRimShadowBrush);
                SafeRelease(&pShadowPath);
            }
        }
        SafeRelease(&pRimShadowBrush);
    }
...
void DrawBalls(ID2D1RenderTarget* pRT)
{
    ...
    pRT->CreateSolidColorBrush(D2D1::ColorF(0.0f, 0.0f, 0.0f, 0.35f), &pShadowBrush);
...
void ReRackPracticeMode() {
    ...
    if (currentGameType == GameType::NINE_BALL) {
        ...
        std::random_shuffle(otherBalls.begin(), otherBalls.end());
        ...
        std::random_shuffle(objectBallsToRack.begin(), objectBallsToRack.end());

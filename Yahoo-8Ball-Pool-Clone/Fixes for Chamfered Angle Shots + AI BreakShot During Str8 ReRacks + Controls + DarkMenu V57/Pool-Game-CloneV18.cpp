...
LRESULT CALLBACK WndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_MEASUREITEM: {
        ...
        if (mis->CtlType == ODT_MENU) {
            ...
            mis->itemWidth = sz.cx + 12; // narrower top-level items
            return TRUE;
        }
        break;
    }
    case WM_DRAWITEM: {
        ...
        if (dis->CtlType == ODT_MENU) {
            ...
            if (miiSep.fType & MFT_SEPARATOR) {
                // Draw a single horizontal line in the middle of the separator rect
                int y = (dis->rcItem.top + dis->rcItem.bottom) / 2;
                HPEN hPen = CreatePen(PS_SOLID, 1, RGB(80, 80, 80)); // subtle dark gray line
                ...
                DeleteObject(hPen);
                return TRUE; // stop here, don’t draw background/text
            }
            ...
            rcText.left += 24; // space for checkmark

            // Handle tab-separated shortcut
    ...
    case WM_CREATE:
    {
        ...
        if (hMenu) {
            ...
            DrawMenuBar(hwnd);
            // Force redraw to apply dark theme immediately

...
void DrawBalls(ID2D1RenderTarget* pRT)
{
    ...
    for (const Ball& b : balls)
    {
        ...
        // ==================== MOTION TRAIL (Improved) ====================
        if (pMotionTrailBrush) {
            float speedSq = b.vx * b.vx + b.vy * b.vy;
            const float TRAIL_MIN_SPEED = 0.8f; // Lower threshold so trails appear more easily
            ...
            if (speedSq > TRAIL_MIN_SPEED * TRAIL_MIN_SPEED) {
                ...
                // Manual clamp (more reliable than std::clamp in some setups)
                float speedFactor = (speed - TRAIL_MIN_SPEED) / 12.0f;
                if (speedFactor < 0.0f) speedFactor = 0.0f;
                if (speedFactor > 1.0f) speedFactor = 1.0f;

                const int TRAIL_SEGMENTS = 5;
                for (int t = 1; t <= TRAIL_SEGMENTS; ++t) {
                    float trailDist = t * (BALL_RADIUS * 0.65f);
                    float alpha = 0.55f * speedFactor * (1.0f - (float)t / (TRAIL_SEGMENTS + 2));

                    pMotionTrailBrush->SetOpacity(alpha);

                    float trailRadius = BALL_RADIUS * (1.0f - 0.10f * t);
                ...
                pMotionTrailBrush->SetOpacity(1.0f); // Reset for safety
            }
        }
...
void DrawCueStick(ID2D1RenderTarget* pRT)
{
    ...
    */
    ...
    // [+] NEW: Cue stick dropshadow (drawn first so the stick renders on top of it)
    {
        ID2D1SolidColorBrush* pCueShadowBrush = nullptr;
        pRT->CreateSolidColorBrush(D2D1::ColorF(0.0f, 0.0f, 0.0f, 0.35f), &pCueShadowBrush);
        if (pCueShadowBrush) {
            const float shadowOffsetX = 3.0f;
            const float shadowOffsetY = 4.0f;
            auto offsetPt = [&](D2D1_POINT_2F p) {
                return D2D1::Point2F(p.x + shadowOffsetX, p.y + shadowOffsetY);
            };

            auto buttShadow = buildRect(offsetPt(base), offsetPt(mid1), buttWidth, shaftWidth);
            if (buttShadow) { pRT->FillGeometry(buttShadow, pCueShadowBrush); SafeRelease(&buttShadow); }

            auto shaftShadow = buildRect(offsetPt(mid1), offsetPt(mid2), shaftWidth, shaftWidth);
            if (shaftShadow) { pRT->FillGeometry(shaftShadow, pCueShadowBrush); SafeRelease(&shaftShadow); }

            auto tipShadow = buildRect(offsetPt(mid2), offsetPt(tip), shaftWidth, shaftWidth);
            if (tipShadow) { pRT->FillGeometry(tipShadow, pCueShadowBrush); SafeRelease(&tipShadow); }
        }
...
void DrawSpinIndicator(ID2D1RenderTarget* pRT) {
    ...
    ID2D1SolidColorBrush* pCrosshairBrush = nullptr;  // [+] NEW: gray crosshair lines
    ID2D1SolidColorBrush* pDotOutlineBrush = nullptr; // [+] NEW: black outline for spin dot
    ...
    pRT->CreateSolidColorBrush(D2D1::ColorF(D2D1::ColorF::Black/*, 0.6f*/), &pCrosshairBrush);  // [+] NEW
    pRT->CreateSolidColorBrush(D2D1::ColorF(D2D1::ColorF::Black), &pDotOutlineBrush);      // [+] NEW
    ...
    if (pWhiteBrush && pRedBrush) {
        ...
        // [+] NEW: Draw gray crosshair through the center of the spin indicator
        if (pCrosshairBrush) {
            pRT->DrawLine(
                D2D1::Point2F(spinIndicatorCenter.x - spinIndicatorRadius, spinIndicatorCenter.y),
                D2D1::Point2F(spinIndicatorCenter.x + spinIndicatorRadius, spinIndicatorCenter.y),
                pCrosshairBrush, 1.0f);
            pRT->DrawLine(
                D2D1::Point2F(spinIndicatorCenter.x, spinIndicatorCenter.y - spinIndicatorRadius),
                D2D1::Point2F(spinIndicatorCenter.x, spinIndicatorCenter.y + spinIndicatorRadius),
                pCrosshairBrush, 1.0f);
        }
        ...
        // [+] NEW: Black outline around the red spin dot
        if (pDotOutlineBrush) {
    ...
    SafeRelease(&pCrosshairBrush);  // [+] NEW
    SafeRelease(&pDotOutlineBrush); // [+] NEW
}

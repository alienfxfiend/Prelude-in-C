...
void ProcessShotResults() {
    ...
    // fouls (unchanged)
    bool turnFoul = false;
    if (cueBallPocketed) { foulCommitted = true; turnFoul = true; }
    if (!turnFoul && !cueHitObjectBallThisShot) { foulCommitted = true; turnFoul = true; }
    if (!turnFoul && firstHitBallIdThisShot != -1) {
        Ball* fh = GetBallById(firstHitBallIdThisShot);
        if (fh && currentPlayerInfo.assignedType != BallType::NONE && fh->type != currentPlayerInfo.assignedType && fh->id != 8) {
    ...
    // if foul, switch immediately
    if (turnFoul) {
        SwitchTurns();
        RespawnCueBall(false);
    }
    // first time colours assignment
        ...
        for (int id : pocketedThisTurn) {
            if (id == firstPocketedId) { firstTypeVerified = true; break; }
        }
        ...
        if (firstTypeVerified && (firstPocketedType == BallType::SOLID || firstPocketedType == BallType::STRIPE)) {
            ...
            // ** NEW: give extra turn if that first ball matches your new group **
            ...
            // e.g. only 8-ball fell on break
            SwitchTurns();
        }
    }
    // normal post-assignment play
    ...
    for (int id : pocketedThisTurn) {
        ...
        if (b->type == player1Info.assignedType) p1New++;
        else if (b->type == player2Info.assignedType) p2New++;
    }
    if (currentGameState != GAME_OVER) {
        player1Info.ballsPocketedCount += p1New;
        player2Info.ballsPocketedCount += p2New;
    }
...
void ProcessShotResults() {
    ...
    for (int id : pocketedThisTurn) {
        Ball* b = GetBallById(id);
        if (!b || b->id == 0 || b->id == 8) continue;
        // Use types assigned *now* after potential assignment call
        if (b->type == player1Info.assignedType) p1NewlyPocketed++;
        else if (b->type == player2Info.assignedType) p2NewlyPocketed++;
    }
    if (currentGameState != GAME_OVER) { // Avoid counting on game-ending 8-ball pocket

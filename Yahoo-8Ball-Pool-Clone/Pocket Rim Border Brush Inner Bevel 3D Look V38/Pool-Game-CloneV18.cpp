...
void SaveSettings() {
    ...
    // Imbue the stream with a UTF-8 locale to correctly handle wide characters
    ...
    if (outFile.is_open()) {
        ...
        // --- NEW: Save player names, replacing spaces with underscores ---
        std::wstring p1_safe = g_player1Name;
        std::replace(p1_safe.begin(), p1_safe.end(), L' ', L'_');
        std::wstring p2_safe = g_player2Name;
        std::replace(p2_safe.begin(), p2_safe.end(), L' ', L'_');
        std::wstring ai_safe = g_aiName;
        std::replace(ai_safe.begin(), ai_safe.end(), L' ', L'_');
...
void LoadSettings() {
    ...
    // Imbue the stream with the same UTF-8 locale for reading
    ...
    if (inFile.is_open()) {
        ...
        // --- NEW: Load player names, converting underscores back to spaces ---
        std::wstring p1_safe, p2_safe, ai_safe;
        if (inFile >> p1_safe) {
            std::replace(p1_safe.begin(), p1_safe.end(), L'_', L' ');
            g_player1Name = p1_safe;
        }
        if (inFile >> p2_safe) {
            std::replace(p2_safe.begin(), p2_safe.end(), L'_', L' ');
            g_player2Name = p2_safe;
        }
        if (inFile >> ai_safe) {
            std::replace(ai_safe.begin(), ai_safe.end(), L'_', L' ');
            g_aiName = ai_safe;
        }

        // --- NEW: Load player statistics ---
        if (!(inFile >> g_stats.p1_wins)) g_stats.p1_wins = 0;
        if (!(inFile >> g_stats.p1_losses)) g_stats.p1_losses = 0;
        if (!(inFile >> g_stats.ai_wins)) g_stats.ai_wins = 0;
        if (!(inFile >> g_stats.human_vs_human_games)) g_stats.human_vs_human_games = 0;
...
void ShowNewGameDialog(HINSTANCE hInstance) {
    if (DialogBoxParam(hInstance, MAKEINTRESOURCE(IDD_NEWGAMEDLG), hwndMain, NewGameDialogProc, 0) == IDOK) {
        ...
        player1Info.name = g_player1Name; // Use the global name
        if (isPlayer2AI) {
            player2Info.name = g_aiName; // Use the global AI name
            //switch (aiDifficulty) {
            ...
            player2Info.name = g_player2Name; // Use the global P2 name
...
void CheckGameOverConditions(bool eightBallPocketed, bool cueBallPocketed)
{
    // This function only runs if the 8-ball was pocketed this turn.
    if (!eightBallPocketed) return;

    // Determine the shooter and opponent
    int shooterId = currentPlayer;
    int opponentId = (currentPlayer == 1) ? 2 : 1;
    PlayerInfo& shooterInfo = (shooterId == 1) ? player1Info : player2Info;

    // Determine which pocket was called by the shooter
    int calledPocket = (shooterId == 1) ? calledPocketP1 : calledPocketP2;

    // Determine if the winning conditions were met
    bool isLegalWin =
        (shooterInfo.ballsPocketedCount >= 7) && // Must have cleared their group
        (calledPocket != -1) &&                   // Must have called a pocket
        (calledPocket == lastEightBallPocketIndex) && // Must have gone in the called pocket
        (!cueBallPocketed);                       // Must not have scratched

    // Prepare a reason string for the game over message
    std::wstring reason = L"(Called: " + std::to_wstring(calledPocket) + L", Actual: " + std::to_wstring(lastEightBallPocketIndex) + L")";
    if (cueBallPocketed) reason = L"(Scratched on 8-Ball)";
    else if (shooterInfo.ballsPocketedCount < 7) reason = L"(8-Ball Pocketed Early)";
    else if (calledPocket == -1) reason = L"(Pocket Not Called)";
    else if (calledPocket != lastEightBallPocketIndex) reason = L"(8-Ball in Wrong Pocket)";


    // Call the finalization function with the correct winner and loser
    if (isLegalWin) {
        FinalizeGame(shooterId, opponentId, L""); // Legal win, no special reason needed
    }
    else {
...
void AIFindBankShots(Ball* targetBall, std::vector<AIShotInfo>& possibleShots) {
    ...
    const D2D1_POINT_2F railLines[4][2] = {
        { {TABLE_LEFT, TABLE_TOP + INNER_RIM_THICKNESS}, {TABLE_RIGHT, TABLE_TOP + INNER_RIM_THICKNESS} },
        { {TABLE_LEFT, TABLE_BOTTOM - INNER_RIM_THICKNESS}, {TABLE_RIGHT, TABLE_BOTTOM - INNER_RIM_THICKNESS} },
        { {TABLE_LEFT + INNER_RIM_THICKNESS, TABLE_TOP}, {TABLE_LEFT + INNER_RIM_THICKNESS, TABLE_BOTTOM} },
        { {TABLE_RIGHT - INNER_RIM_THICKNESS, TABLE_TOP}, {TABLE_RIGHT - INNER_RIM_THICKNESS, TABLE_BOTTOM} },
    };
    ...
    for (int p = 0; p < 6; ++p) {
        ...
        for (int r = 0; r < 4; ++r) {
            ...
            for (int pocketIdx = 0; pocketIdx < 6; ++pocketIdx) {
                if (GetDistanceSq(bouncePoint.x, bouncePoint.y, pocketPositions[pocketIdx].x, pocketPositions[pocketIdx].y) < powf(GetPocketVisualRadius(pocketIdx) * 1.5f, 2)) {
            ...
            // Path Checks: Cue -> Ghost, Target -> Bounce, Bounce -> Pocket
                ...
                IsPathClear(bouncePoint, pocketPos, targetBall->id, -1))
            {
                ...
                if (bankCand.possible) {
                    // Give bank shots a slight scoring boost to make them more viable when direct shots are poor.
                    bankCand.score += 150.0f;

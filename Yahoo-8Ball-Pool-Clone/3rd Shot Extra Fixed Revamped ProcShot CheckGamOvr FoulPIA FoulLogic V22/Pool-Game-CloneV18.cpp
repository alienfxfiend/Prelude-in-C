...
void ProcessShotResults()
{
    ...
    for (int id : pocketedThisTurn)
    {
        ...
        if (bb->type == player1Info.assignedType)
            ++player1Info.ballsPocketedCount;
    ...
    for (int id : pocketedThisTurn) { if (id != 0) { anyObjectPocketed = true; break; } }

    if (!anyObjectPocketed)
    {
        // 1) Scratch
        if (cueBallPocketed) turnFoul = true;
        else
        {
            // 2) Missed every ball / wrong first hit
            Ball* fh = GetBallById(firstHitBallIdThisShot);
            if (!fh) turnFoul = true;
            else
            {
                bool tableOpen = (player1Info.assignedType == BallType::NONE &&
                    player2Info.assignedType == BallType::NONE);
                bool mustHit8 = (!tableOpen && shootingPlayer.ballsPocketedCount >= 7);

                if (!tableOpen)
                {
                    if (mustHit8 && fh->id != 8)                       turnFoul = true;
                    else if (!mustHit8 && fh->type != shootingPlayer.assignedType)
                        turnFoul = true;
                }
            }
        }
        // 3) (optional rail-after-contact rule is commented out)
    }
    foulCommitted = turnFoul;
    if (isBreakShot) foulCommitted = false;  // never foul on opening break

    // -----------------------------------------------------------
    //  FREE TURN after a LEGAL BREAK
    // -----------------------------------------------------------
    if (isBreakShot)
    {
        isOpeningBreakShot = false;                 // break done
        bool legalBreak = anyObjectPocketed && !cueBallPocketed && !foulCommitted;
    ...
    // -----------------------------------------------------------
    //  Handle foul (ball-in-hand to opponent)
    // -----------------------------------------------------------
    if (foulCommitted)
    {
        SwitchTurns();
    ...
    // -----------------------------------------------------------
    //  Colour assignment (table was open)
    // -----------------------------------------------------------
    if (player1Info.assignedType == BallType::NONE &&
        ...
        !cueBallPocketed)
    {
        ...
        {
            Ball* b = GetBallById(id);
            if (!b || b->type == BallType::EIGHT_BALL) continue;

            AssignPlayerBallTypes(b->type);
            didAssignThisTurn = true;

            (currentPlayer == 1 ? player1Info : player2Info).ballsPocketedCount = 1;
            break;
        }
    }

    // -----------------------------------------------------------
    //  FREE TURN immediately after colour assignment
    // -----------------------------------------------------------
    if (didAssignThisTurn)
    {
        currentGameState = (currentPlayer == 1) ? PLAYER1_TURN : PLAYER2_TURN;
        if (currentPlayer == 2 && isPlayer2AI) aiTurnPending = true;
        pocketedThisTurn.clear();
    ...
    if (ownBallsPocketedThisTurn > 0)
    {
        ...
        if (shooter.ballsPocketedCount >= 7 && !eightBallPocketed)
        {
            ...
            currentGameState = (currentPlayer == 1)
                ? CHOOSING_POCKET_P1
                : CHOOSING_POCKET_P2;
            return;    // wait for pocket selection
        }
    }
    ...
    // -----------------------------------------------------------
    //  Ordinary continuation / turn switch
    // -----------------------------------------------------------
    ...
    // -----------------------------------------------------------
    pocketedThisTurn.clear();
    if (currentGameState == PLAYER1_TURN) calledPocketP2 = -1;
    if (currentGameState == PLAYER2_TURN) calledPocketP1 = -1;
}

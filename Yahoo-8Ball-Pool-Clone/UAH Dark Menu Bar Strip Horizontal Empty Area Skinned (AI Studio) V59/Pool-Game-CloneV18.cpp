...
AIShotInfo AIFindBestShot_StraightPool() {
    ...
    // [FIX] Detect if we are facing a full or partial rack (Re-Rack scenario)
    // 14.1 rules re-rack when 14 balls are pocketed (leaving 1 + cue). 
    // So if count >= 10, we are likely in a clustered/fresh rack state.
    ...
    for (Ball& b : balls) {
        ...
        for (int p = 0; p < 6; ++p) {
            ...
            if (cand.possible) {
                // [FIX] CLUSTER BREAKING LOGIC
                if (isClusterScenario) {
                    // 1. Calculate the vector from Ghost Ball to Target (The impact line)
                    float impactDx = b.x - cand.ghostBallPos.x;
                    float impactDy = b.y - cand.ghostBallPos.y;
                    float impactAngle = atan2f(impactDy, impactDx);

                    // 2. Calculate Cue Ball arrival angle
                    float cueArrDx = cand.ghostBallPos.x - cue->x;
                    float cueArrDy = cand.ghostBallPos.y - cue->y;
                    float cueArrAngle = atan2f(cueArrDy, cueArrDx);

                    // 3. Determine if this is a "Full" hit or "Cut" hit?
                    // Ideally, we want to drive the Cue Ball into the rack area (RACK_POS_X, RACK_POS_Y).
                    // For simplicity: If the shot sends the CUE ball towards the center of the table/rack.

                    // Simple break heuristic: 
                    // If the target ball is part of the outer edge of the rack, and we hit it hard,
                    // we want the cue ball to continue into the mass.

                    // Check distance of the target ball to the Rack Center
                    float distToRack = GetDistance(b.x, b.y, RACK_POS_X, RACK_POS_Y);

                    // If target is near the rack (part of the cluster)
                    if (distToRack < BALL_RADIUS * 6.0f) {
                        // FORCE HIGH POWER to scatter
                        cand.power = MAX_SHOT_POWER;

                        // BOOST SCORE significantly to prioritize this break shot over easy picks
                        cand.score += 2000.0f;
                    }
                }
    ...
    if (!possibleShots.empty()) {
        ...
        // [Existing Fallback Safety Logic Preserved]
        bestShot.possible = true;
        bestShot.score = -99999.0f;

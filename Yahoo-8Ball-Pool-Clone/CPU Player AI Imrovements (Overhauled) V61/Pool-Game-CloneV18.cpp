...
const WCHAR* tableColorNames[] = { L"Indigo", L"Tan", L"Teal", L"Green", L"Red", L"Custom..." };
const D2D1_COLOR_F tableColorValues[6] = {
...
int WINAPI wWinMain(HINSTANCE hInstance, HINSTANCE, PWSTR, int nCmdShow) {
    ...
    if (hCurrentMenu) {
        ...
        CheckMenuRadioItem(hCurrentMenu, ID_TABLECOLOR_INDIGO, ID_TABLECOLOR_RED, colorID, MF_BYCOMMAND);
    }
        ...
        case ID_TABLECOLOR_CUSTOM:
        {
            if (wmId == ID_TABLECOLOR_CUSTOM) {
                ...
                if (ChooseColor(&cc)) {
                    ...
                    TABLE_COLOR = g_customTableColor;
                    CheckMenuRadioItem(hMenu, ID_TABLECOLOR_INDIGO, ID_TABLECOLOR_CUSTOM, wmId, MF_BYCOMMAND);
...
// --- REPLACED: Strategic Ball-in-Hand Placement ---
void AIPlaceCueBall() {
    Ball* cue = GetCueBall();
    if (!cue) return;

    AIShotInfo bestSetup;
    bestSetup.score = -99999.0f;
    D2D1_POINT_2F optimalPos = { TABLE_LEFT + TABLE_WIDTH * 0.5f, TABLE_TOP + TABLE_HEIGHT * 0.5f };

    // Increase grid density for "Hard" AI
    int steps = (aiDifficulty == HARD) ? 15 : 8;
    float stepX = TABLE_WIDTH / steps;
    float stepY = TABLE_HEIGHT / steps;

    for (int x = 1; x < steps; ++x) {
        for (int y = 1; y < steps; ++y) {
            float testX = TABLE_LEFT + x * stepX;
            float testY = TABLE_TOP + y * stepY;

            if (!IsValidCueBallPosition(testX, testY, false)) continue;

            // Simulate best shot from this coordinate
            float oldX = cue->x; float oldY = cue->y;
            cue->x = testX; cue->y = testY;

            AIShotInfo testShot = (currentGameType == GameType::EIGHT_BALL_MODE) ? AIFindBestShot_8Ball() : AIFindBestShot_StraightPool();

            if (testShot.possible && testShot.score > bestSetup.score) {
                bestSetup = testShot;
                optimalPos = { testX, testY };
            }
            cue->x = oldX; cue->y = oldY;
        }
    }
    cue->x = optimalPos.x; cue->y = optimalPos.y;
    cue->vx = cue->vy = 0;
}
...
// --- FULL REPLACEMENT: High-Level Control Power Calculation ---
float CalculateShotPower(float cueToGhostDist, float targetToPocketDist, bool isBank) {
    // 1. Energy for object ball to reach pocket with 'drop' speed
    // Friction takes about 1 unit of power per 100 pixels.
    float target_leg = 3.0f + (targetToPocketDist / 70.0f);

    // 2. Energy for cue ball to reach the ghost position
    float cue_leg = (cueToGhostDist / 85.0f);

    // 3. Combine
    float power = target_leg + cue_leg;

    // 4. Multipliers
    if (isBank) power *= 1.65f; // Banks lose significant energy on cushions

    // 5. Professional "Touch":
    // If the ball is very close to the pocket, use LESS power to ensure it drops.
    if (targetToPocketDist < 100.0f && !isBank) {
        power = std::clamp(power, 3.5f, 6.0f);
    }

    // 6. Clamp to game limits
    return std::clamp(power, 3.0f, MAX_SHOT_POWER);
}
...
// --- FULL REPLACEMENT: Professional 8-Ball Evaluation ---
// --- FULL REPLACEMENT: Formidable 8-Ball Evaluation ---
AIShotInfo EvaluateShot_8Ball(Ball* targetBall, int pocketIndex, bool isBank) {
    AIShotInfo shotInfo;
    ...
    // 1. DYNAMIC GHOST POSITION (Deep Throat targeting)
    shotInfo.ghostBallPos = CalculateGhostBallPos(targetBall, pocketPos);

    // 2. PATH CHECKS (Capsule clearance)
    if (!IsPathClear({ cue->x, cue->y }, shotInfo.ghostBallPos, cue->id, targetBall->id)) return shotInfo;
    if (!isBank && !IsPathClear({ targetBall->x, targetBall->y }, pocketPos, targetBall->id, -1)) return shotInfo;

    // 3. CUT ANGLE & KNUCKLE PENALTY MATH
    D2D1_POINT_2F vCue = { shotInfo.ghostBallPos.x - cue->x, shotInfo.ghostBallPos.y - cue->y };
    D2D1_POINT_2F vObj = { pocketPos.x - targetBall->x, pocketPos.y - targetBall->y };
    float mC = sqrtf(vCue.x * vCue.x + vCue.y * vCue.y);
    float mO = sqrtf(vObj.x * vObj.x + vObj.y * vObj.y);

    if (mC < 1e-4f || mO < 1e-4f) return shotInfo;

    float dot = (vCue.x * vObj.x + vCue.y * vObj.y) / (mC * mO);
    float cutAngle = acosf(std::clamp(dot, -1.0f, 1.0f));
    float cutProb = std::max(0.0f, 1.0f - (cutAngle / (PI * 0.45f)));

    // --- FORMIdable FIX: KNUCKLE PENALTY ---
    // If the cut angle is sharp, we add a penalty because the ball 
    // is more likely to hit the chamfered rim "knuckle".
    float knucklePenalty = 0.0f;
    if (cutAngle > 0.35f) { // Sharp cut detection
        knucklePenalty = cutAngle * 350.0f;
    }

    // 4. SCORING HEURISTIC
    shotInfo.score = (cutProb * 1200.0f) - (mC * 0.15f) - (mO * 0.4f) - knucklePenalty;

    if (isBank) shotInfo.score -= 500.0f;
    if (shotInfo.involves8Ball) shotInfo.score += 250.0f;

    // 5. FINAL PARAMETERS
    shotInfo.angle = atan2f(shotInfo.ghostBallPos.y - cue->y, shotInfo.ghostBallPos.x - cue->x);
    shotInfo.power = CalculateShotPower(mC, mO, isBank);
...
// --- FULL REPLACEMENT: Professional Straight Pool Evaluation ---
// --- FULL REPLACEMENT: Professional Straight Pool Strategy ---
// --- FULL REPLACEMENT: Formidable Straight Pool Evaluation ---
AIShotInfo EvaluateShot_StraightPool(Ball* targetBall, int pocketIndex, bool isBank) {
    AIShotInfo shotInfo;
    shotInfo.possible = false;
    shotInfo.targetBall = targetBall;
    ...
    // --- FORMIdable FIX: KNUCKLE PENALTY ---
    float knucklePenalty = (cutAngle > 0.4f) ? (cutAngle * 400.0f) : 0.0f;

    // 4. SCORING HEURISTIC (Bonus for breaking clusters)
    shotInfo.score = (cutProb * 1200.0f) - (mC * 0.1f) - (mO * 0.2f) - knucklePenalty;

    float distToRack = GetDistance(targetBall->x, targetBall->y, RACK_POS_X, RACK_POS_Y);
    if (distToRack < BALL_RADIUS * 5.0f) shotInfo.score += 350.0f; // High priority for rack balls
...
// ==================== 8-BALL AI ====================
// --- FULL REPLACEMENT: Find All Shots 8-Ball ---
// --- FULL REPLACEMENT: Professional Search (Banks, Combos, Direct) ---
std::vector<AIShotInfo> AIFindAllShots_8Ball() {
    std::vector<AIShotInfo> possibleShots;
    Ball* cue = GetCueBall();
    ...
    for (Ball& b : balls) {
        if (b.isPocketed || b.id == 0) continue;
        bool isLegal = on8 ? (b.id == 8) : (wantType == BallType::NONE || b.type == wantType);
        if (wantType != BallType::NONE && !on8 && b.id == 8) isLegal = false;
        if (isLegal) legalTargets.push_back(&b);
    }

    // 2. SEARCH: DIRECT SHOTS
    for (Ball* target : legalTargets) {
        for (int p = 0; p < 6; ++p) {
            AIShotInfo cand = EvaluateShot_8Ball(target, p, false);
            if (cand.possible) possibleShots.push_back(cand);
        }
    }

    // 3. SEARCH: BANK SHOTS (Only for higher difficulties)
    if (aiDifficulty > EASY) {
        for (Ball* target : legalTargets) {
            // This calls the geometric rail-reflection helper
            AIFindBankShots(target, possibleShots);
        }
    }

    // 4. SEARCH: COMBO SHOTS (Hitting Ball A into Ball B)
    if (aiDifficulty == HARD) {
        for (Ball* firstBall : legalTargets) { // Ball we hit with cue
            for (Ball& secondBall : balls) {   // Ball that goes in pocket
                if (secondBall.isPocketed || secondBall.id == 0 || secondBall.id == firstBall->id) continue;

                // Only combo into your own type or 8-ball
                if (wantType != BallType::NONE && secondBall.type != wantType && secondBall.id != 8) continue;

                for (int p = 0; p < 6; ++p) {
                    D2D1_POINT_2F pocketPos = pocketPositions[p];
                    D2D1_POINT_2F ghostForSecond = CalculateGhostBallPos(&secondBall, pocketPos);

                    // Check Path: SecondBall -> Pocket
                    if (IsPathClear({ secondBall.x, secondBall.y }, pocketPos, secondBall.id, -1)) {
                        // Check Path: FirstBall -> Ghost Position for SecondBall
                        D2D1_POINT_2F ghostForFirst = CalculateGhostBallPos(firstBall, ghostForSecond);
                        if (IsPathClear({ firstBall->x, firstBall->y }, ghostForSecond, firstBall->id, secondBall.id)) {
                            // Check Path: Cue -> Ghost Position for FirstBall
                            if (IsPathClear({ cue->x, cue->y }, ghostForFirst, cue->id, firstBall->id)) {
                                AIShotInfo combo = EvaluateShot_8Ball(firstBall, p, false);
                                combo.ghostBallPos = ghostForFirst;
                                combo.score -= 300.0f; // Penalty for complexity
                                combo.score += 100.0f; // Bonus for a professional combo
                                possibleShots.push_back(combo);
                            }
                        }
                    }
                }
    ...
    // THE PRO FILTER: Sort by pure high-percentage EV
    std::sort(possibleShots.begin(), possibleShots.end(), [](const AIShotInfo& a, const AIShotInfo& b) {
        return a.score > b.score;
        });

    if (possibleShots.empty()) {
        // PROFESSIONAL SAFETY: If no pot is possible, don't just nudge.
        // Hit the closest legal ball softly to hide the cue ball.
        AIShotInfo safety;
        safety.possible = true;
        safety.score = -5000.0f;

        Ball* closest = legalTargets.empty() ? nullptr : legalTargets[0];
        if (closest) {
            float dx = closest->x - cue->x;
            float dy = closest->y - cue->y;
            safety.angle = atan2f(dy, dx);
            safety.power = 2.0f; // Soft defensive tap
            possibleShots.push_back(safety);
        }
    }
...
// ==================== STRAIGHT POOL AI ====================
// --- FULL REPLACEMENT: Find All Shots Straight Pool ---
std::vector<AIShotInfo> AIFindAllShots_StraightPool() {
    std::vector<AIShotInfo> possibleShots;
    Ball* cue = GetCueBall();
    if (!cue) return possibleShots;
    ...
    // THE PRO FILTER: Sort by highest score
    std::sort(possibleShots.begin(), possibleShots.end(), [](const AIShotInfo& a, const AIShotInfo& b) {
        return a.score > b.score;
        });
    ...
    if (possibleShots.empty()) {
        ...
        // In Straight Pool, any ball (1-15) is a legal safety target
        Ball* closest = nullptr;
        float minDist = 1e10f;
        for (Ball& b : balls) {
            if (b.id == 0 || b.isPocketed) continue;
            float d = GetDistanceSq(cue->x, cue->y, b.x, b.y);
            if (d < minDist) { minDist = d; closest = &b; }
        }

        if (closest) {
            safety.angle = atan2f(closest->y - cue->y, closest->x - cue->x);
            safety.power = 2.0f; // Soft defensive tap
            possibleShots.push_back(safety);
        }
    }
...
// --- FULL REPLACEMENT: Professional Nine-Ball Evaluation ---
// --- FULL REPLACEMENT: Formidable Nine-Ball Evaluation ---
AIShotInfo EvaluateShot_NineBall(Ball* targetBall, int pocketIndex, bool isBank, bool isCombo) {
    ...
    if (!isCombo && !IsPathClear({ cue->x, cue->y }, shotInfo.ghostBallPos, cue->id, targetBall->id)) return shotInfo;
    if (!isBank && !IsPathClear({ targetBall->x, targetBall->y }, pocketPos, targetBall->id, -1)) return shotInfo;
    ...
    float dot = (vCue.x * vObj.x + vCue.y * vObj.y) / (mC * mO);
    float cutAngle = acosf(std::clamp(dot, -1.0f, 1.0f));
    float cutProb = std::max(0.0f, 1.0f - (cutAngle / (PI * 0.5f)));

    // --- FORMIdable FIX: KNUCKLE PENALTY ---
    // In 9-Ball, we take the risk on the 9-ball, but play safe on others.
    float knucklePenalty = 0.0f;
    if (targetBall->id != 9 && cutAngle > 0.45f) {
        knucklePenalty = cutAngle * 450.0f;
    }

    // 4. SCORING HEURISTIC
    shotInfo.score = (cutProb * 1000.0f) - (mC * 0.1f) - (mO * 0.2f) - knucklePenalty;
    if (targetBall->id == 9) shotInfo.score += 5000.0f; // Absolute priority
    if (isCombo) shotInfo.score += 600.0f;
...
// ==================== 9-BALL AI ====================
// --- FULL REPLACEMENT: Find All Shots Nine-Ball (Combo Specialist) ---
std::vector<AIShotInfo> AIFindAllShots_NineBall() {
    std::vector<AIShotInfo> possibleShots;
    Ball* cue = GetCueBall();
    Ball* lowest = GetBallById(lowestBallOnTable);
    if (!cue || !lowest) return possibleShots;

    // 1. SEARCH: Direct Shots on Lowest Ball
    for (int p = 0; p < 6; ++p) {
        AIShotInfo cand = EvaluateShot_NineBall(lowest, p, false, false);
        if (cand.possible) possibleShots.push_back(cand);
    }

    // 2. SEARCH: 9-BALL COMBOS (Lowest Ball -> 9-Ball -> Pocket)
    // This is the "Pro Move" that wins games early.
    Ball* nineBall = GetBallById(9);
    if (nineBall && !nineBall->isPocketed && lowest->id != 9) {
        for (int p = 0; p < 6; ++p) {
            D2D1_POINT_2F pocketPos = pocketPositions[p];
            D2D1_POINT_2F ghostForNine = CalculateGhostBallPos(nineBall, pocketPos);

            // Can Lowest Ball hit the 9-Ball's ghost position?
            if (IsPathClear({ lowest->x, lowest->y }, ghostForNine, lowest->id, 9)) {
                D2D1_POINT_2F ghostForLowest = CalculateGhostBallPos(lowest, ghostForNine);
                // Can Cue Ball hit the Lowest Ball's ghost position?
                if (IsPathClear({ cue->x, cue->y }, ghostForLowest, cue->id, lowest->id)) {
                    AIShotInfo combo = EvaluateShot_NineBall(lowest, p, false, true);
                    combo.ghostBallPos = ghostForLowest;
                    combo.angle = atan2f(ghostForLowest.y - cue->y, ghostForLowest.x - cue->x);
                    combo.score += 5000.0f; // MASSIVE priority for 9-ball win
                    possibleShots.push_back(combo);
                }
            }
        }
    ...
    // 3. SEARCH: Banks on Lowest Ball
    if (aiDifficulty > EASY) AIFindBankShots(lowest, possibleShots);

    // 4. SORT & SAFETY FALLBACK
    std::sort(possibleShots.begin(), possibleShots.end(), [](const AIShotInfo& a, const AIShotInfo& b) {
        return a.score > b.score;
        });

    if (possibleShots.empty()) {
        // Professional "Snooker" Safety: Hide the cue ball behind another ball
        AIShotInfo safety;
        safety.possible = true;
        safety.score = -5000.0f;
        float dx = lowest->x - cue->x;
        float dy = lowest->y - cue->y;
        safety.angle = atan2f(dy, dx);
        safety.power = 2.5f; // Soft touch
        possibleShots.push_back(safety);
    }
...
void AIMakeDecision() {
    ...
    // --- STEP 3: NINE-BALL SMART PUSH-OUT ---
    if (currentGameType == GameType::NINE_BALL && isPushOutAvailable) {
        Ball* lowest = GetBallById(lowestBallOnTable);
        if (lowest) {
            // If the AI cannot see the lowest ball (snookered), perform a Pro Push-Out
            if (!IsPathClear({ cueBall->x, cueBall->y }, { lowest->x, lowest->y }, cueBall->id, lowest->id)) {
                isPushOutShot = true;
                isPushOutAvailable = false;

                // Professional Strategy: Push the ball to a neutral spot near the center 
                // to make the "return" shot difficult for the human player.
                aiPlannedShotDetails.angle = atan2f(RACK_POS_Y - cueBall->y, (TABLE_LEFT + TABLE_WIDTH / 2.0f) - cueBall->x);
                aiPlannedShotDetails.power = 3.0f; // Firm but controlled tap
                aiPlannedShotDetails.isValid = true;
                ...
                aiIsDisplayingAim = true;
                aiAimDisplayFramesLeft = AI_AIM_DISPLAY_DURATION_FRAMES;
                return; // Exit function early; the push-out is now the planned shot
            }
        }
    }
    // --- END PUSH-OUT ---
...
// [Location: Replace existing CalculateGhostBallPos function]
// --- REPLACED: Chamfer-Aware Ghost Ball Calculation ---
// Projects the target point deeper into the pocket throat to clear the V-cut rims.
// --- REPLACED: Chamfer-Aware Ghost Ball Calculation ---
// --- FULL REPLACEMENT: Professional Mouth-Clearance Targeting ---
D2D1_POINT_2F CalculateGhostBallPos(Ball* targetBall, D2D1_POINT_2F pocketPos) {
    if (!targetBall) return { 0, 0 };

    // 1. Get vector from Ball to Pocket center
    float dx = pocketPos.x - targetBall->x;
    float dy = pocketPos.y - targetBall->y;
    float distToPocket = sqrtf(dx * dx + dy * dy);

    if (distToPocket < 1e-4f) return { targetBall->x, targetBall->y };

    // 2. Normalize direction
    float dirX = dx / distToPocket;
    float dirY = dy / distToPocket;

    // 3. DYNAMIC PROJECTION: 
    // If the shot is long, we aim deeper to ensure it stays in. 
    // If the shot is short, we aim shallower to prevent exaggerated angles.
    float dynamicDepth = BALL_RADIUS * 1.15f;
    if (distToPocket > 300.0f) dynamicDepth = BALL_RADIUS * 1.4f;

    // The 'Deep Target' is inside the pocket throat
    D2D1_POINT_2F deepTarget = {
        pocketPos.x + dirX * dynamicDepth,
        pocketPos.y + dirY * dynamicDepth
    };

    // 4. Recalculate vector to the Deep Target
    float finalDx = deepTarget.x - targetBall->x;
    float finalDy = deepTarget.y - targetBall->y;
    float finalLen = sqrtf(finalDx * finalDx + finalDy * finalDy);

    // 5. GHOST BALL POSITION
    // Place the ghost ball exactly 2 radii away from the target center
    // along the line that leads perfectly into the throat.
    return {
        targetBall->x - (finalDx / finalLen) * (BALL_RADIUS * 2.0f),
        targetBall->y - (finalDy / finalLen) * (BALL_RADIUS * 2.0f)
    };
}
...
// --- REPLACED: Professional Capsule Pathfinding ---
// --- FULL REPLACEMENT: High-Fidelity Pathfinding with Rim-Buffer ---
bool IsPathClear(D2D1_POINT_2F start, D2D1_POINT_2F end, int ignoredBallId1, int ignoredBallId2) {
    // 1. BALL CLEARANCE: 5% safety margin to prevent grazing shots
    const float ballClearanceSq = (BALL_RADIUS * 1.05f) * (BALL_RADIUS * 1.05f);
    for (const Ball& b : balls) {
        if (b.isPocketed || b.id == ignoredBallId1 || b.id == ignoredBallId2) continue;
        if (PointToLineSegmentDistanceSq(D2D1::Point2F(b.x, b.y), start, end) < ballClearanceSq)
            return false;
    }

    // 2. RIM CLEARANCE: The AI must respect the 12 rail segments
    D2D1_POINT_2F vertices[12] = {
        { TABLE_LEFT + INNER_RIM_THICKNESS, TABLE_TOP + CHAMFER_SIZE }, { TABLE_LEFT + CHAMFER_SIZE, TABLE_TOP + INNER_RIM_THICKNESS },
        { TABLE_RIGHT - CHAMFER_SIZE, TABLE_TOP + INNER_RIM_THICKNESS }, { TABLE_RIGHT - INNER_RIM_THICKNESS, TABLE_TOP + CHAMFER_SIZE },
        { TABLE_RIGHT - INNER_RIM_THICKNESS, TABLE_BOTTOM - CHAMFER_SIZE }, { TABLE_RIGHT - CHAMFER_SIZE, TABLE_BOTTOM - INNER_RIM_THICKNESS },
    ...
    D2D1_POINT_2F segments[12][2] = {
        {vertices[1],vertices[2]}, {vertices[2],vertices[3]}, {vertices[3],vertices[11]}, {vertices[11],vertices[10]},
        {vertices[10],vertices[4]}, {vertices[4],vertices[5]}, {vertices[5],vertices[6]}, {vertices[6],vertices[7]},
        {vertices[7],vertices[9]}, {vertices[9],vertices[8]}, {vertices[8],vertices[0]}, {vertices[0],vertices[1]}
    };

    // Buffer: If the path is within 2 pixels of a rail, consider it blocked.
    // However, we IGNORE this if we are inside a pocket mouth.
    float safetyBufferSq = (BALL_RADIUS + 1.5f) * (BALL_RADIUS + 1.5f);

    for (int j = 0; j < 12; ++j) {
        float distSq = PointToLineSegmentDistanceSq(end, segments[j][0], segments[j][1]);
        if (distSq < safetyBufferSq) {
            // Check if this specific spot is a 'safe zone' (the actual hole)
            bool inSafeHole = false;
            for (int p = 0; p < 6; ++p) {
                float r = GetPocketVisualRadius(p);
                if (GetDistanceSq(end.x, end.y, pocketPositions[p].x, pocketPositions[p].y) < r * r) {
                    inSafeHole = true;
                    break;
                }
            }
            if (!inSafeHole) return false; // Hit a rim/knuckle
        }
    }
    return true;
}

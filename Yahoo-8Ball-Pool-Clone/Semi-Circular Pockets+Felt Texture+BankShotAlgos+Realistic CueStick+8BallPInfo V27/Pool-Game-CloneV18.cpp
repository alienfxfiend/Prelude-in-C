...
D2D1::ColorF(0.0f, 0.5f, 0.5f)/*(0.0f, 0.4f, 0.4392f)*/,       // Teal
        ...
        if (cheatModeEnabled && draggingBallId != -1) {
            ...
            if (b) {
                for (int p = 0; p < 6; ++p) {
                    ...
                    // FIXED: Use correct radius for cheat-mode pocketing
                    float pocketingRadius = (p == 1 || p == 4) ? MIDDLE_HOLE_VISUAL_RADIUS : CORNER_HOLE_VISUAL_RADIUS;
                    if (dx * dx + dy * dy <= pocketingRadius * pocketingRadius) {
                        b->isPocketed = true;
...
// --------------------------------------------------------------------------------
// DrawCueStick: tapered stick (thick butt, narrow tip), beige shaft, white tip, black butt
// --------------------------------------------------------------------------------
void DrawCueStick(ID2D1RenderTarget* pRT)
{
    // --- BEGIN: FIX CUE STICK RENDERING ---
    // This logic determines if the cue stick should be visible. It should only be drawn
    // during aiming states for a human player, or when the AI is visualizing its shot.
    // It should NOT be drawn during SHOT_IN_PROGRESS, BALL_IN_HAND, or GAME_OVER.
    bool shouldDrawStick = false;

    if (aiIsDisplayingAim) {
        // Always show the stick when the AI is telegraphing its shot.
        shouldDrawStick = true;
    }
    else if (currentPlayer == 1 || !isPlayer2AI) { // Only check for human players otherwise
    ...
    if (!shouldDrawStick) return; // Do not draw the stick in any other state.
    // --- END: FIX CUE STICK RENDERING ---

    Ball* cue = GetCueBall();
    if (!cue) return;
    const float tipLength = 30.0f;
    const float buttLength = 50.0f;
    const float totalLength = 200.0f;
    const float buttWidth = 8.0f;
    const float shaftWidth = 4.0f;
    float theta = cueAngle + PI;
    D2D1_POINT_2F base = D2D1::Point2F(
        cue->x + cosf(theta) * buttLength,
        cue->y + sinf(theta) * buttLength
    );
    D2D1_POINT_2F tip = D2D1::Point2F(
        cue->x + cosf(theta) * totalLength,
        cue->y + sinf(theta) * totalLength
    );
    ...
    D2D1_POINT_2F mid1 = { cue->x + cosf(theta) * (buttLength * 0.5f), cue->y + sinf(theta) * (buttLength * 0.5f) };
    ...
    D2D1_POINT_2F mid2 = { cue->x + cosf(theta) * (totalLength - tipLength), cue->y + sinf(theta) * (totalLength - tipLength) };
...
void DrawPocketSelectionIndicator(ID2D1RenderTarget* pRT) {
    ...
    float arrowHeadSize = currentPocketRadius * 0.5f;
    float arrowShaftLength = currentPocketRadius * 0.3f;
    float arrowShaftWidth = arrowHeadSize * 0.4f;
    float verticalOffsetFromPocketCenter = currentPocketRadius * 1.6f;

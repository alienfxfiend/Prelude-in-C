...
{
    // Defensive: pIndex must be 0..5
    if (pIndex < 0 || pIndex > 5) return false;

    // Pocket center
    D2D1_POINT_2F pc = pocketPositions[pIndex];

    // Choose a conservative inner radius for the black "hole" area.
    // These values control sensitivity — tweak if you want more/less forgiving.
    const float cornerInner = CORNER_HOLE_VISUAL_RADIUS * 0.55f; // corner inner black radius
    const float middleInner = MIDDLE_HOLE_VISUAL_RADIUS * 0.55f; // middle inner black radius
    ...
    // Ball treated as circle of radius BALL_RADIUS; we'll check overlap
    // between ball circle and inner pocket circle (or semicircle for middles).
    float inner = (pIndex == 1 || pIndex == 4) ? middleInner : cornerInner;
    float threshold = inner + BALL_RADIUS; // if center-to-center <= threshold => overlap

    if (distSq > threshold * threshold) {
        // Too far to overlap at all
        return false;
    }

    // Require ball to be on the *table side* of the pocket opening:
    // Top pockets are indices 0,1,2, bottom are 3,4,5 (y increases downward).
    bool topPocket = (pIndex == 0 || pIndex == 1 || pIndex == 2);
    if (topPocket) {
        // pocket is at TABLE_TOP; black semicircle extends *into* table (positive y).
        // So require ball center to be at or just below the pocket center (allow slight overlap).
        if (b.y < pc.y - BALL_RADIUS) return false; // ball is above the opening (outside)
    }
    else {
        // bottom pockets: pocket center at TABLE_BOTTOM; black semicircle extends upward (negative y)
        if (b.y > pc.y + BALL_RADIUS) return false; // ball is below the opening (outside)
    }

    // For middle pockets we also prefer a narrow horizontal window to avoid big-angle false positives.
    if (pIndex == 1 || pIndex == 4) {
        // require the ball to be reasonably centered horizontally over the middle pocket
        float maxXOffset = (middleInner + BALL_RADIUS) * 1.0f; // small tolerance
        if (fabs(dx) > maxXOffset) return false;
    }

    // Passed overlap + orientation tests -> treat as pocketed (touching the black area).
    return true;
}
        ...
        if (cheatModeEnabled && draggingBallId != -1) {
            ...
            if (b) {
                ...
                for (int p = 0; p < 6; ++p) {
                    ...
                    if (!IsBallInPocketBlackArea(*b, p)) continue;

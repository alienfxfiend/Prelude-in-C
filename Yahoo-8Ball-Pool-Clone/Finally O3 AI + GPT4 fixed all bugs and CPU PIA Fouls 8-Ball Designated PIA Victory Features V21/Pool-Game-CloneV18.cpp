...
void ProcessShotResults() {
    ...
    // --- CHEAT MODE BYPASS: count and clear, no foul! ---
    /*if (cheatModeEnabled) {
        PlayerInfo& shooter = (currentPlayer == 1 ? player1Info : player2Info);
        for (int id : pocketedThisTurn) {
            Ball* b = GetBallById(id);
            if (!b || b->id == 0 || b->id == 8) continue;
            // credit for your color
            if (b->type == shooter.assignedType)
                shooter.ballsPocketedCount++;
        }
        pocketedThisTurn.clear();
        foulCommitted = false;
        return;
    ...
    // --- Detect opening break shot; will clear after processing ---
    bool isBreakShot = isOpeningBreakShot;

    // --- Preliminary Analysis: detect what fell this turn ---
    // First, just identify what happened on the shot without changing any scores yet.
        // (fills cueBallPocketed, eightBallPocketed, and ownBallsPocketedThisTurn)
    for (int id : pocketedThisTurn) {
        if (id == 0) {
            ...
            Ball* b = GetBallById(id);
            if (b && b->type == shootingPlayer.assignedType) {
                ownBallsPocketedThisTurn++;
            }
        }
    }

    // --- EARLY BALL CREDIT: count each object?ball once per turn ---
    for (int id : pocketedThisTurn) {
        if (id == 0 || id == 8) continue;             // skip cue & 8?ball
        Ball* bb = GetBallById(id);
        if (!bb) continue;
        if (bb->type == player1Info.assignedType)     // credit correct owner
            player1Info.ballsPocketedCount++;
        else if (bb->type == player2Info.assignedType)
            player2Info.ballsPocketedCount++;
    }

    // --- Step 1: Foul detection (skip entirely if any object ball pocketed) ---
    bool turnFoul = false;
    // If you pocketed any object ball (ID ? 0), we do not foul.
    bool anyObjectPocketed = false;
    for (int id : pocketedThisTurn) {
        if (id != 0) { anyObjectPocketed = true; break; }
    }
    if (!anyObjectPocketed) {
        // 1) Scratch (cue into pocket)
        if (cueBallPocketed) {
            turnFoul = true;
        }
        else {
            // 2) Missed every ball
            Ball* fh = GetBallById(firstHitBallIdThisShot);
            if (!fh) {
                turnFoul = true;
            }
            else {
                // 3) Wrong?first?hit (once types assigned or on 8?ball roll)
                bool isBreakShot = (player1Info.assignedType == BallType::NONE
                    && player2Info.assignedType == BallType::NONE);
                bool mustHit8Next = (!isBreakShot
                    && shootingPlayer.ballsPocketedCount >= 7);
                if (!isBreakShot) {
                    if (mustHit8Next) {
                        if (fh->id != 8) turnFoul = true;
                    }
                    else if (shootingPlayer.assignedType != BallType::NONE) {
                        if (fh->type != shootingPlayer.assignedType)
                            turnFoul = true;
                    }
                }
            }
        }
        // 4) No rail after contact and no pocketed object => foul
        /*if (!turnFoul
            && cueHitObjectBallThisShot
            && !railHitAfterContact
            && !anyObjectPocketed)
        {
            turnFoul = true;
    ...
    if (isBreakShot) {
        // Never foul on the opening break
        foulCommitted = false;
    }

    // --- Step 1: Check for fouls based on PRE-SHOT state (skip entirely in Cheat Mode) ---
    /*bool isSeventhObjectShot = (shootingPlayer.ballsPocketedCount == 6 && !eightBallPocketed);
    bool turnFoul = false;
    if (!cheatModeEnabled) {
        if (!isSeventhObjectShot) {
            if (cueBallPocketed) {
                turnFoul = true;
            }
            else {
                ...
                if (!firstHit) {
                    turnFoul = true; // Hitting nothing is a foul.
                }
                else {
                    // Existing “first?hit” rules…
                    bool wasOnEightBall = (shootingPlayer.assignedType != BallType::NONE
                        && shootingPlayer.ballsPocketedCount >= 7);
                    if (player1Info.assignedType != BallType::NONE) {
                        if (wasOnEightBall) {
            ...
            // Existing “no rail” foul
            if (!turnFoul
                && cueHitObjectBallThisShot
                //&& !railHitAfterContact
                && pocketedThisTurn.empty()
                && ownBallsPocketedThisTurn == 0)
            {
                turnFoul = true;
            }
        }
    }
    // Always clear fouls under Cheat Mode
    foulCommitted = cheatModeEnabled ? false : turnFoul;*/

    // --- Step 1: Check for Fouls, except on the 7th legal object?ball shot ---
// If you're sinking your 7th ball (just before the 8?ball), never foul.
    /*bool isSeventhObjectShot = (shootingPlayer.ballsPocketedCount == 6 && !eightBallPocketed);
    bool turnFoul = false;
    if (!isSeventhObjectShot) {
        if (cueBallPocketed) {
    ...
    /* // --- Step 1: Check for Fouls based on PRE-SHOT state ---
    bool turnFoul = false;
    if (cueBallPocketed) {
        turnFoul = true;
    }
        ...
        Ball* firstHit = GetBallById(firstHitBallIdThisShot);
        if (!firstHit) {
            turnFoul = true; // Rule: Hitting nothing is a foul.
        }
        else {
            // Check for illegal first hit based on the rules as they were BEFORE this shot.
            // This is the key fix for the foul on the 7th ball.
    ...
    // Rule: If cue hit object but didn't hit a rail AFTER and no balls were pocketed, that's a foul.
    /*if (!turnFoul
        && cueHitObjectBallThisShot
        && !railHitAfterContact
        && pocketedThisTurn.empty()
        && ownBallsPocketedThisTurn == 0)    // only foul if YOU pocketed none
    {
        turnFoul = true;
    }*/
    //foulCommitted = turnFoul;    
    ...
    //temp disabled
    // --- BLANK?SHOT OR FOUL ?– Ball?in?hand for opponent and stop here ---
    if (foulCommitted) {
        SwitchTurns();
        RespawnCueBall(false);
        pocketedThisTurn.clear();
        return;  // bail out so no further sinking logic runs
    }

    // --- Step 2: Update Scores and Handle Game Over ---
        // --- Step 2: Handle turn continuation & first?ball assignment (no scoring here) ---
    if (!turnFoul) {
        if (ownBallsPocketedThisTurn > 0) {
            playerContinuesTurn = true;
        }

        // First legal pocket: assign SOLID/STRIPE *and* credit that ball
        if (player1Info.assignedType == BallType::NONE
            && !pocketedThisTurn.empty()
            && !cueBallPocketed)
        {
            for (int id : pocketedThisTurn) {
                ...
                if (b && b->type != BallType::EIGHT_BALL) {
                    AssignPlayerBallTypes(b->type);
                    // Credit that first ball immediately
                    if (currentPlayer == 1)
                        player1Info.ballsPocketedCount = 1;
                    else
                        player2Info.ballsPocketedCount = 1;
                    playerContinuesTurn = true;
                    break;
                }
            }
    ...
    /*
    if (!turnFoul) {
        player1Info.ballsPocketedCount += (currentPlayer == 1) ? ownBallsPocketedThisTurn : 0;
        player2Info.ballsPocketedCount += (currentPlayer == 2) ? ownBallsPocketedThisTurn : 0;
        if (ownBallsPocketedThisTurn > 0) {
            playerContinuesTurn = true;
        }

        if (player1Info.assignedType == BallType::NONE && !pocketedThisTurn.empty() && !cueBallPocketed) {
            for (int id : pocketedThisTurn) {
                Ball* b = GetBallById(id);
                if (b && b->type != BallType::EIGHT_BALL) {
                    AssignPlayerBallTypes(b->type);
                    player1Info.ballsPocketedCount = (b->type == player1Info.assignedType) ? 1 : 0;
                    player2Info.ballsPocketedCount = (b->type == player2Info.assignedType) ? 1 : 0;
                    break;
                }
            }
        }
    ...
    // --- 7th BALL SANK ? enter pocket?call state with default arrow ---
    if (ownBallsPocketedThisTurn > 0) {
        PlayerInfo& shooter = (currentPlayer == 1 ? player1Info : player2Info);
        if (shooter.ballsPocketedCount >= 7 && !eightBallPocketed) {
            // Clear any previous call so arrow appears freshly
            if (currentPlayer == 1) calledPocketP1 = -1;
            else                  calledPocketP2 = -1;
            currentGameState = (currentPlayer == 1)
                ? CHOOSING_POCKET_P1
                : CHOOSING_POCKET_P2;
            return; // suspend until pocket clicked
        }
    }

    /*// --- LEGAL 8?BALL ? WIN, not switch turn ---
    if (eightBallPocketed && !foulCommitted) {
        currentGameState = GAME_OVER;
        gameOverMessage = (currentPlayer == 1 ? player1Info.name : player2Info.name) + L" Wins!";
        calledPocketP1 = calledPocketP2 = -1;
        pocketedThisTurn.clear();
        return;
    }*/
    //revert to this temp
    // --- CRITICAL 8?BALL HANDLING: ignore any spurious fouls on the 8?ball ---
    /*if (eightBallPocketed) {
        // wipe out any foul for this precise moment
        foulCommitted = false;

        // Which pocket did we CALL?
        int called = (currentPlayer == 1 ? calledPocketP1 : calledPocketP2);

        // Only win if it actually fell into that same pocket
        if (lastEightBallPocketIndex == called) {
            currentGameState = GAME_OVER;
            gameOverMessage = (currentPlayer == 1
                ? player1Info.name
                : player2Info.name)
                + std::wstring(L" Wins!");
        }
        else {
            // Wrong?pocket: opponent wins
            currentGameState = GAME_OVER;
            gameOverMessage = (currentPlayer == 1
                ? player2Info.name
                : player1Info.name)
                + std::wstring(L" Wins! (8?Ball in wrong pocket)");
        }
        pocketedThisTurn.clear();
        return;
    }*/
    ...
    /*if (eightBallPocketed) {
        ...
        // Defensive: if we somehow never recorded a pocket, force loss
        if (lastEightBallPocketIndex == -1) {
            currentGameState = GAME_OVER;
            gameOverMessage = (currentPlayer == 1 ? player2Info.name : player1Info.name)
                + L" Wins! (8-Ball fell into unknown pocket)";
            return;
        }
        ...
        if (lastEightBallPocketIndex == called) {
            ...
            currentGameState = GAME_OVER;
            gameOverMessage = (currentPlayer == 1 ? player2Info.name : player1Info.name)
                + L" Wins! (8-Ball in wrong pocket)";
        }
        pocketedThisTurn.clear();
        return;
    }*/

    // --- CRITICAL 8?BALL HANDLING: compare called vs. actual pocket ---
    int called = (currentPlayer == 1 ? calledPocketP1 : calledPocketP2);
    int actual = lastEightBallPocketIndex; // set in CheckPockets()
    ...
    // --- Step 3: Final State Transition (strict legal?pocket check) ---
    if (foulCommitted) {
        // On foul, opponent gets ball?in?hand
        SwitchTurns();
        RespawnCueBall(false);
    }
    else if (ownBallsPocketedThisTurn > 0) {
        // You legally sank at least one of your balls — you keep the table
        PlayerInfo& shooter = (currentPlayer == 1 ? player1Info : player2Info);

        if (shooter.ballsPocketedCount >= 7) {
            // Delegate to the helper—handles both human and AI pocket calls
            CheckAndTransitionToPocketChoice(currentPlayer);
            // Helper will set:
            //   - CHOOSING_POCKET_P1 for human,
            //   - or immediately choose & return to PLAYER2_TURN for AI
            return;
        }
        /*if (shooter.ballsPocketedCount >= 7) {
            // --- ALWAYS enter pocket?call on 7th ball ---
            // Clear any prior call so we force a fresh selection
            if (currentPlayer == 1) calledPocketP1 = -1;
            else                  calledPocketP2 = -1;

            // Now put us in the pocket?call state
            currentGameState = (currentPlayer == 1)
                ? CHOOSING_POCKET_P1
                : CHOOSING_POCKET_P2;
            return;  // suspend further turn logic until click
        }*/
        else {
        ...
        // No balls and no foul ? turn passes
        SwitchTurns();
    }

    // --- Clear opening?break flag after first shot ---
    if (isBreakShot) {
        isOpeningBreakShot = false;
    }

    //may need to put this above clear-opening-break
    // --- Clear call arrow for the player whose turn just ended ---
    if (currentGameState == PLAYER1_TURN) {
        // We just gave Player?1 the table, so drop Player?2’s arrow
        calledPocketP2 = -1;
    }
    else if (currentGameState == PLAYER2_TURN) {
        // We just gave Player?2 the table, so drop Player?1’s arrow
        calledPocketP1 = -1;
    }
}
...
void CheckGameOverConditions(bool eightBallPocketed, bool cueBallPocketed)
{
    ...
    int callPocket = (currentPlayer == 1 ? calledPocketP1 : calledPocketP2);

    // Must have cleared 7 balls
    bool cleared7 = (shooter.ballsPocketedCount >= 7);
    // Must not have scratched
    bool noScratch = !cueBallPocketed;
    // Must have made a call
    bool callMade = (callPocket >= 0);

    // use the pocket we recorded in CheckPockets()
    int actualPocket = lastEightBallPocketIndex;

    /*// Determine which hole the 8?ball actually went into
    int actualPocket = -1;
    Ball* bEight = GetBallById(8);
    if (bEight) {
        for (int i = 0; i < 6; ++i) {
            float dx = bEight->x - pocketPositions[i].x;
            float dy = bEight->y - pocketPositions[i].y;
            if (dx * dx + dy * dy <= POCKET_RADIUS * POCKET_RADIUS * 1.5f) {
                actualPocket = i;
                break;
            }
        }
    ...
    /*// Get the actual pocket index from last pocket event
    int actualPocket = currentlyHoveredPocket;*/

    // Legal 8?ball win check
    if (cleared7 && noScratch && callMade && actualPocket == callPocket)
    {
        gameOverMessage = shooter.name + L" Wins!";
    }
    else
    {
        gameOverMessage = opponent.name + L" Wins! (8?Ball Pocketed Early)";
    }
...
void SwitchTurns() {
    ...
    /* ---------------------------------------------------------------
       If the PREVIOUS turn ended with a foul we must first give the
       incoming player BALL-IN-HAND and clear any pocket calls; if
       we immediately ask for a pocket-choice the state machine can
       get stuck (arrow shown while cue-ball not yet placed).
       --------------------------------------------------------------- */
    if (foulCommitted)
    {
        /*  clear any outstanding 8-ball “called pocket” information   */
        calledPocketP1 = -1;
        calledPocketP2 = -1;
        pocketCallMessage.clear();

        /*  give the new player ball-in-hand                           */
        if (currentPlayer == 1)
        {
            currentGameState = BALL_IN_HAND_P1;
            aiTurnPending = false;
        ...
        {
            currentGameState = BALL_IN_HAND_P2;
            aiTurnPending = isPlayer2AI;     // AI will auto-place
        }

        /*  this foul has been handled – reset the flag                */
        foulCommitted = false;
        return;              // skip pocket–choice for now
    }

    /*  normal (non-foul) turn switch                                 */
    CheckAndTransitionToPocketChoice(currentPlayer);
}
...
void CheckAndTransitionToPocketChoice(int playerID) {
    ...
    if (needsToCall) {
        if (playerID == 1) { // Human Player 1
            ...
            if (calledPocketP1 == -1) calledPocketP1 = 5; // Default to bottom-right
        }
        else { // Player 2
            if (isPlayer2AI) {
                ...
                if (calledPocketP2 == -1) calledPocketP2 = 5; // Default to bottom-right
            }
        }
    }
    else {
...
// --------------------------------------------------------------------------------
void AIMakeDecision() {
    aiPlannedShotDetails.isValid = false; // Always start with a clean plan.
    Ball* cueBall = GetCueBall();
    ...
    if (bestShot.possible) {
        ...
        if (bestShot.involves8Ball) {
            //  ??? Call exactly the pocket we're about to shoot into ???
            calledPocketP2 = bestShot.pocketIndex;
        }
        /*// A good shot was found.
        if (bestShot.involves8Ball) {
            // THE CRITICAL FIX: The AI officially "calls" the pocket it's aiming for.
                // 1) Remember which pocket we're "calling"
            calledPocketP2 = bestShot.pocketIndex;

            // 2) Force the UI into the pocket?selection state
            currentGameState = CHOOSING_POCKET_P2;

            // 3) Clear any lingering hover
            currentlyHoveredPocket = -1;

            // 4) Immediately redraw so the arrow appears for one frame
            InvalidateRect(hwndMain, NULL, FALSE);

            // 5) Now we can go back to AIMING and let our “AI click” fire
            currentGameState = AIMING;
        }*/
        // Commit to the shot plan.
        aiPlannedShotDetails.angle = bestShot.angle;
        aiPlannedShotDetails.power = bestShot.power;
        aiPlannedShotDetails.spinX = 0.0f; // AI plays center ball for now
        aiPlannedShotDetails.spinY = 0.0f;
        aiPlannedShotDetails.isValid = true;
    }
    else {
        ...
        if (IsPlayerOnEightBall(2)) {
            ballToNudge = GetBallById(8);
            // Even on a safety, the AI must call a pocket. Default to one far away.
            calledPocketP2 = 5;
        }
        else {
            // Add logic here to find the closest of your own balls to tap safely.
            // For now, we'll just handle the 8-ball case.
        }
    ...
    // If any valid plan was made, prepare to show the aim and shoot.
    if (aiPlannedShotDetails.isValid) {
        cueAngle = aiPlannedShotDetails.angle;
        shotPower = aiPlannedShotDetails.power;
        aiIsDisplayingAim = true;
...
AIShotInfo AIFindBestShot() {
    ...
    bool mustShoot8Ball = IsPlayerOnEightBall(2);

    for (auto& potentialTarget : balls) {
        if (potentialTarget.isPocketed || potentialTarget.id == 0) continue;

        // Determine if this ball is a valid target for this turn
        bool isValidTarget = false;
        if (mustShoot8Ball) {
            // 8-BALL MODE: The only valid target is the 8-ball.
            isValidTarget = (potentialTarget.id == 8);
        }
        else {
            // NORMAL MODE: The only valid targets are the AI's own assigned balls.
            isValidTarget = (potentialTarget.type == player2Info.assignedType && potentialTarget.id != 8);
        }

        if (!isValidTarget) continue;

        // Evaluate shooting this valid target into all 6 pockets
        for (int p = 0; p < 6; ++p) {
            AIShotInfo currentShot = EvaluateShot(&potentialTarget, p);
            if (currentShot.possible) {
                // A shot is only truly viable if the first ball it hits is the one we're aiming for.
                float firstHitDistSq;
                Ball* firstHit = FindFirstHitBall(D2D1::Point2F(cueBall->x, cueBall->y), currentShot.angle, firstHitDistSq);

                if (firstHit && firstHit->id == currentShot.targetBall->id) {
                    // This is a legal, viable shot. Is it better than the best one we've found so far?

...
void ShowNewGameDialog(HINSTANCE hInstance) {
    if (DialogBoxParam(hInstance, MAKEINTRESOURCE(IDD_NEWGAMEDLG), hwndMain, NewGameDialogProc, 0) == IDOK) {
        ...
        if (g_isDemoMode) {
            player1Info.name = L"Johnny Cache";
...
D2D1_POINT_2F CalculateGhostBallPos(Ball* targetBall, D2D1_POINT_2F pocketPos) {
    ...
    // 1. Calculate the vector from the target ball to the pocket center
    ...
    // 2. [FIX] Apply "Throat Depth" Projection
    // Instead of aiming at the rail line (pocketPos), we aim slightly *past* it.
    // This forces the angle calculation to be sharper, avoiding the chamfer knuckles.
    // 1.5f * BALL_RADIUS ensures the ball center crosses the threshold fully.
    float throatDepth = BALL_RADIUS * 1.5f;

    float deepTargetX = pocketPos.x + dirX * throatDepth;
    float deepTargetY = pocketPos.y + dirY * throatDepth;

    // 3. Recalculate vector to the new Deep Target
    float deepDx = deepTargetX - targetBall->x;
    float deepDy = deepTargetY - targetBall->y;
    float deepLen = sqrtf(deepDx * deepDx + deepDy * deepDy);

    // 4. Calculate Ghost Ball Position
    // The ghost ball must be exactly 2.0 radii away from the target ball 
    // along the line to the DEEP target.
    return {
        targetBall->x + (deepDx / deepLen) * (BALL_RADIUS * 2.0f),
        targetBall->y + (deepDy / deepLen) * (BALL_RADIUS * 2.0f)
    };
}

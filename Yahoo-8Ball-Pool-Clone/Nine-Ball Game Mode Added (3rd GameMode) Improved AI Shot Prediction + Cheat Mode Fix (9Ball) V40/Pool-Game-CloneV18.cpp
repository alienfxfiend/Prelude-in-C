...
INT_PTR CALLBACK NewGameDialogProc(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam) {
    ...
    bool enableAIOptions = isCPUGame; // AI options enabled only if Human vs CPU
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_KEYDOWN:
    {
        ...
        if (wParam == VK_F2) {
            ...
            MessageBox(hwnd,
                L"Direct2D-based StickPool game made in C++ from scratch (10192+ lines of code)\n"
                ...
                L"About This Game", MB_OK | MB_ICONINFORMATION); /*4827+ 8903+ 10192+*/
            return 0;
        }
...
void InitGame() {
    ...
    if (currentGameType == GameType::NINE_BALL) {
        ...
        std::random_shuffle(objectBalls.begin(), objectBalls.end()); // Shuffle balls 2-8
...
void ProcessShotResults() {
    ...
    for (int id : pocketedThisTurn) {
        ...
        if (b->id == 0) {
            ...
            if (currentGameType != GameType::NINE_BALL) ballsOnTableCount--; // Decrement balls on table count (9-ball handles this differently)

            // Score for Straight Pool
            if (currentGameType == STRAIGHT_POOL) {
                if (currentPlayer == 1) player1StraightPoolScore++;
                else player2StraightPoolScore++;
                playerContinuesTurn = true; // Player continues in Straight Pool if they pocket any numbered ball
            }
...
void AIFindBankShots(Ball* targetBall, std::vector<AIShotInfo>& possibleShots) {
    // ... (Keep the geometry calculation for virtual pockets and bounce points) ...
    if (!targetBall) return;
    Ball* cue = GetCueBall();
    if (!cue) return;
    // ... (railLines definition) ...
    ...
    for (int p = 0; p < 6; ++p) {
        ...
        for (int r = 0; r < 4; ++r) {
            // ... (virtualPocket calculation) ...
            D2D1_POINT_2F p1 = railLines[r][0];
            D2D1_POINT_2F virtualPocket;
            if (r < 2) virtualPocket = { pocketPos.x, 2 * p1.y - pocketPos.y };
            else virtualPocket = { 2 * p1.x - pocketPos.x, pocketPos.y };

            D2D1_POINT_2F bouncePoint;
            if (!LineSegmentIntersection(targetPos, virtualPocket, railLines[r][0], railLines[r][1], bouncePoint)) continue;
            // ... (bouncePointIsSafe check) ...
            bool bouncePointIsSafe = true; /* ... check distance to pockets ... */
                ...
                IsPathClear(bouncePoint, pocketPos, targetBall->id, -1))
            {
                ...
                AIShotInfo bankCand;
                if (currentGameType == GameType::EIGHT_BALL_MODE) {
                    bankCand = EvaluateShot_8Ball(targetBall, p, true); // Use 8-ball evaluation
                    if (bankCand.possible) bankCand.score += 150.0f; // 8-ball bank bonus
                }
                else { // STRAIGHT_POOL
                    bankCand = EvaluateShot_StraightPool(targetBall, p, true); // Use Straight Pool evaluation
                    // No specific bank bonus needed here, already penalized in scoring
                }
...
float CalculateShotPower(float cueToGhostDist, float targetToPocketDist, bool isBank) {
    ...
    float target_leg_power = 2.5f * sqrtf(targetToPocketDist); // FIX: Use sqrtf for float operations

    // Part 2: Power needed for the cue ball to reach the target with enough energy left to transfer.
    float cue_leg_power = 2.0f * sqrtf(cueToGhostDist); // FIX: Use sqrtf for float operations
...
AIShotInfo EvaluateShot_8Ball(Ball* targetBall, int pocketIndex, bool isBank) {
    AIShotInfo shotInfo;
    shotInfo.possible = false; // Assume not possible until proven otherwise
    ...
    // --- FOOLPROOF AIMING FIX: Dynamic Target Selection for Cut Shots ---
// This new logic aims for the "far side" of the pocket opening, which is crucial for
// making angled shots and avoids hitting the near-side "horn" of the pocket.

    D2D1_POINT_2F pocketPos = pocketPositions[pocketIndex];
    D2D1_POINT_2F targetPos = { targetBall->x, targetBall->y };
    D2D1_POINT_2F cuePos = { cue->x, cue->y };

    // 1. Define the two "jaw" points for the target pocket's opening.
    const float jawOffset = GetPocketVisualRadius(pocketIndex) - 4.0f; // A point near the edge
    D2D1_POINT_2F jaw1, jaw2;
    if (pocketIndex == 1 || pocketIndex == 4) { // Middle pockets
        jaw1 = { pocketPos.x - jawOffset, TABLE_TOP + INNER_RIM_THICKNESS };
        jaw2 = { pocketPos.x + jawOffset, TABLE_TOP + INNER_RIM_THICKNESS };
        if (pocketIndex == 4) { // Bottom middle pocket
            jaw1.y = TABLE_BOTTOM - INNER_RIM_THICKNESS;
            jaw2.y = TABLE_BOTTOM - INNER_RIM_THICKNESS;
        }
    }
    else { // Corner pockets
        if (pocketIndex == 0) { // Top-left
            jaw1 = { TABLE_LEFT + INNER_RIM_THICKNESS, TABLE_TOP + jawOffset };
            jaw2 = { TABLE_LEFT + jawOffset, TABLE_TOP + INNER_RIM_THICKNESS };
        }
        else if (pocketIndex == 2) { // Top-right
            jaw1 = { TABLE_RIGHT - jawOffset, TABLE_TOP + INNER_RIM_THICKNESS };
            jaw2 = { TABLE_RIGHT - INNER_RIM_THICKNESS, TABLE_TOP + jawOffset };
        }
        else if (pocketIndex == 3) { // Bottom-left
            jaw1 = { TABLE_LEFT + jawOffset, TABLE_BOTTOM - INNER_RIM_THICKNESS };
            jaw2 = { TABLE_LEFT + INNER_RIM_THICKNESS, TABLE_BOTTOM - jawOffset };
        }
        else { // Bottom-right (pocketIndex == 5)
            jaw1 = { TABLE_RIGHT - INNER_RIM_THICKNESS, TABLE_BOTTOM - jawOffset };
            jaw2 = { TABLE_RIGHT - jawOffset, TABLE_BOTTOM - INNER_RIM_THICKNESS };
        }
    }

    // 2. Determine which jaw is the "far jaw" relative to the cue ball's position.
    // Aiming for the far jaw provides the largest margin for error on cut shots.
    float distToJaw1Sq = GetDistanceSq(cuePos.x, cuePos.y, jaw1.x, jaw1.y);
    float distToJaw2Sq = GetDistanceSq(cuePos.x, cuePos.y, jaw2.x, jaw2.y);
    D2D1_POINT_2F farJaw = (distToJaw1Sq > distToJaw2Sq) ? jaw1 : jaw2;

    // 3. The final aim target is slightly inside the far jaw, towards the pocket center.
    float aimShiftX = pocketPos.x - farJaw.x;
    float aimShiftY = pocketPos.y - farJaw.y;
    float shiftLen = sqrtf(aimShiftX * aimShiftX + aimShiftY * aimShiftY);
    if (shiftLen > 1e-6f) {
        aimShiftX /= shiftLen;
        aimShiftY /= shiftLen;
    }
    D2D1_POINT_2F aimTargetPos = {
        farJaw.x + aimShiftX * BALL_RADIUS * 1.2f, // Nudge it slightly deeper into the pocket
        farJaw.y + aimShiftY * BALL_RADIUS * 1.2f
    };
    // --- END FOOLPROOF AIMING FIX ---

    /*// Use the reliable aiming logic of aiming deeper into the pocket
    D2D1_POINT_2F pocketPos = pocketPositions[pocketIndex];
    D2D1_POINT_2F targetPos = D2D1::Point2F(targetBall->x, targetBall->y);
    float shotLineX = pocketPos.x - targetPos.x, shotLineY = pocketPos.y - targetPos.y;
    float len = sqrtf(shotLineX * shotLineX + shotLineY * shotLineY);
    if (len > 1e-6f) { shotLineX /= len; shotLineY /= len; }
    D2D1_POINT_2F aimTargetPos = { pocketPos.x - shotLineX * (BALL_RADIUS * 0.5f), pocketPos.y - shotLineY * (BALL_RADIUS * 0.5f) };*/

    D2D1_POINT_2F ghostBallPos = CalculateGhostBallPos(targetBall, aimTargetPos);

    if (!IsPathClear(D2D1::Point2F(cue->x, cue->y), ghostBallPos, cue->id, targetBall->id)) return shotInfo;
    if (!isBank && !IsPathClear(targetPos, aimTargetPos, targetBall->id, -1)) return shotInfo;

    // --- NEW: Calculate Cut Angle to determine shot difficulty ---
    D2D1_POINT_2F cueToGhostVec = { ghostBallPos.x - cue->x, ghostBallPos.y - cue->y };
    D2D1_POINT_2F targetToPocketVec = { aimTargetPos.x - targetPos.x, aimTargetPos.y - targetPos.y };

    float lenCue = sqrtf(cueToGhostVec.x * cueToGhostVec.x + cueToGhostVec.y * cueToGhostVec.y);
    float lenTarget = sqrtf(targetToPocketVec.x * targetToPocketVec.x + targetToPocketVec.y * targetToPocketVec.y);

    if (lenCue < 1e-6f || lenTarget < 1e-6f) return shotInfo; // Avoid division by zero

    // Dot product gives cosine of angle between vectors. -1 is straight, 0 is 90-degree cut.
    float dot = (cueToGhostVec.x * targetToPocketVec.x + cueToGhostVec.y * targetToPocketVec.y) / (lenCue * lenTarget);
    dot = std::clamp(dot, -1.0f, 1.0f);

    // Convert dot product to a 'straightness' factor from 0.0 (hard cut) to 1.0 (perfectly straight)
    float straightness = (dot + 1.0f) / 2.0f;

    // --- Score Calculation ---
    float cueToGhostDist = lenCue;
    float targetToPocketDist = lenTarget;
    float distanceScore = 2000.0f - (cueToGhostDist + targetToPocketDist);

    // Heavily penalize shots that are not straight
    shotInfo.score = distanceScore * (0.1f + 0.9f * powf(straightness, 2));

    if (isBank) shotInfo.score -= 500.0f; // Banks are inherently risky

    // Positional play and spin is now simplified to not interfere with core scoring
    float leaveScore = EvaluateTableState(2, cue); // Evaluate leave from current position
    shotInfo.score += leaveScore;

    shotInfo.power = CalculateShotPower(cueToGhostDist, targetToPocketDist, isBank);
    shotInfo.angle = atan2f(cueToGhostVec.y, cueToGhostVec.x);
    shotInfo.spinX = 0.0f; // Keep spin simple for reliability
    shotInfo.spinY = 0.0f;
...
AIShotInfo EvaluateShot_StraightPool(Ball* targetBall, int pocketIndex, bool isBank) {
    AIShotInfo shotInfo;
    shotInfo.possible = false; // Assume not possible
    shotInfo.targetBall = targetBall;
    shotInfo.pocketIndex = pocketIndex;
    shotInfo.isBankShot = isBank;
    shotInfo.involves8Ball = false; // Not relevant

    Ball* cue = GetCueBall();
    if (!cue || !targetBall) return shotInfo;

    // Use the same reliable aiming logic (aim slightly inside pocket)
    D2D1_POINT_2F pocketPos = pocketPositions[pocketIndex];
    D2D1_POINT_2F targetPos = { targetBall->x, targetBall->y };
    float shotLineX = pocketPos.x - targetPos.x, shotLineY = pocketPos.y - targetPos.y;
    float len = sqrtf(shotLineX * shotLineX + shotLineY * shotLineY);
    if (len > 1e-6f) { shotLineX /= len; shotLineY /= len; }
    D2D1_POINT_2F aimTargetPos = { pocketPos.x - shotLineX * (BALL_RADIUS * 0.75f), pocketPos.y - shotLineY * (BALL_RADIUS * 0.75f) };
    D2D1_POINT_2F ghostBallPos = CalculateGhostBallPos(targetBall, aimTargetPos);


    // Path Clearing Checks
    ...
    float targetToPocketDist = GetDistance(targetPos.x, targetPos.y, aimTargetPos.x, aimTargetPos.y);
    shotInfo.angle = atan2f(ghostBallPos.y - cue->y, ghostBallPos.x - cue->x);
    shotInfo.power = CalculateShotPower(cueToGhostDist, targetToPocketDist, isBank); // Reuse power calculation

    // Scoring: Base score on difficulty, add positional score
    shotInfo.score = 1500.0f - (cueToGhostDist + targetToPocketDist * 1.2f);
    if (isBank) shotInfo.score -= 500.0f; // Penalize banks

    // Simplified Positional Play: Evaluate leave without complex spin simulation
    // Predict cue ball ending near ghost ball (stun shot approximation)
    float perpAngle = shotInfo.angle + PI / 2.0f;
    float cueTravelDist = shotInfo.power * 3.0f; // Rough estimate
    D2D1_POINT_2F cueEndPos = {
         ghostBallPos.x + cosf(perpAngle) * cueTravelDist * 0.2f,
         ghostBallPos.y + sinf(perpAngle) * cueTravelDist * 0.2f
    };
    Ball tempCue = *cue; tempCue.x = cueEndPos.x; tempCue.y = cueEndPos.y;
    float leaveScore = EvaluateTableState_StraightPool(&tempCue); // Need a Straight Pool version
    // Penalize scratches
    bool scratchPredicted = false;
    for (int p = 0; p < 6; ++p) {
        if (IsBallInPocketBlackArea(tempCue, p)) { // Reuse 8-ball helper
            leaveScore -= 5000.0f; scratchPredicted = true; break;
        }
    }

    shotInfo.score += leaveScore * 0.6f; // Add positional score, weighted slightly less maybe
    shotInfo.spinX = 0; // Keep spin simple
    shotInfo.spinY = 0;
...
float EvaluateTableState_StraightPool(Ball* cueBall) {
    ...
    score += potentialShots * 20.0f; // Bonus for just having options available
    return score;
}
...
AIShotInfo EvaluateShot_NineBall(Ball* targetBall, int pocketIndex, bool isBank, bool isCombo) {
    ...
    shotInfo.pocketIndex = pocketIndex;
    shotInfo.isBankShot = isBank;
    shotInfo.involves8Ball = (targetBall && targetBall->id == 9); // Mark if it's a 9-ball shot
    ...
    // For Combos, the aim target isn't the pocket, it's the 9-ball.
    // This is a simplified "target" combo. A true combo eval is much harder.
    // Let's assume for now `targetBall` is the ball being pocketed.
    // If `isCombo` is true, it means we are *hitting* the lowest ball to pocket `targetBall`.
    // This function structure needs clarification.

    // Let's assume this function evaluates a shot to pocket `targetBall`
    // and `AIFindBestShot_NineBall` handles *which* ball is the legal target.

    // Aiming logic (same as Straight Pool)
    /*float shotLineX = pocketPos.x - targetPos.x, shotLineY = pocketPos.y - targetPos.y;
    float len = sqrtf(shotLineX * shotLineX + shotLineY * shotLineY);
    if (len > 1e-6f) { shotLineX /= len; shotLineY /= len; }
    D2D1_POINT_2F aimTargetPos = { pocketPos.x - shotLineX * (BALL_RADIUS * 0.75f), pocketPos.y - shotLineY * (BALL_RADIUS * 0.75f) }; */
    ...
    // --- UPGRADED AIMING: Use "Far Jaw" logic from 8-Ball AI ---
    const float jawOffset = GetPocketVisualRadius(pocketIndex) - 4.0f;
    D2D1_POINT_2F jaw1, jaw2;
    if (pocketIndex == 1 || pocketIndex == 4) { // Middle pockets
        jaw1 = { pocketPos.x - jawOffset, TABLE_TOP + INNER_RIM_THICKNESS };
        jaw2 = { pocketPos.x + jawOffset, TABLE_TOP + INNER_RIM_THICKNESS };
        if (pocketIndex == 4) { jaw1.y = TABLE_BOTTOM - INNER_RIM_THICKNESS; jaw2.y = TABLE_BOTTOM - INNER_RIM_THICKNESS; }
    }
    else { // Corner pockets
        if (pocketIndex == 0) { jaw1 = { TABLE_LEFT + INNER_RIM_THICKNESS, TABLE_TOP + jawOffset }; jaw2 = { TABLE_LEFT + jawOffset, TABLE_TOP + INNER_RIM_THICKNESS }; }
        else if (pocketIndex == 2) { jaw1 = { TABLE_RIGHT - jawOffset, TABLE_TOP + INNER_RIM_THICKNESS }; jaw2 = { TABLE_RIGHT - INNER_RIM_THICKNESS, TABLE_TOP + jawOffset }; }
        else if (pocketIndex == 3) { jaw1 = { TABLE_LEFT + jawOffset, TABLE_BOTTOM - INNER_RIM_THICKNESS }; jaw2 = { TABLE_LEFT + INNER_RIM_THICKNESS, TABLE_BOTTOM - jawOffset }; }
        else { jaw1 = { TABLE_RIGHT - INNER_RIM_THICKNESS, TABLE_BOTTOM - jawOffset }; jaw2 = { TABLE_RIGHT - jawOffset, TABLE_BOTTOM - INNER_RIM_THICKNESS }; }
    }
    float distToJaw1Sq = GetDistanceSq(cuePos.x, cuePos.y, jaw1.x, jaw1.y);
    float distToJaw2Sq = GetDistanceSq(cuePos.x, cuePos.y, jaw2.x, jaw2.y);
    D2D1_POINT_2F farJaw = (distToJaw1Sq > distToJaw2Sq) ? jaw1 : jaw2;
    ...
    float aimShiftY = pocketPos.y - farJaw.y;
    float shiftLen = sqrtf(aimShiftX * aimShiftX + aimShiftY * aimShiftY);
    if (shiftLen > 1e-6f) { aimShiftX /= shiftLen; aimShiftY /= shiftLen; }
    D2D1_POINT_2F aimTargetPos = {
        farJaw.x + aimShiftX * BALL_RADIUS * 1.2f,
        farJaw.y + aimShiftY * BALL_RADIUS * 1.2f
    };
    // --- END UPGRADED AIMING ---
    D2D1_POINT_2F ghostBallPos = CalculateGhostBallPos(targetBall, aimTargetPos);
    ...
    // Path Clearing
    // This is tricky for combos. If `isCombo` is true, we must check cue->lowest AND lowest->targetBall.
    // This function isn't set up for that. Let's simplify:
    // `EvaluateShot_NineBall` ONLY evaluates a *direct* shot on a ball.
    // `AIFindBestShot_NineBall` will handle checking if it's a legal combo.
    ...
    // If this is a combo, we only check the ball-to-pocket path.
// The cue-to-lowest and lowest-to-target is checked in AIFindBestShot_NineBall.
    if (isCombo) {
        if (!isBank && !IsPathClear(targetPos, aimTargetPos, targetBall->id, -1)) return shotInfo;
    }
    else {
        // If direct shot, check both paths
        if (!IsPathClear({ cue->x, cue->y }, ghostBallPos, cue->id, targetBall->id)) return shotInfo;
        if (!isBank && !IsPathClear(targetPos, aimTargetPos, targetBall->id, -1)) return shotInfo;
    }

    // --- Calculate Cut Angle (from 8-Ball AI) ---
    D2D1_POINT_2F cueToGhostVec = { ghostBallPos.x - cue->x, ghostBallPos.y - cue->y };
    D2D1_POINT_2F targetToPocketVec = { aimTargetPos.x - targetPos.x, aimTargetPos.y - targetPos.y };
    float lenCue = sqrtf(cueToGhostVec.x * cueToGhostVec.x + cueToGhostVec.y * cueToGhostVec.y);
    float lenTarget = sqrtf(targetToPocketVec.x * targetToPocketVec.x + targetToPocketVec.y * targetToPocketVec.y);
    if (lenCue < 1e-6f || lenTarget < 1e-6f) return shotInfo;
    float dot = (cueToGhostVec.x * targetToPocketVec.x + cueToGhostVec.y * targetToPocketVec.y) / (lenCue * lenTarget);
    dot = std::clamp(dot, -1.0f, 1.0f);
    float straightness = (dot + 1.0f) / 2.0f; // 1.0 = straight, 0.0 = 90-degree cut
    // --- End Cut Angle ---
    ...
    //shotInfo.score = 1500.0f - (cueToGhostDist + targetToPocketDist * 1.2f);
    float distanceScore = 2000.0f - (cueToGhostDist + targetToPocketDist);
    // Heavily penalize thin cuts, just like in 8-ball
    shotInfo.score = distanceScore * (0.1f + 0.9f * powf(straightness, 2));
    if (isCombo) shotInfo.score += 500.0f; // Bonus for making a combo

        // Bonus for pocketing the *lowest* ball (good play)
    if (targetBall->id == lowestBallOnTable) {
        shotInfo.score += 1000.0f;
    }

    // BIG bonus for pocketing the 9-ball (a winning shot)
    if (targetBall->id == 9) {
        //shotInfo.score += 10000.0f;
    //}

    // TODO: Add positional leave score (e.g., EvaluateTableState_NineBall)
        shotInfo.score += 20000.0f; // Win the game
    }

    // --- IMPLEMENT TODO: Add positional leave score ---
    // Predict cue ball ending position (simplified stun shot model)
    float perpAngle = shotInfo.angle + PI / 2.0f;
    float cueTravelDist = shotInfo.power * 2.0f;
    D2D1_POINT_2F cueEndPos = {
         ghostBallPos.x + cosf(perpAngle) * cueTravelDist * 0.2f,
         ghostBallPos.y + sinf(perpAngle) * cueTravelDist * 0.2f
    };
    Ball tempCue = *cue; tempCue.x = cueEndPos.x; tempCue.y = cueEndPos.y;
    float leaveScore = EvaluateTableState_NineBall(&tempCue);

    // Penalize scratches
    for (int p = 0; p < 6; ++p) {
        if (IsBallInPocketBlackArea(tempCue, p)) {
            leaveScore -= 10000.0f; // Huge penalty for scratch
            break;
        }
    }
    shotInfo.score += leaveScore; // Add the positional score
    // --- END TODO ---


    shotInfo.spinX = 0; shotInfo.spinY = 0;
    shotInfo.possible = true;
    return shotInfo;
}

// --- NEW: 9-Ball AI Top-Level Decision ---
AIShotInfo AIFindBestShot_NineBall() {
    ...
    if (isPushOutAvailable) {
        ...
        if (canSeeTarget) {
            for (int p = 0; p < 6; ++p) {
                ...
                if (cand.possible && cand.score > 1000.0f) { // 1000 is an arbitrary "good shot" threshold
                    hasGoodShot = true;
    ...
    // 2. Evaluate combos: Hit lowest ball (targetBall) to pocket another ball
    for (Ball& otherBall : balls) {
        if (otherBall.isPocketed || otherBall.id == 0 || otherBall.id == targetBall->id) continue;

        for (int p = 0; p < 6; ++p) {
            // Can we pocket `otherBall` in pocket `p`?
            D2D1_POINT_2F pocketPos = pocketPositions[p];
            D2D1_POINT_2F targetPos = { otherBall.x, otherBall.y };
            // Where does the lowest ball need to hit `otherBall`?
            D2D1_POINT_2F ghostPosForCombo = CalculateGhostBallPos(&otherBall, pocketPos);

            // Is path clear from lowest ball to that spot?
            if (!IsPathClear({ targetBall->x, targetBall->y }, ghostPosForCombo, targetBall->id, otherBall.id)) continue;

            // Where does cue need to hit lowest ball to send it there?
            D2D1_POINT_2F ghostPosForCue = CalculateGhostBallPos(targetBall, ghostPosForCombo);

            // Is path clear from cue to *that* spot?
            if (!IsPathClear({ cue->x, cue->y }, ghostPosForCue, cue->id, targetBall->id)) continue;

            // This is a possible combo. Evaluate it.
            AIShotInfo cand = EvaluateShot_NineBall(&otherBall, p, false, true); // Evaluate pocketing `otherBall`
            if (cand.possible) {
                // Re-calculate angle/power for the *actual* shot (cue -> lowest ball)
                cand.angle = atan2f(ghostPosForCue.y - cue->y, ghostPosForCue.x - cue->x);
                float d1 = GetDistance(cue->x, cue->y, ghostPosForCue.x, ghostPosForCue.y);
                float d2 = GetDistance(targetBall->x, targetBall->y, ghostPosForCombo.x, ghostPosForCombo.y);
                float d3 = GetDistance(otherBall.x, otherBall.y, pocketPos.x, pocketPos.y);
                cand.power = CalculateShotPower(d1, d2 + d3, false);
                cand.score += 500.0f; // Add bonus for a combo
                if (otherBall.id == 9) cand.score += 20000.0f; // HUGE bonus for 9-ball combo
                possibleShots.push_back(cand);
            }
        }
    }
    ...
    if (aiDifficulty > EASY) {
        AIFindBankShots(targetBall, possibleShots); // This will use EvaluateShot_NineBall
    }
    ...
    if (!possibleShots.empty()) {
        ...
        // 5. No shots found, play safety
        // (Logic is same as Straight Pool safety)
        bestShot.possible = true;
        bestShot.score = -99999.0f;
        bestShot.spinX = 0; bestShot.spinY = 0; bestShot.pocketIndex = -1;
...
D2D1_POINT_2F CalculateGhostBallPos(Ball* targetBall, D2D1_POINT_2F pocketPos) {
    ...
    // Vector from the target ball's center to the intended pocket/target.
    float dirX = pocketPos.x - targetCenter.x;
    float dirY = pocketPos.y - targetCenter.y;
    float len = sqrtf(dirX * dirX + dirY * dirY);

    if (len < 1e-6f) { // Avoid division by zero if target is on top of pocket.
        return targetCenter;
    }

    // Normalize the direction vector. This is the path the target ball will take.
    dirX /= len;
    dirY /= len;

    // The ghost ball's center is exactly 2 * BALL_RADIUS away from the target ball's center,
    // in the opposite direction of its intended travel. This is the key physical principle.
    float ghostX = targetCenter.x - dirX * (BALL_RADIUS * 2.0f);
    float ghostY = targetCenter.y - dirY * (BALL_RADIUS * 2.0f);
...
void DrawCueStick(ID2D1RenderTarget* pRT)
{
    ...
    const float totalLength = 220.0f * scale;
    ...
    for (const auto& b : balls) {
        if (b.isPocketed) {
            ...
            if (isPlayer1Ball && p1DrawnCount < maxBallsToShow) {
                pBallBrush->SetColor(b.color); // FIXED
                ...
                pRT->FillEllipse(&ballEllipse, pBallBrush); // FIXED
                p1DrawnCount++;
            }

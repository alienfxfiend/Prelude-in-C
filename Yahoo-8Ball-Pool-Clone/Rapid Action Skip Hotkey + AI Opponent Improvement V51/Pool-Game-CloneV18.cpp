...
LRESULT CALLBACK WndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_KEYDOWN:
    {
        ...
        if (wParam == VK_F2) {
            ...
            MessageBox(hwnd,
                L"Direct2D-based StickPool game made in C++ from scratch (11885+ lines of code)\n"
...
// making angled shots and avoids hitting the near-side "horn" of the pocket.
    ...
    // Heavily penalize shots that are not straight
       // [+] Pro AI Algorithm: Logarithmic Cut-Angle Penalty
   // Highly angled shots are exponentially harder. The AI now calculates the 'Margin of Error'
    float cutAngle = acosf(dot); // radians
    float errorMargin = sinf(PI / 2.0f - cutAngle) * (GetPocketVisualRadius(pocketIndex) / targetToPocketDist);

    // Score based on likelihood of success (errorMargin) and positional leave
    shotInfo.score = (errorMargin * 5000.0f) + (distanceScore * 0.5f);

    // Pruning: If the error margin is too small for the current difficulty, discard the shot
    float threshold = (aiDifficulty == HARD) ? 0.02f : 0.05f;
    if (errorMargin < threshold) { shotInfo.possible = false; return shotInfo; }

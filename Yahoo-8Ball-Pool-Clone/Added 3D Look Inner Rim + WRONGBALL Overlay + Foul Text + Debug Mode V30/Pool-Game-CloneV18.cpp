...
void DrawTable(ID2D1RenderTarget* pRT, ID2D1Factory* pFactory) {
    ...
    if (pPocketBackdropBrush) {
        for (int i = 0; i < 6; ++i) {
            float currentVisualRadius = (i == 1 || i == 4) ? MIDDLE_HOLE_VISUAL_RADIUS : CORNER_HOLE_VISUAL_RADIUS;
            // Make funnel backdrop slightly larger to accommodate the brown funnel
            D2D1_ELLIPSE backdrop = D2D1::Ellipse(pocketPositions[i], currentVisualRadius * 1.2f + 5.0f, currentVisualRadius * 1.2f + 5.0f);
    ...
    // --- FOOLPROOF FIX: Draw Inner Chamfered Rim on all 4 Sides ---
    const float rimWidth = 15.0f; // This MUST match the rimWidth in CheckCollisions

    // Create a single Gradient Stop Collection to be reused for all 4 sides
    ID2D1GradientStopCollection* pStops = nullptr;
    D2D1_GRADIENT_STOP stops[2];
    stops[0] = { 0.0f, Lighten(TABLE_COLOR, 1.5f) }; // Lighter edge
    stops[1] = { 1.0f, Lighten(TABLE_COLOR, 0.7f) }; // Darker edge
    pRT->CreateGradientStopCollection(stops, 2, &pStops);

    if (pStops) {
        // 1. TOP RIM
        ID2D1LinearGradientBrush* pTopBrush = nullptr;
        pRT->CreateLinearGradientBrush(
            D2D1::LinearGradientBrushProperties(D2D1::Point2F(TABLE_LEFT, TABLE_TOP), D2D1::Point2F(TABLE_LEFT, TABLE_TOP + rimWidth)),
            pStops, &pTopBrush);
        if (pTopBrush) {
            pRT->FillRectangle(D2D1::RectF(TABLE_LEFT, TABLE_TOP, TABLE_RIGHT, TABLE_TOP + rimWidth), pTopBrush);
            SafeRelease(&pTopBrush);
        }

        // 2. BOTTOM RIM
        ID2D1LinearGradientBrush* pBottomBrush = nullptr;
        pRT->CreateLinearGradientBrush(
            D2D1::LinearGradientBrushProperties(D2D1::Point2F(TABLE_LEFT, TABLE_BOTTOM), D2D1::Point2F(TABLE_LEFT, TABLE_BOTTOM - rimWidth)),
            pStops, &pBottomBrush);
        if (pBottomBrush) {
            pRT->FillRectangle(D2D1::RectF(TABLE_LEFT, TABLE_BOTTOM - rimWidth, TABLE_RIGHT, TABLE_BOTTOM), pBottomBrush);
            SafeRelease(&pBottomBrush);
        }

        // 3. LEFT RIM
        ID2D1LinearGradientBrush* pLeftBrush = nullptr;
        pRT->CreateLinearGradientBrush(
            D2D1::LinearGradientBrushProperties(D2D1::Point2F(TABLE_LEFT, TABLE_TOP), D2D1::Point2F(TABLE_LEFT + rimWidth, TABLE_TOP)),
            pStops, &pLeftBrush);
        if (pLeftBrush) {
            pRT->FillRectangle(D2D1::RectF(TABLE_LEFT, TABLE_TOP + rimWidth, TABLE_LEFT + rimWidth, TABLE_BOTTOM - rimWidth), pLeftBrush);
            SafeRelease(&pLeftBrush);
        }

        // 4. RIGHT RIM
        ID2D1LinearGradientBrush* pRightBrush = nullptr;
        pRT->CreateLinearGradientBrush(
            D2D1::LinearGradientBrushProperties(D2D1::Point2F(TABLE_RIGHT, TABLE_TOP), D2D1::Point2F(TABLE_RIGHT - rimWidth, TABLE_TOP)),
            pStops, &pRightBrush);
        if (pRightBrush) {
            pRT->FillRectangle(D2D1::RectF(TABLE_RIGHT - rimWidth, TABLE_TOP + rimWidth, TABLE_RIGHT, TABLE_BOTTOM - rimWidth), pRightBrush);
            SafeRelease(&pRightBrush);
        }

        SafeRelease(&pStops);
    }
    // --- END OF 4-SIDED RIM DRAWING ---
    ...
    // --- FOOLPROOF FIX 3: Draw Funnel Pockets ---
    ID2D1SolidColorBrush* pFunnelBrush = nullptr;
    pRT->CreateSolidColorBrush(D2D1::ColorF(0.3f, 0.15f, 0.0f), &pFunnelBrush); // Dark brown for funnel
    ID2D1SolidColorBrush* pPocketBrush = nullptr;
    //ID2D1SolidColorBrush* pRimBrush = nullptr;
    pRT->CreateSolidColorBrush(POCKET_COLOR, &pPocketBrush);
    //pRT->CreateSolidColorBrush(D2D1::ColorF(0.1f, 0.1f, 0.1f), &pRimBrush);
    
    if (pPocketBrush && pFunnelBrush) {
        for (int i = 0; i < 6; ++i) {
            float radius = (i == 1 || i == 4) ? MIDDLE_HOLE_VISUAL_RADIUS : CORNER_HOLE_VISUAL_RADIUS;
            // First, draw a larger brown funnel shape
            pRT->FillEllipse(D2D1::Ellipse(pocketPositions[i], radius * 1.2f, radius * 1.2f), pFunnelBrush);
            // Then, draw the smaller black hole on top
            pRT->FillEllipse(D2D1::Ellipse(pocketPositions[i], radius, radius), pPocketBrush);
        }
    }
    SafeRelease(&pFunnelBrush);
    ...
    if (!pBrush) return;
    // --- FOOLPROOF FIX 4: Correct Kitchen Line Drawing ---
    pRT->DrawLine(
        D2D1::Point2F(HEADSTRING_X, TABLE_TOP + rimWidth), // Start line at the inner rim
        D2D1::Point2F(HEADSTRING_X, TABLE_BOTTOM - rimWidth), // End line at the inner rim
        pBrush,
        1.0f
    );
    SafeRelease(&pBrush);

    // --- FOOLPROOF FIX 5: Correct Kitchen Semicircle Drawing ---
    ...
    if (SUCCEEDED(hr) && pGeometry)
    {
        ...
        {
            float radius = 60.0f;
            // Center the semicircle on the kitchen line, within the inner rim
            D2D1_POINT_2F center = D2D1::Point2F(HEADSTRING_X, (TABLE_TOP + rimWidth + TABLE_BOTTOM - rimWidth) / 2.0f);
...
void DrawAimingAids(ID2D1RenderTarget* pRT) {
    ...
    const float rimWidth = 15.0f; // must match rimWidth used elsewhere in the project
    const float rimLeft = TABLE_LEFT + rimWidth;
    const float rimRight = TABLE_RIGHT - rimWidth;
    const float rimTop = TABLE_TOP + rimWidth;
    const float rimBottom = TABLE_BOTTOM - rimWidth;

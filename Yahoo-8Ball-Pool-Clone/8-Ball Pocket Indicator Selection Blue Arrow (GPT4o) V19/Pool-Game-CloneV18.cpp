...
void ProcessShotResults() {
    ...
    // --- Step 1: Foul detection, suppressed if any object ball pocketed ---
    bool turnFoul = false;
    // If you pocketed at least one object ball (IDs != 0), skip all foul checks
    bool anyObjectPocketed = false;
    for (int id : pocketedThisTurn) {
        if (id != 0) { anyObjectPocketed = true; break; }
    }
    if (!anyObjectPocketed) {
        // Only then do we check cue scratches or true misses
        if (cueBallPocketed) {
            turnFoul = true;
        }
        else {
            ...
            if (!fh) {
                turnFoul = true; // no hit = foul
            }
            else {
                // existing first?hit rules:
                bool wasOn8 = (shootingPlayer.assignedType != BallType::NONE
                ...
                if (player1Info.assignedType != BallType::NONE) {
                    if (wasOn8) {
                        ...
                        if (fh->type != shootingPlayer.assignedType) turnFoul = true;
                    }
                }
            }
        }
        // existing "no rail after contact" foul
        if (!turnFoul

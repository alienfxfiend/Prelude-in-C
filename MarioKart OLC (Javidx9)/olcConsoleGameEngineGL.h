...
class olcConsoleGameEngine
{
	...
	HWND ConstructWindow(int width, int height)
	{
		...
		// Replace the original WNDCLASS with this WNDCLASSEX:
		WNDCLASSEX wc =
		{
			sizeof(WNDCLASSEX),                                // cbSize
			CS_HREDRAW | CS_VREDRAW | CS_OWNDC,                 // style
			olcWndProc,                                        // lpfnWndProc
			0,                                                 // cbClsExtra
			0,                                                 // cbWndExtra
			hInstance,                                         // hInstance
			LoadIcon(hInstance, MAKEINTRESOURCE(IDI_APPLICATION)),   // hIcon (Large Icon)
			LoadCursor(NULL, IDC_ARROW),                       // hCursor
			NULL,                                              // hbrBackground
			NULL,                                              // lpszMenuName
			wnd_class,                                         // lpszClassName
			LoadIcon(hInstance, MAKEINTRESOURCE(IDI_ICON1))    // hIconSm (Small Icon for titlebar)
		};

		RegisterClassEx(&wc);

		DWORD dwExStyle = WS_EX_APPWINDOW | WS_EX_WINDOWEDGE | WS_EX_ACCEPTFILES;
		DWORD dwStyle = WS_CLIPSIBLINGS | WS_CLIPCHILDREN | WS_OVERLAPPEDWINDOW | WS_VISIBLE | WS_MAXIMIZE;
